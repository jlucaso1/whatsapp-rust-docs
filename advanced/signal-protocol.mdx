...
- **Admin revoke:** When you send an admin revoke (`RevokeType::Admin`), the client treats it as an ordinary group message for distribution purposes — it takes the same incremental-targeting diff described above (own-device exception included), rather than a message-type-specific override ([#1278](https://github.com/oxidezap/whatsapp-rust/pull/1278)). Earlier versions forced full redistribution on every admin revoke on the mistaken assumption that the server required it; on a warm group this could turn a single-recipient revoke into one `<enc>` node per device. A cold group (no cached key, or mid-rotation) still distributes to every device, same as a cold ordinary send.
