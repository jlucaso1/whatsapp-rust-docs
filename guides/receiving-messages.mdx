...
The client automatically sends delivery receipts for successfully decrypted messages. During an offline-sync reconnect, receipts are buffered and flushed as one aggregate `<receipt>` per (chat, author) group rather than one stanza per message — matching WhatsApp Web's `sendAggregateOfflineReceipts`. Live receipts keep the existing per-message path:
