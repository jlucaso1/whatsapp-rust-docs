...
This is the counterpart to `OfflineSyncCompleted`, not a variant of it: the drain did not finish, the client is not caught up, and the rest of the backlog is still queued server-side. No receipt was sent for anything in this drain — offline delivery receipts are only flushed by a *completed* drain — so the server redelivers the whole backlog, including whatever batch was still in flight when the connection dropped, once a new connection sends its own `OfflineSyncPreview`. A consumer that gates "caught up" UI or startup work on `OfflineSyncCompleted` should treat this event as "not caught up yet, wait for the next preview" rather than as completion.
...
If instead the connection ends before the drain finishes, the resume is never left silent: it is reported as `OfflineSyncInterrupted` rather than `OfflineSyncCompleted`, and the undelivered backlog comes back on the next connection's `OfflineSyncPreview` (added in PR #1380).
</Note>
