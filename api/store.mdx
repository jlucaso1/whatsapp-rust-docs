...
Use `db_err` for convenient error conversion:

```rust
use wacore::store::error::db_err;

async fn my_operation(&self) -> Result<()> {
    self.connection.execute("...")
        .await
        .map_err(db_err)?;  // Converts any Display error to StoreError::Database
    Ok(())
}
```
...
```rust
match store.load_identity("address").await {
    Ok(Some(key)) => {
        // Identity found
    }
    Ok(None) => {
        // No identity stored for this address
    }
