...
```toml Cargo.toml
[dependencies]
whatsapp-rust = { version = "0.7", features = ["tracing"] }
tracing = "0.1"
...
| Area | Covers |
|------|--------|
| `wa.conn.*` | Connect, disconnect, reconnect, handshake, frame decrypt, stream errors, plus debug-level entry/exit events for the read loop and keepalive loop — neither loop is itself wrapped in a span (see [Levels](#levels) below) |
