...
| `mlow-fast-fft` | FFT real, opcional, para o estágio de análise do encoder MLow, calculada por uma transformada complexa de meio comprimento em vez de uma de comprimento total — mesma matemática, ~27% menos instruções no encoder, mas o bitstream de saída não é bit-idêntico ao do encoder de referência. Implica `voip-mlow`; desabilitada por padrão porque o bitstream resultante ainda não foi validado contra um decoder real do WhatsApp. Veja [Chamadas VoIP](/guides/voip-calls#mlow-codec) | ❌ Não |
