...
- **Shared message encode in DM send** — The DM send path (`prepare_dm_stanza`) encodes the message once and threads those bytes through both the reporting token's whitelisted-field extraction (`generate_reporting_token_from_encoded`) and the wire plaintext construction (`pad_with_context_from_encoded` / `dm_plaintexts_from_encoded`), eliminating a redundant full protobuf encode per token-bearing DM. The rare mci-hoist path (message carries a top-level `message_context_info`) keeps separate encodes because its plaintext folds the reporting secret into the existing mci, diverging from the bytes the token is computed over.
- **Early reporting token skip** — Message types that carry no reporting token (reactions, poll votes, keep-in-chat) now bail before any secret generation, JID stringification, HKDF derivation, or message encoding via an upfront `should_include_reporting_token` check in `generate_reporting_token`. Previously all of that work was performed and then discarded when the content extractor returned `None`.
