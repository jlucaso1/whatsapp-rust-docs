...
For more comprehensive integration benchmarks, the `bench-integration` suite measures real-world scenarios (connect, send, receive, reconnect) against a mock server:
...
For low-level protocol benchmarks, the `wacore` crate includes a [divan](https://github.com/nvzqz/divan) benchmark suite — tracked in CI through [CodSpeed](https://codspeed.io) — covering the full send/receive pipeline (DM and group messaging with various participant counts), binary protocol encoding, Signal Protocol operations, and reporting token generation:

```bash
# Run all wacore benchmarks
