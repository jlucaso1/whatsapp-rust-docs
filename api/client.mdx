...
Access profile management operations (requires `Arc<Client>`).

**Methods:**
- `set_push_name(name: &str)` - Set display name
- `set_status_text(text: &str)` - Set "About" text
- `set_profile_picture(image_data: Vec<u8>)` - Set profile picture
...
See [Profile](/api/profile) for full documentation.
...
Access chat management actions (requires `Arc<Client>`).
...
- `mute_chat_until(jid: &Jid, timestamp_ms: i64)` - Mute until a specific time
...
See [Chat Actions](/api/chat-actions) for full documentation.
