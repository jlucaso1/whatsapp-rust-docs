...
Per-key X25519 generation and prost encoding for the batch are offloaded via `wacore::runtime::blocking` (runtime-agnostic; runs inline on wasm) since the caller-controlled batch size can be large.
...
- **No private keys or secrets are ever logged** — `ChainKey`, `MessageKeys`, `RootKey`, and `SenderKeyState` do not expose private key bytes through logging; `SenderKeyState` has a manual `Debug` impl that shows only structural metadata and redacts the signing key as `"<redacted>"`
...
### Sender key signing cache

`SenderKeyState` memoizes the parsed signing `PrivateKey` in a `OnceLock<PrivateKey>` field alongside the protobuf state. The implementation pre-warms the key via `PrivateKey::precompute_signing_cache()` before memoization — clones of an initialized `OnceLock` carry its contents, so every caller receives a warm clone and skips the XEdDSA scalar + Edwards basepoint re-derivation.

**Population points:**

- **`SenderKeyState::new`** — inserts the caller-supplied key directly; no re-deserialization
- **`signing_key_private()` on first call after a cold load** — deserializes, warms, then memoizes the key; a benign concurrent-first race computes equal values and `OnceLock` discards the loser silently

The memo is never serialized (protobuf surface unchanged). After a cold database load the memo is absent and rebuilds on the first `signing_key_private()` call. Since the moka record cache writes the live `SenderKeyState` back after every send, the warm memo effectively persists for the cache lifetime.

**New public entry point:** `PrivateKey::precompute_signing_cache()` — side-effect-only; pre-derives the XEdDSA scalar and Edwards basepoint. Clones of a pre-warmed key carry the warm cache.

**Measured impact (2000-iteration `group_encrypt` loop, core-pinned `perf stat`):**
...
This optimization is exclusive to sender-key (group) sends. `group_decrypt` verifies with the public key and is unaffected; session (DM) messages authenticate via HMAC, not per-message XEdDSA.

**Security:** `SenderKeyState` previously used `#[derive(Debug)]`, printing the raw private signing key bytes into any `{:?}` log or panic message. It now has a manual `Debug` impl that shows `chain_id`, `chain_iteration`, and `message_keys` count with `signing_key: "<redacted>"`.

Location: `wacore/libsignal/src/core/curve.rs`, `wacore/libsignal/src/protocol/sender_keys.rs`
