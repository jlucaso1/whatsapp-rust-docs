[Resource from github at repo://oxidezap/whatsapp-rust-docs/sha/b61b832df2fbf80e918f171acf2aa03ef136ae48/contents/advanced/signal-protocol.mdx] ---
...
`sort_dedup_by_device` keys on `Jid::identity_agent()` rather than the raw `agent` field. Its notion of "same device" then matches exactly what `Jid`'s `PartialEq`/`Hash` already treat as equal (see [Binary Protocol](/advanced/binary-protocol#jid-encoding) for why the two fields differ). Both directions matter here. Keying on the raw `agent` field can leave two JIDs that are actually one device — an inert agent byte on `Pn`/`Lid`/`Hosted`/`HostedLid`, but the same AD-JID and the same Signal address — both surviving the dedup, which gives that one session two concurrent encryption jobs. Dropping `agent` from the key entirely goes too far the other way: it silently collapses two genuinely distinct `@bot`/`@interop` devices, which *do* render the agent, losing a fan-out destination.
