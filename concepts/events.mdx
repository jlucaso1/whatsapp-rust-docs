...
<Note>
  [PR #1355](https://github.com/oxidezap/whatsapp-rust/pull/1355) added `video_orientation`. `IncomingCall` only ever fires for a stanza the peer sent, so this is always *their* rotation, never your own outgoing stanza's — the caller's, carried on the `Offer` a callee receives, or the callee's, carried on the `Accept` a caller receives back. A video-from-start party announces its camera rotation exactly once, in that stanza, and sends no mid-call `<video>` of its own until the camera actually turns. Before this field existed, that initial rotation was dropped, so a call starting with the peer's phone already sideways rendered upright until their next rotation change. See [Video I/O](/guides/voip-calls#video-io) in the VoIP guide for how this compares to `VideoFrame::orientation`.
</Note>
