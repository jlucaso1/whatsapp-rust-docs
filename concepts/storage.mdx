...
- **Redundant write elimination**: Before marking a key as dirty, `put()` compares the incoming bytes against the cached value — if they are identical, the write is skipped entirely. This avoids unnecessary dirty marks and reduces database writes during `flush()`, particularly for Signal sessions and identities that are loaded and "saved" without actual changes
