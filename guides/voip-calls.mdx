...
**We initiate** — upgrade an audio-only call to video:

```rust
// Sends the upgrade request; the peer's acceptance arrives as
// CallEvent::VideoStateChanged { state: VideoState::UpgradeAccept, .. } on handle.events().
