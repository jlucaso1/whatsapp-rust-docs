...
|---------|---------|-------|
| `pool_size` | `1` | Single connection; store serializes via its internal semaphore |
| `read_pool_size` | `0` | Extra connections reserved for reads only; `0` queues reads on the same permit as writes, unchanged from before this knob existed |
...
pub struct SqliteStoreConfig {
    /// Max concurrent operations: r2d2 `max_size` AND the internal semaphore
    /// permits, kept in lockstep. Clamped to at least 1.
    ///
...
// High-throughput profile for a single busy session
let backend = Arc::new(
...
// Let reads run alongside a pending write, without raising pool_size
let backend = Arc::new(
    SqliteStore::with_config_for_device(
        "whatsapp.db",
        1,
...
```toml
[dependencies]
whatsapp-rust = { version = "0.6", features = ["debug-snapshots"] }
```
