...
Added in [#1362](https://github.com/oxidezap/whatsapp-rust/pull/1362). A DM whose recipient half loses some but not all of its devices still builds a stanza, gets acked, and returns `Ok(SendResult)` with a real message id — matching WA Web, which builds `<participants>` from whichever per-device encryptions survived and only rejects when every device fails. `SendResult::recipient_fanout` is how a caller sees that this happened, since the wire outcome alone can't distinguish a full delivery from a partial one:
...
This is purely additive: a caller that ignores the field, or matches on an older `SendResult` shape without it (not possible across the `#[non_exhaustive]` boundary, but worth stating), sees no change in behavior. Nothing about the wire stanza or the DM's success on a partial fan-out changes — erroring on a partial fan-out was considered and rejected, since it would be a divergence from WA Web; it stays a policy the caller opts into by inspecting `is_partial()` and `skipped_primary`.
