...
if let Some(lid) = &tags.lid {
    tracing::info!(lid = %lid, "custom event");
}
if let Some(pn) = &tags.pn {
    tracing::info!(pn = %pn, "custom event");
}
```

`identity_tags()` returns an `IdentityTags { lid: Option<String>, pn: Option<String> }` snapshot — a named struct rather than a tuple, so LID/PN cannot be silently transposed at the call site. `pn` is already the redacted `pn#<token>` form produced by `Jid::observe()`, not the raw phone number, so logging it directly — as in the example above — does not leak PII; `lid` is rendered in full since it is pseudonymous. Both fields read from the same device snapshot used internally, so they stay consistent with what the library's own spans record.
