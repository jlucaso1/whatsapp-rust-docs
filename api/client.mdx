...
| `pending_device_sync` | `usize` | Users with a device refresh queued (offline path) or in flight (online path), added when a refresh is requested and suppressing a repeat request for the same user. Offline entries are drained by the next backlog drain; an online-path entry is released as soon as its refresh finishes, so outside a drain this counts only what is actually in flight. A value that stays nonzero across repeated samples means refreshes aren't completing, not that many users were seen ([#1273](https://github.com/oxidezap/whatsapp-rust/pull/1273), [#1408](https://github.com/oxidezap/whatsapp-rust/pull/1408)) |
