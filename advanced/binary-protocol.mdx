...
Both single-byte and double-byte tokens are stored in a single compile-time [hashify](https://crates.io/crates/hashify) PTHash map (using FNV-1a hashing), generated by a build script from `tokens.json`. A single call to `index_of_token` resolves any known protocol string:
