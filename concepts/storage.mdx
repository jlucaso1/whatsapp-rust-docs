...
pub struct HashState {
    ...
    // it as `false` and re-bootstrap once rather than being mistaken for an
    // already-synced-but-empty collection.
    pub bootstrapped: bool,
}
```

Call `state.has_baseline()` to check whether there's a real ltHash to build a patch on top of (`bootstrapped || version > 0`) — that's the question that matters before sending an outgoing patch, distinct from asking whether a bootstrap run specifically finished.
