...
// Check the envelope type and encrypted media type
match &info.r#type {
    Some(StanzaMessageType::Text) => println!("Text envelope"),
    Some(StanzaMessageType::Media) => println!("Media envelope"),
    Some(other) => println!("Envelope type: {other:?}"),
    None => println!("No type attribute on the stanza"),
}

if let Some(media_type) = &info.media_type {
    println!("Enc media type: {}", media_type.as_str());
}
...
Two fields that read like message metadata are not on `MessageInfo`. They're on the `InboundMessage` event that carries `info` instead. Writing them onto the shared `info` would need an `Arc::make_mut` copy of the whole struct on every message; putting them on the event, which is already built fresh per dispatch, avoids that copy:

```rust
for InboundMessage { ephemeral_expiration, comment_target, .. } in batch.iter() {
...
Inside a `MessageContext` (via `on_message` or `MessageContext::from_inbound`), the same two fields are available as `ctx.ephemeral_expiration` / `ctx.comment_target` — see [`MessageContext`](/api/bot#messagecontext).

<Warning>
Breaking change: `MessageInfo::r#type` changed from `String` to `Option<StanzaMessageType>`, and `MessageInfo::media_type` changed from `String` to `Option<EncMediaType>`. Both are `WireEnum`s carrying an `Unknown(String)` fallback for a value this build doesn't model, so no wire value is dropped.

Migrate a string comparison like `info.r#type == "text"` to `info.r#type == Some(StanzaMessageType::Text)`, or keep comparing strings with `info.r#type.as_ref().map(StanzaMessageType::as_str) == Some("text")`. The same shape applies to `media_type` with `EncMediaType`. Code that previously treated `""` as "no type" now matches `None` instead — the empty-string sentinel is gone.

`media_type` is populated by aggregating the `mediatype` attribute across the stanza's `<enc>` nodes: the first `<enc>` that declares one wins, in enumeration order (direct children first, then this device's own `<enc>` under `<participants>`). A later disagreeing value on the same stanza is dropped.
</Warning>

`InboundMessage::ephemeral_expiration` contains the disappearing messages timer in seconds. The client extracts it from the decrypted message's `contextInfo.expiration`. It tells you how long the message will be visible before it auto-deletes. Use this value when you send replies to the same chat via [`SendOptions.ephemeral_expiration`](/api/send#sendoptions).

You get `unavailable_request_id` when a message was recovered via PDO rather than normal decryption. It contains the PDO request message ID. Use it to correlate recovered messages with the original `UndecryptableMessage` event.

You get `InboundMessage::comment_target` when the dispatched message is a decrypted CAG channel comment. It contains the `MessageKey` of the parent post. The inner `Message` proto has no slot for the threading link, so it surfaces here instead. See [Channel comments](#channel-comments) below.
...
The client decrypts encrypted channel comments from Community Announcement Groups transparently and dispatches them as `Event::Messages` carrying the comment body. You read the parent post key off `InboundMessage::comment_target` — the inner `Message` proto has no slot for the threading link, so it surfaces there instead:
...
<Note>
When a retry is requested for a stanza that carried `decrypt-fail="hide"`, the retry receipt can report that back to the sender in a `<meta mode>` bitmask bit, gated behind two ab-props that both default off. See [`RetryRequestOptions::with_decrypt_fail_mode`](#requesting-a-retry-manually) below and [Retry receipts and decrypt-fail visibility](/api/receipt#retry-receipts-and-decrypt-fail-visibility) for the wire details.
</Note>
...
RetryRequestOptions::new()
    ...
    .with_decrypt_fail_mode(DecryptFailMode::Hide)   // carry the original stanza's decrypt-fail mode into the receipt
```
...
<Note>
`with_decrypt_fail_mode` defaults to `DecryptFailMode::Show`, meaning a caller that doesn't call it sends no `<meta mode>` node at all — behavior is unchanged unless you opt in. Pass the `DecryptFailMode` your own [decrypt-failure handling](#decrypt-fail-mode) determined for the original stanza. See [Retry receipts and decrypt-fail visibility](/api/receipt#retry-receipts-and-decrypt-fail-visibility) for what the receiving side does with the bit.
</Note>
