...
<Note>
**The sender-key repair (`markForgetSenderKey`) is not gated on finding the original message ([#1300](https://github.com/oxidezap/whatsapp-rust/pull/1300)).** A group/status send marks its *entire* distribution list as holding the sender key — including a device whose SKDM failed to encrypt — so the retry receipt's cold mark is the only path back for that device. The recent-message lookup (`RECORD_MISSING`/`MESSAGE_EXPIRED` above) governs only the **resend**: it has its own TTL (`sent_message_ttl_secs`, default 2h) and can miss on a genuine retry storm. Before #1300 the repair sat behind that lookup, so a device that outlived the TTL was stranded warm forever, even across restarts. The repair now runs before the lookup, so it fires whether or not the original message is still cached.
</Note>
...
<Warning>
Enabling a `RetryAdmission` policy has a cost beyond a dropped resend: since [#1300](https://github.com/oxidezap/whatsapp-rust/pull/1300), a missed repair is absorbing. If `admit` drops a receipt from a device whose sender key genuinely needs to be forgotten, that device stays warm (excluded from future SKDM distribution) until a later retry from the same requester is admitted. Size the policy's refill rate with that in mind — this SDK's default (policy unset) never has this failure mode, since every receipt is processed.
</Warning>
