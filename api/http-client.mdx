...
On `wasm32`, `sw.js` can't be reached from a page: it only answers 200 to a `Sec-Fetch-Site: none` request, which is a forbidden header name a browser silently drops, and no response from `web.whatsapp.com` carries `Access-Control-Allow-Origin` either way. So the wasm build instead reads the same build revision from `https://connect.facebook.net/en_US/sdk.js` — the Facebook JS SDK bundle, which Meta serves with `Access-Control-Allow-Origin: *` for cross-origin loading and no fetch-metadata gate. The number is Meta's shared `www` build revision, identical across `web.whatsapp.com`, `facebook.com`, `instagram.com`, and `messenger.com`, so it's the same revision `sw.js` would have reported. The request carries no custom headers — a browser sets its own and discards anything else. There's no fallback between the two sources: whichever one applies to the target, a failure there surfaces as `ConnectError::Version` rather than trying the other.
...
ureq 3.4 treats any 4xx/5xx as `ureq::Error::StatusCode` by default — the opposite of the `HttpClient` contract above. `UreqHttpClient` sends every request through `status_as_response`, which disables `http_status_as_error` **per request** instead of once on the shared agent. That placement matters: a caller-supplied agent from [`with_agent`](#with_agent) carries ureq's own defaults, not `UreqHttpClient`'s, so only a per-request override reaches it too:
...
4. **Timeouts** - ureq 3.4 applies per-IP connection timeouts automatically; implement request-level timeouts for reliability
