...
| `ensure_inflight` | `u64` | Addresses whose `ensure_e2e_sessions` prekey fetch is in flight, so a concurrent caller for the same address waits on it instead of fetching the same bundle again (count only). Normally zero — it only holds an address for the span of one prekey fetch. A value that stays nonzero across repeated samples points at a fetch not completing rather than at coalescing volume alone ([#1315](https://github.com/oxidezap/whatsapp-rust/pull/1315)) |
