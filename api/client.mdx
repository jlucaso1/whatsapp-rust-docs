...
Entry counts plus estimated retained heap bytes for the client's internal collections. On-demand only — it walks the in-process caches under their locks when called, and costs nothing otherwise (unused report code is eliminated by fat LTO). Counts include entries that have expired but not yet been swept: the TTL/TTI caches expire lazily (an entry leaves on the access that finds it stale, or under capacity pressure from a new insert), so a quiet connection can hold stale entries indefinitely between sweeps. [`Client::run_cache_maintenance()`](#run_cache_maintenance) sweeps them, and runs automatically on the keepalive's periodic maintenance tick, spawned as its own task (see [Periodic maintenance](/advanced/websocket-handling#periodic-maintenance)) — so a report taken between completed sweeps can read higher than a fresh lookup would, and one taken concurrently with a sweep can land on either side of it ([#1408](https://github.com/oxidezap/whatsapp-rust/pull/1408)). Every count field that has an async counterpart is read through it, so a report taken under write load doesn't silently under-read the caches that were busy. Byte figures are honest estimates rather than byte-exact accounting — Signal records use their protobuf encoded size, other collections sum key/payload capacities — suitable for per-session attribution and leak detection. Store-backed caches (e.g. Redis-backed custom stores) report `bytes: 0`, since their entries don't live in this process's memory.
...
| `lid_pn_contact_hash_entries` | `CollectionStats` | Contact-hash → LID index of the LID/PN cache, one entry per identifier (both sides of every pair). Bytes are the table alone — the LID it points at is already counted in `lid_pn_lid_entries`. Unbounded with the cache it indexes, and rebuilt from warm-up each process ([#1408](https://github.com/oxidezap/whatsapp-rust/pull/1408)) |
| `lid_pn_persisted_entries` | `CollectionStats` | PN → LID pairs this process has durably persisted, the dedup that lets the learn path skip a re-persist. Bytes are the table alone. One entry per persisted contact for the process lifetime ([#1408](https://github.com/oxidezap/whatsapp-rust/pull/1408)) |
...
| `group_metadata_inflight` | `u64` | Groups whose [`get_metadata`](/api/groups#get_metadata) query is in flight, so a concurrent caller for the same group shares that round trip instead of sending its own (count only). Normally zero — it only holds a group for the span of one query. A value that stays nonzero across repeated samples warrants investigation, though sustained request turnover across many groups can also keep it nonzero without any single query being stuck ([#1316](https://github.com/oxidezap/whatsapp-rust/pull/1316)) |
| `chat_lanes` | `u64` | Per-chat lanes — combined enqueue lock + message queue (count only) |
| `chat_lane_backlog` | `u64` | Inbound messages queued behind their chat's lane worker, summed over every lane. The lanes are capacity-bounded; their queues are not, and each queued message retains its whole inbound frame, so a worker that is stuck (a slow durability hook, a hung decrypt) shows up here as a backlog that grows with the chat's inbound rate ([#1408](https://github.com/oxidezap/whatsapp-rust/pull/1408)) |
...
| `pending_device_sync` | `usize` | Users with a device refresh queued (offline path) or in flight (online path), added when a refresh is requested and suppressing a repeat request for the same user. Offline entries are drained by the next backlog drain; an online-path entry is released as soon as its refresh finishes, so outside a drain this counts only what is actually in flight. A value that stays nonzero across repeated samples can reflect an ongoing offline drain, a failed flush awaiting retry, or sustained request turnover — as well as refreshes that aren't completing ([#1273](https://github.com/oxidezap/whatsapp-rust/pull/1273), [#1408](https://github.com/oxidezap/whatsapp-rust/pull/1408)) |
...
`CollectionStats` carries both `entries: u64` and `bytes: u64`. `MemoryReport::total_estimated_bytes(&self) -> u64` sums `.bytes` across every byte-carrying field, `subsystems` included. `MemoryReport` implements `Display` for a pretty-printed, human-readable breakdown. This output includes an `--- In-flight history sync ---` section with the two peak fields above, followed by a `--- Transient retention ---` section for `inbound_commit_batch`, `msg_secret_buffer`, and `pending_device_sync` ([#1273](https://github.com/oxidezap/whatsapp-rust/pull/1273)).
...
Sweeps expired entries out of the in-process TTL/TTI caches (`recent_messages`, `message_retry_counts`, `session_recreate_history`, `undecryptable_dispatched`, `dispatched_messages`, `pdo_pending_requests`, `pdo_requested`, `device_registry_cache`, `sender_key_device_cache`, `lid_pn_cache`, and `group_cache` if built). Store-backed caches expire on their own and no-op here; capacity-only caches have nothing to expire and are left alone.

Called automatically on the keepalive's ~5-minute periodic maintenance tick (see [Periodic maintenance](/advanced/websocket-handling#periodic-maintenance)) — most consumers never need to call it directly. Without it, a client that fanned out to a large group set once would keep every one of those records — up to the 20,000-entry `device_registry_cache` — for as long as it stayed connected, since these caches only expire lazily, on the access that finds an entry stale or on capacity pressure from a new insert, neither of which happens on a quiet connection ([#1408](https://github.com/oxidezap/whatsapp-rust/pull/1408)).
