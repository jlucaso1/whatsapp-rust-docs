...
`set_link_previews_disabled` is a distinct privacy control from everything above. Every method in [Methods](#methods) goes through the `set_privacy` IQ namespace and is answered synchronously. This one instead rides app state sync — the same mutation mechanism used by [chat actions](/api/chat-actions#app-state-sync) and [labels](/api/labels) — and has no query/fetch IQ counterpart at all.

### set_link_previews_disabled

Turn outgoing link previews off or on for the whole account. Access it through `client.app_state_settings()`, not directly on `Client`.
