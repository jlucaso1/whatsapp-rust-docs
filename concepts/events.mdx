...
<Note>
`ContactRemoved` is distinct from `ContactUpdate`. The underlying mutation is a syncd `Remove`, and carries no action payload — WhatsApp Web ignores the value on that branch and simply drops the contact from the address book. See [Chat actions — Save and remove contacts](/api/chat-actions#save-and-remove-contacts) for the outbound API (`remove_contact`) that emits this event on other devices.
</Note>
...
**Fields:**
- `previews_disabled` - `true` when link previews are now disabled. Only emitted when the wire carried the flag; WhatsApp Web treats an absent one as a malformed mutation, so no event fires for that case rather than one carrying a default `false`.
- `timestamp` - When the change occurred
...
<Note>
Unlike `DisappearingModeChanged` above — a server-push notification about *another contact's* setting — this event is app-state sync about *your own account's* setting, changed from one of your own linked devices. See [Privacy — App-state settings](/api/privacy#app-state-settings-syncd) for the outbound API (`set_link_previews_disabled`) that emits this event on other devices.
</Note>
