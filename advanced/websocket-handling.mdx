...
| **500** / unknown / missing code | Logs as warning, stays connected (since v0.6) | None | N/A — no disconnect; session preserved |
| `<stream:error>` with an `<xml-not-well-formed/>` child (no numeric code) | Force-closes the socket | None | Yes — standard backoff |
