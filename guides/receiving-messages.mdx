...
For `ViewOnce`, `Hosted`, and `Bot`, only steps 1–2 happen: the client dispatches `UndecryptableMessage` and acks immediately. There is no PDO round-trip and no follow-up `Event::Message` to wait for.
...
Register an `InboundDurabilityHook` to defer the ack until your consumer durably commits the message:
...
The hook **must be idempotent** — deduplicate by `(info.source.chat, info.source.sender, info.id)` since a crash after the consumer commits but before the ack lands will replay the message.

See [Inbound Durability Hook](/advanced/inbound-durability) for the full contract, caveats, and a worked example.
