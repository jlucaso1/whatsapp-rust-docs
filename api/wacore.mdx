...
- `crypto` - Cryptographic primitives (HKDF, HMAC, AES-CBC, AES-GCM). Noise transport exports:
  - `Aes256GcmKey` — connection-lifetime pre-keyed AEAD state that holds the expanded AES cipher and GHASH subkey
  - `TransportAead` — pluggable AEAD interface used by `NoiseCipher`
  - `transport_aead(key: &[u8; 32])` — creates a `Box<dyn TransportAead>` from the active `SignalCryptoProvider`
  - `SignalCryptoProvider::transport_aead` — optional method with a default implementation that preserves existing behavior; override only to precompute key-dependent state once per connection for improved performance
...
- `IkServerHelloOutcome` - Either `Continue(Box<IkHandshakeOutcome>)` when IK succeeds or `Fallback` to pivot into XXfallback. `Continue` boxes its payload because `IkHandshakeOutcome` holds two `NoiseCipher` fields, making it oversized relative to the already-boxed `Fallback` variant (clippy's `large_enum_variant` lint).
...
- **Pre-keyed Noise transport cipher** — `NoiseCipher` holds a `Box<dyn TransportAead>` backed by a pre-keyed `Aes256GcmKey` (via `RustCryptoProvider`). The AES key schedule and GHASH subkey are computed once per connection instead of on every frame. Decrypt improves ~20% on 1500-byte stanzas (AES-NI host); encrypt improves ~7%. See [changelog](/changelog/2026-06-11-noise-prekeyed-gcm).
