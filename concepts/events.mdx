...
<Note>
Breaking change: `ephemeral_expiration` and `comment_target` moved onto `InboundMessage` from `MessageInfo`, where earlier docs (and some examples below) still show them as `info.ephemeral_expiration` / `info.comment_target`. They used to be written into the shared `MessageInfo` via `Arc::make_mut` — which deep-copied the whole struct on every message in a disappearing chat — so moving them to the event, where they're already known, lets `info` stay a cheap shared `Arc` for every message. Read them by destructuring the field directly off `InboundMessage` (`for InboundMessage { comment_target, .. } in batch.iter()`) rather than off `info`. [`MessageContext`](/api/bot#messagecontext) carries both fields too — `MessageContext::from_inbound` (what `Bot::on_message` uses internally) copies them from the event, so a bot handler can read `ctx.ephemeral_expiration` / `ctx.comment_target` directly.
</Note>
