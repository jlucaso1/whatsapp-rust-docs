...
/// pairing code. The consumer should request a fresh code via
...
Event::PairingCodeRefresh(PairingCodeRefresh { force_manual, .. }) => {
    // The previous code is no longer guaranteed valid — request a new one,
    // e.g. by calling `client.pair_with_code(options)` again.
...
<Note>
This is a pure observability hook. It does not replace or gate the client's built-in handling. The client always sends the matching `<clean>` IQ, throttled behind offline-sync completion for `Groups`/`NewsletterMetadata` (per `WAWebHandleDirtyBits`), and, for `SyncdAppState`, re-syncs all app-state collections — exactly as it did before this event existed. The bus dispatches `DirtyState` immediately before that built-in work starts, but whether your handler actually finishes first depends on how it's registered. A struct-based [`EventHandler`](#eventhandler-trait) runs `handle_event` inline on the dispatch path, so it completes before the built-in resync begins. A `Bot` closure — under either [`EventDelivery`](#delivery-order-and-backpressure) mode — runs on a separate task, so it is not guaranteed to finish before the built-in resync starts; register a struct-based handler instead if that ordering matters to you. Either way, you don't need to parse raw `<ib>` stanzas via [`RawNode`](#raw-stanza-events) to observe this event.
</Note>
