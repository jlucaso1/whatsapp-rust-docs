...
[`with_http_client`](#with_http_client) for a client that's already behind an `Arc<dyn HttpClient>`. Reach for this instead of `with_http_client(shared.clone())` when a process runs several bots and you want them all pointing at one client — and one connection pool — rather than a clone apiece per builder. A single non-`Clone` client passed by value to one builder already works fine with `with_http_client` (it carries no `Clone` bound); this setter is what a non-`Clone` client needs to reach *more than one* builder, and it's the only route in for a client already type-erased to `Arc<dyn HttpClient>` because the host picks it at runtime.
