...
The sender-side issuance timestamp isn't carried over in the migration — it only gates re-issuing your own token to the contact, so losing it costs at most one extra issuance, whereas the received token itself is what the send path can't do without ([#1345](https://github.com/oxidezap/whatsapp-rust/pull/1345)).
