...
`PrimaryDeviceRejected` — added in [PR #1362](https://github.com/oxidezap/whatsapp-rust/pull/1362). The pre-key fetch that establishes sessions ahead of a DM send got back a `406` naming a primary device (device 0) — either the recipient's or the sender's own. Nothing was built or sent. This mirrors WA Web's `ensureE2ESessions`, which throws on a named rejection unless every rejected device is a companion (`device != null && device !== DEFAULT_DEVICE_ID`); a companion's `406`, and any non-406 rejection code even on a primary, still behave as before — refreshed and skipped, not fatal. The device lists the fetch named have already been refreshed by the time this returns, so an immediate retry re-resolves them rather than repeating the same question. Distinct from `NoRecipientDevice`: that variant means encryption was attempted and failed or had nothing to attempt; this one means the fetch that would have supplied key material was refused outright before encryption started.
...
```rust
// Before — Ok told you the server accepted the stanza, nothing about who got it
...
Separately, a DM phash mismatch (see [Phash validation](/advanced/signal-protocol#phash-validation-for-stale-device-list-detection)) now re-resolves the peer's device list and may resend the message pairwise, under the original message id, to devices that list holds and the original stanza did not cover. Previously a mismatch only invalidated caches. This has no `SendError`/`SendResult` shape to match on — it runs after the original `send_message` call has already returned — but it means a receiver can see a second copy of the same message id shortly after the first, which it dedupes on the id the same way it dedupes any retransmission.

Both new variants are additive: `SendError` and `SendResult` are `#[non_exhaustive]`, and a caller that does not match `PrimaryDeviceRejected` or read `recipient_fanout` keeps behaving exactly as before.
