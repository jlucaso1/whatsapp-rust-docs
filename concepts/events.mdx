...
<Warning>
Breaking change ([whatsapp-rust#1343](https://github.com/oxidezap/whatsapp-rust/pull/1343)): `target_id` and `thread_message_id` changed from `Option<MessageId>` to `Option<CompactString>`, and `content_type`/`appdata` changed from `Option<String>` to `Option<CompactString>` (`wacore_binary::CompactString`, inline up to 24 bytes — a message id and these short keywords always fit). `reporting_tag` and `reporting_token` changed from `Option<Vec<u8>>` to `Option<ReportingBytes>`, a `SmallVec<[u8; 20]>` alias that keeps a 16-or-20-byte tag or a 16-byte token inline; a payload wider than 20 bytes still spills to the heap. A call site that assigns these fields from a `String`/`Vec<u8>` literal, or moves a value out into one, needs one `.into()` (or `.to_vec()` for `ReportingBytes` → `Vec<u8>`) per site — both types deref to `&str`/`&[u8]`, so read-only access (comparisons, `.as_deref()`, formatting) is unaffected.
</Warning>
