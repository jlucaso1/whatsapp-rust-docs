...
3. Computes the diff using a **primary-device gate** (matching WA Web's `ParticipantStore.js getGroupSenderKeyListFromParticipantRecord`): a device receives SKDM when it is cold (`has_key=false`) **or** its primary device (device 0) is cold. A cold primary redistributes the whole user; a cold companion redistributes only itself. A missing primary row is treated as cold.
