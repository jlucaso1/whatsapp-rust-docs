...
**What changed.** `SenderKeyState` now holds the backlog in a dedicated `Arc<Vec<SenderMessageKey>>` field. The protobuf struct's `sender_message_keys` list is kept empty in memory; it is only reassembled at serialization (`as_protobuf`). A `debug_assert` guards this invariant.
