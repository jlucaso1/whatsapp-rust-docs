...
Prevents concurrent Signal protocol operations on the same session. Keys are protocol address strings generated by `to_protocol_address_string()` (format: `user[:device]@server.0`), which builds the key in a single allocation:
