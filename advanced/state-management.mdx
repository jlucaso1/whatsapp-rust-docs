...
`ChatLane` itself (`src/client.rs`) carries a third field, `worker_running: Arc<async_lock::Mutex<()>>`, alongside `enqueue_lock` and `queue_tx`:
...
Location: `src/client.rs`, `src/handlers/message.rs`
