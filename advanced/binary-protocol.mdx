...
`AttrsRef<'a>` is an enum with two variants — `Empty` (no attributes) and `Slice(Box<[(NodeStr<'a>, ValueRef<'a>)]>)` (boxed slice of key-value pairs). It is **covariant** in `'a`, which is required for the `Yokeable` implementation used by `OwnedNodeRef`. A compile-time assertion enforces this:
...
If `AttrsRef` ever becomes invariant (e.g., by adding a `Cell` or `&mut`), this function will fail to compile, catching the regression before it breaks `OwnedNodeRef`.
