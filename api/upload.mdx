...
// 1. Encrypt to a temp file (constant memory)
let mut cipher_file = tempfile::tempfile()?;
let info = encrypt_media_streaming(
    File::open("big_video.mp4")?,
    BufWriter::new(&mut cipher_file),
    MediaType::Video,
)?;

// 2. Stream the ciphertext straight to the CDN — wrap the file in your own
//    UploadSource, or read it into Bytes if it fits.
let ciphertext = std::fs::read("…")?;                 // or a custom file-backed source
let upload = client
    .upload_stream(bytes::Bytes::from(ciphertext), info, MediaType::Video)
    .await?;
