...
```toml Cargo.toml
[dependencies]
whatsapp-rust = { version = "0.7", features = ["mlow-fast-fft"] }
```

<Warning>
  This is a different operation sequence for the same transform, so it does **not** reproduce the reference encoder bit for bit — MLow's pulse search, pitch estimator, and LSF quantizer all resolve ties on `f32` comparisons, so that last-bit difference cascades into a different, still well-formed bitstream. [PR #1323](https://github.com/oxidezap/whatsapp-rust/pull/1323) bounds the numerical drift and shows it's the same size as a one-ULP input nudge, but nothing in this repository's test suite can confirm a real WhatsApp client decodes the resulting bitstream the same way it decodes the reference one. The feature stays off by default — turn it on only if you can validate the output against a live WhatsApp decoder yourself.
</Warning>
