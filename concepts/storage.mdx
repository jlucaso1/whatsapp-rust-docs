...
Sessions are cached as deserialized `SessionRecord` objects (not bytes), matching WhatsApp Web's pattern where the JS object IS the cache. Serialization only happens during `flush()` — not on every `store_session` call. Identity and sender key stores use `Arc<[u8]>` byte caches.
...
- **Deferred writes**: Changes are accumulated in memory and batch-written on `flush()`. Sessions are serialized only during `flush()`, not on every store
- **Redundant write elimination**: For identities (which rarely change), `put_dedup()` compares incoming bytes against the cached value and skips if identical. Sender keys use unconditional `put()` since they change every message
