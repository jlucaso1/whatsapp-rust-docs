...
<Note>
  [PR #1355](https://github.com/oxidezap/whatsapp-rust/pull/1355) added `IncomingCall::video_orientation` (see [Events](/concepts/events#incomingcall)). A video-from-start party announces its rotation once, before any `VideoFrame` exists to carry one. It arrives on whichever call-signaling stanza that party sent — the peer's `Offer` if you're the callee, or their `Accept` if you're the caller. Read it off that `Event::IncomingCall` instead of waiting for the first frame, so a call that starts with the peer's camera already rotated doesn't render upright for a beat.
</Note>
