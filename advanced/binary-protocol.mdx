...
The `wacore_derive` crate provides macros for type-safe protocol node handling.

### ProtocolNode

Generates `ProtocolNode` trait implementation for structs:
...
- `String` - Basic string attribute
- `Jid` - WhatsApp JID with `jid` marker
- `StringEnum` - Enum types with `string_enum` marker
- `u64` / `u32` - Numeric attributes with respective markers
- All types support `Option<T>` for optional attributes

### StringEnum

Generates string conversion for enums:
...
// With fallback for unknown values
#[derive(StringEnum)]
pub enum PrivacyCategory {
    ...
    Online,
    #[string_fallback]
    Other(String),  // Captures unknown strings
}
```

**Attributes:**
- `#[str = "value"]` - Required string representation
- `#[string_default]` - Marks the default variant
- `#[string_fallback]` - Catch-all variant for unknown strings (generates `From<&str>` instead of `TryFrom<&str>`)
