...
impl<'a> HistorySyncStream<'a> {
    ...
    /// Advance to the next conversation, decoding it via prost.
...
Event::NewsletterLiveUpdate(update) => {
    ...
    for msg in &update.messages {
        for reaction in &msg.reactions {
            println!("  {} × {}", reaction.code, reaction.count);
        }
    }
}
```
...
Event::IncomingCall(call) => {
    println!("{} call from {}",
        if call.is_video { "Video" } else { "Voice" },
        call.from
    );
    // Reject call (bots cannot accept calls)
    client.reject_call(&call.id, &call.from).await?;
}
```
...
**Emitted:** When the global disappearing messages setting changes

```rust
#[derive(Debug, Clone, Serialize)]
pub struct DisappearingModeChanged {
    pub expiration: u32,
    pub initiator: DisappearingModeInitiator,
    pub timestamp: DateTime<Utc>,
}
...
**Example:**
```rust
Event::DisappearingModeChanged(mode) => {
    if mode.expiration == 0 {
        println!("Disappearing messages disabled globally");
    } else {
        println!("Default disappearing: {}s", mode.expiration);
    }
}
```
...
**Emitted:** The raw stanza received from the server, dispatched only when at least one handler has `EventKind::RawNode` in its `interest()`. When no handler wants `RawNode`, the bus skips this dispatch entirely (zero overhead on the hot path).
...
This event gives you access to every raw stanza. Use it for debugging, protocol research, or handling server messages that do not yet have a dedicated event type.

## Related

<CardGroup cols={2}>
  <Card title="Receiving messages" icon="inbox" href="/guides/receiving-messages">
    Guide to handling incoming messages
  </Card>
