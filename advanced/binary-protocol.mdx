...
A separate `JidExt` trait in `wacore::types::jid` converts JIDs into Signal Protocol address strings. The conversion maps `s.whatsapp.net` to `c.us` (matching WhatsApp Web's internal format) and formats the result as `{user}[:device]@{server}`:

| Method | Output format | Example |
|--------|--------------|---------|
| `to_signal_address_string()` | `user[:device]@server` | `5511999887766@c.us` |
| `to_protocol_address_string()` | `user[:device]@server.0` | `5511999887766@c.us.0` |
| `to_protocol_address()` | `ProtocolAddress { name, device_id: 0 }` | — |

`to_protocol_address_string()` reuses `to_signal_address_string()` internally, appending a `.0` suffix. Both methods pre-allocate capacity to avoid reallocation.

See [Signal protocol](/advanced/signal-protocol#jid-to-signal-address-conversion) for usage details.

Location: `wacore/src/types/jid.rs`
...
|--------|-----------|-------------|
| `new` | `new(tag: impl Into<String>) -> Self` | Create a builder with a tag |
| `attr` | `attr(key: impl Into<String>, value: impl Into<String>) -> Self` | Add a string attribute |
| `jid_attr` | `jid_attr(key: impl Into<String>, jid: Jid) -> Self` | Add a JID attribute without stringifying |
| `attrs` | `attrs(attrs: impl IntoIterator<Item = (K, V)>) -> Self` | Bulk-add attributes from an iterator |
