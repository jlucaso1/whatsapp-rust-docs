...
pub struct Message {
    ...
    // Group messages
    pub sender_key_distribution_message: Option<Box<SenderKeyDistributionMessage>>,
    ...
    pub edit_message: Option<Box<EditMessage>>,
    ...
    // AI/Bot messages
    pub ai_rich_response_message: Option<Box<AiRichResponseMessage>>,
    pub bot_feedback_message: Option<Box<BotFeedbackMessage>>,
    
    // Metadata
    pub message_context_info: Option<Box<MessageContextInfo>>,
    // ... and many more
}
```
...
|---|---|---|
| `generate` | Enables `prost-build` as a build dependency and runs code generation from `whatsapp.proto`. Only needed when modifying the `.proto` file — normal builds use the checked-in `whatsapp.rs`. | — |
