...
Use the typed `StoreError` variants for proper error categorization:
...
// Use db_err helper for concise error conversion
async fn put_session(&self, address: &str, session: &[u8]) -> Result<()> {
    self.connection
        .execute("INSERT OR REPLACE INTO sessions ...", &[address, session])
        .await
        .map_err(db_err)?;
    Ok(())
}
...
| `Serialization(String)` | Encode/decode failures |
| `NotFound(String)` | Missing required resources |
| `Backend(Box<dyn Error>)` | Underlying storage errors |
| `Connection(String)` | Connection establishment failures |
| `Database(String)` | Query execution errors |
| `Migration(String)` | Schema migration failures |
| `DeviceNotFound(i32)` | Device ID not in store |
