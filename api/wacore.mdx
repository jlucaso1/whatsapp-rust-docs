...
The project includes two categories of benchmarks: **protocol-level** (divan, tracked on CodSpeed) and **integration-level** (real client operations with allocation tracking).
...
| `voip_benchmark` | `wacore/benches/voip_benchmark.rs` | VoIP media-path hot loops (requires the `voip` feature) |
...
The `bench-integration` suite (`tests/bench-integration/benches/integration.rs`) drives real client operations end-to-end against a mock server. It is a divan bench target as well, so it reports through CodSpeed rather than the custom timing/allocation binary it replaced. It installs a `DeterministicAlloc` global allocator whose `realloc` always allocates-and-copies — the system allocator's in-place growth depends on live heap layout and would otherwise be charged to the benchmark as run-to-run memory noise.
