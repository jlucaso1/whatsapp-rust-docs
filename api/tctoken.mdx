...
<Note>
An entry with an empty `token` and only `sender_timestamp` set is a **byte-less placeholder** — it records that a post-send issuance IQ succeeded before any real token was received from the contact. See [Post-send issuance](#post-send-issuance).
</Note>
...
3. When sending a message to a recipient with no valid stored TC token, and `NCT_TOKEN_SEND_ENABLED` is on, the library computes `HMAC-SHA256(nct_salt, recipient_lid)` and includes it as a `<cstoken>` stanza child — independently of whether the tctoken AB prop is enabled
