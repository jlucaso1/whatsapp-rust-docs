...
**Fields:**
- `app_version_fallback` — added in PR #1360; additive, so an existing `Connected::builder().build()` still compiles (`app_version_fallback` defaults to `None`). Present when this connect could not resolve the app version from its source and settled for the version the device already held instead; absent on every normal connect, so `Some` is the whole signal. In practice this only fires on the `wasm32` target. The native build's source is `web.whatsapp.com/sw.js`; a client that can't reach it treats that as a real break of WhatsApp Web itself and fails `connect()` with `ConnectError::Version` rather than falling back — see [`ConnectError`](/api/errors#connecterror). The browser build's source, the Facebook JS SDK bundle at `connect.facebook.net`, sits on common tracker blocklists (uBlock Origin, Brave shields, corporate DNS), so a client blocked from reaching it connects anyway on the version it already has, rather than refusing to connect over what is, for a large share of browser users, an ad blocker doing its job.
...
- `AppVersionFallback::compiled_default` — `true` when that version is the one compiled into the library, so its staleness is the release's age. `false` when the device had already resolved a version on an earlier connect, so its staleness is only the length of this outage. (A `with_version` call on *this* connect never produces a fallback at all — see the [`with_version` note](/api/bot#with_version) — but a device that carried an override from a past connect and later drops it can still hit this path, and would correctly report `false` here too.)
- `AppVersionFallback::reason` — `SourceUnreachable` is the routine case and needs no action. `SourceUnparsable` means the source answered but the bundle no longer carries the field the parser looks for — worth acting on, since it won't fix itself on the next connect.
...
The next `query_info` (or a send) for an affected group transparently re-fetches its metadata from the server, since `query_info`'s default `Freshness::CachePreferred` behavior only serves the cache when it is populated. You do not need to call `query_info_with_freshness(&jid, Freshness::Refresh)` yourself in response to this notification — see [Cache group information](/guides/group-management#cache-group-information).
