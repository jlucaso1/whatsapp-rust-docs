...
If the connection ends before the drain finishes — rather than the end marker arriving or the stall timer firing — the resume is never left silent. It is reported once as `OfflineSyncInterrupted { total, delivered }` instead of `OfflineSyncCompleted` (added in PR #1380). No receipt is sent for anything in an interrupted drain, so the server redelivers the whole undelivered backlog — including the batch still in flight at teardown — the next time the client reconnects and gets a fresh `OfflineSyncPreview`.
