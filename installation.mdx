...
<Note>
  A chat/message history store (`whatsapp-rust-chat-store`) previously lived in this repository as an opt-in crate. It has been extracted into its own repository, since materializing an event stream into chats, previews, and unread counts is an application decision rather than a protocol one — it no longer ships from `whatsapp-rust`. Link to be added here once the replacement repository is published.
</Note>
