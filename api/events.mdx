...
The `Events` feature creates WhatsApp **event** messages (the scheduled-event bubble with a date, location, and join link) and sends RSVPs. Like polls, an event carries a per-message `message_secret`; responders derive their RSVP encryption key from it, so you must keep the secret to send or decrypt responses.
...
Re-exported from the generated proto.
