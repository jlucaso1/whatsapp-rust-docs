...
| `voip` | 1:1 voice and video calls, full profile: bundled MLow + libopus codec adapters, codec-neutral H.264 video plane, E2E-SRTP encryption, the native relay transport. Alias for `voip-mlow` + `voip-libopus` + `voip-relay-native` — unaffected by the split below (see [VoIP Calls](/guides/voip-calls)) | ❌ No |
| `voip-runtime` | The portable half of VoIP: call signaling, the facade, `CallHandle`. Owns no socket, and no longer requires `tokio-runtime` — builds on `wasm32`. Enabled by every `voip-*` feature below. Not meant to be named directly — pick `voip-encoded` or a codec feature instead | ❌ No |
...
<Note>
  `wacore` is not the only crate that builds for `wasm32` — as of [PR #1364](https://github.com/oxidezap/whatsapp-rust/pull/1364), `whatsapp-rust` itself does too, with `voip-mlow`, `voip-libopus`, or `voip-encoded` (never `voip-relay-native`, which needs a UDP socket the target doesn't have). Still enable `js` on `wacore` for random number generation, and install your own [`RelayTransportProvider`](/guides/voip-calls#custom-relay-transport) — an `RTCPeerConnection` in a browser — since none of those features link a way onto the media wire on their own.
</Note>
