...
| `pending_lid_refreshes` | **Deliberately not cleared** | Keys are released by a `scopeguard` that runs on drop as well as on completion, so a refresh whose query dies with the socket already frees its own key — there is nothing stale left to sweep. Clearing here would drop a reservation belonging to a live task: a refresh spanning a reconnect would release a key the new connection had since taken, and the peer would get the duplicate `<ack refresh_lid="true">` query the set exists to prevent ([#1313](https://github.com/oxidezap/whatsapp-rust/pull/1313)) |
