...
pub struct ClientProfile {
    ...
    pub locale: String,        // e.g. "en-US"; emitted as ClientPayload.lc
    pub phone_id: Option<String>, // anti-abuse phone ID; auto-generated as UUID if None
}
```

<Note>
Since v0.6 the locale and `phone_id` come from the active `ClientProfile` instead of being hard-coded to `"en"` / `None`. If `phone_id` is `None`, the client generates a UUID on first use and persists it on `Device`, matching WA Web's `WAWebClientPayload.phoneId` initialization. Set both fields explicitly via your own constructor when impersonating a specific locale/install.
</Note>
