...
| `ServerClose { code: None \| Some(1000) \| Some(1001), .. }` | `true` — no code, normal closure, or going-away are graceful |
| `ServerClose { code: Some(_), .. }` (any other code) | `false` — protocol/server error, restart, etc. |
