...
- **Sans-IO `CallEngine`** - No owned sockets or threads; the `wacore` engine targets Tokio, WASM, and embedded (esp32); the top-level `voip` feature (webrtc-rs + libopus) is native-only
