...
<Note>
When you send an admin revoke (`RevokeType::Admin`), the client no longer forces a full sender-key redistribution to deliver it. It uses the same [per-device incremental targeting](/advanced/signal-protocol#per-device-sender-key-tracking) as any other group message: devices already tracked as holding the key are skipped, except your own companion devices, which are never marked `has_key=true` and so are always included. `<participants>`/`<device-identity>` are omitted entirely once the rest of the group is warm. Previously every admin revoke re-sent the sender key to the whole device list regardless of tracking state, which on a large warm group could turn a few-hundred-byte revoke into tens of kilobytes. A cold group (or one mid-rotation) still gets full distribution, same as any other cold send. No code changes are needed; this only affects stanza size on the wire.
</Note>
