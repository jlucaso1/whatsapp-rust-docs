...
Since [#1131](https://github.com/oxidezap/whatsapp-rust/pull/1131), a fresh `ProtocolAddress` for a typical WhatsApp address (up to 47 bytes) is already allocation-free — inline storage covers it. Reuse still matters for addresses that spill to the heap: rewriting a spilled `ProtocolAddress` keeps its existing `String` allocation instead of dropping and reallocating one per device, and for a group with 100 participant devices sharing a reused buffer, that saves up to 100 heap allocations on the send path.
