...
If your process runs many WhatsApp sessions, build **one** `UreqHttpClient` and hand a clone to each `BotBuilder::with_http_client` rather than constructing one per session. `UreqHttpClient` is `Clone`, and cloning it shares the underlying `ureq::Agent` — and therefore its connection pool — instead of copying it.

This is worth doing because `Client::connect()` fetches `https://web.whatsapp.com/sw.js` to check for a version update unless you've set `with_version` or the cached version is under 24h old, so a session opens a pooled TLS connection on connect even if it never touches media. ureq keeps that connection idle rather than closing it, at roughly 98 KiB per client. Sharing one agent across sessions pays that cost once for the whole process instead of once per session — worthwhile from roughly ten concurrent sessions up.

This is safe to share: `sw.js` is unauthenticated and identical for every session, and media requests carry their own auth per request, so a shared connection doesn't leak anything between sessions that the shared source IP doesn't already reveal. Concurrency isn't affected either — ureq opens a new connection whenever no idle one matches the target host, so the pool only caps how many idle connections are *retained*, not how many requests can run at once.
