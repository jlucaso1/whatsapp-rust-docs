...
Download media and write to a file. Note that this method buffers the entire file in memory before writing (it calls `download()` internally). For true streaming downloads, use `download_to_writer` instead.
