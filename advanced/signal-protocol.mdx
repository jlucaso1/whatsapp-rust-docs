...
The per-batch upload count is configurable via `CacheConfig::wanted_pre_key_count` (default `812`, matching WhatsApp Web's `UPLOAD_KEYS_COUNT`). The upload-trigger threshold is a private constant.

| Setting | Default | Description |
|---------|---------|-------------|
| `wanted_pre_key_count` (on [`CacheConfig`](/api/bot#pre-key-pool-sizing)) | 812 | Number of pre-keys generated and uploaded per batch. Clamped to `5..=16_777_214` at upload time. |
| `MIN_PRE_KEY_COUNT` (private const) | 5 | Minimum server-side pre-key count before triggering an upload. |

```rust
use whatsapp_rust::CacheConfig;

let config = CacheConfig {
    wanted_pre_key_count: 256, // smaller batches for embedded consumers
    ..Default::default()
};
```

Values outside `[5, 16_777_214]` are clamped at upload time (an out-of-range value logs a `warn!`). The floor avoids an empty-but-flagged pool or a re-upload loop; the ceiling prevents duplicate IDs within a single 24-bit batch.
...
The public `refresh_pre_keys()` method force-uploads a fresh batch of `CacheConfig::wanted_pre_key_count` pre-keys (default 812), giving the server new IDs the caller has locally. Old unmatched IDs drain naturally as peers consume them.
