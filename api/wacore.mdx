...
- **Inline storage in `HistoryMsgSecretRecord`** — `chat_id` is `Arc<str>` (allocated once per conversation, shared across all records in that conversation), `msg_id` is `CompactString` (inline for typical 20–22 char WA IDs), and `secret` is `SecretBytes` (inline for secrets ≤32 bytes, heap for larger)
