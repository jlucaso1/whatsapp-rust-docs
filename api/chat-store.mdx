...
An outgoing row's timestamp starts as the local clock value passed to `record_outgoing`. When the server's positive message ack for that id arrives, the store replaces it with the ack's own `t` — the same clock inbound messages already use — so a reply that lands quickly no longer sorts above the message it's answering:
...
- **Reorders the thread and the chat list.** Because the row's timestamp changes, its position in [`messages()`](#querying) can move, and the chat list's `last_message_at` moves with it — unless a deleted message currently holds a newer activity time, in which case that time is kept and only `last_message_preview`/`last_message_kind` are refreshed to the newest surviving row.
- **A nack never reconciles the timestamp.** `ack.error` being set means the send failed; the row keeps its optimistic local timestamp (status becomes `Error` instead).
