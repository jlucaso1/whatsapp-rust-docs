...
Unpacking a run of packed values dispatches through a 256-entry byte-pair table — one lookup and a 2-byte store per input byte, rather than the two-shift/two-lookup scalar walk this snippet shows. This is an implementation detail of `read_packed`; the nibble/hex value mapping above still describes what each byte decodes to.
...
- **`NodeRef::content` dropped its `Box`** (see the note under [Zero-copy decoding](#zero-copy-decoding)). Removing the allocation cuts instructions 9.5% on a group-fanout decode (one child per device) and 5.8% on a small ack, at the cost of growing `NodeRef` from 48 to 72 bytes.
...
Both are internal to the decoder — no signature or wire-format change — but they explain why decode benchmarks on 30-byte-ack and fanout-shaped stanzas moved between recent versions.
