...
`Client::connect()` fetches `https://web.whatsapp.com/sw.js` to check for a version update, unless you've set `with_version` or the cached version is under 24h old. That request now sends `Connection: close`, so ureq drops the connection at cleanup instead of returning it to the pool. Previously this fetch left one idle TLS connection resident for the rest of the session — even for a session that never touched media — measured at roughly 88 KiB of `RssAnon` per session; now it costs nothing after the exchange completes.

`resource_report()` reflects the same change: a request carrying `Connection: close` is treated as non-pooling, so a session whose only HTTP traffic is the version fetch reports an empty pool (`pool_connections: Some(0)`, `pool_buffer_bytes: Some(0)`) instead of latching onto the 96 KiB cap described [below](#internal-implementation) after its first request. Media requests are untouched by this — the pool still exists there to make the next range request against the same CDN host cheap.

### Sharing one client across many sessions

If your process runs many WhatsApp sessions, build **one** `UreqHttpClient` and hand a clone to each `BotBuilder::with_http_client` instead of constructing one per session. `UreqHttpClient` is `Clone`. Cloning it shares the underlying `ureq::Agent` — and therefore its connection pool — rather than copying it.

This is worth doing for a process with pooled HTTP traffic — media, in practice. Media requests keep a CDN connection idle between downloads the same way the version fetch used to before it started sending `Connection: close` (see above), so sharing the agent pays that idle-connection cost once for the process instead of once per session.

You can share the connection safely. Media requests carry their own auth per request and the client sends no cookies, so a shared connection doesn't leak anything between sessions beyond what the shared source IP already reveals — with one exception: TLS session resumption lets a server correlate two sessions even across a source-IP change, which is why sharing stays opt-in rather than the default. Sharing doesn't limit concurrency either: ureq opens a new connection whenever no idle one matches the target host, so the pool only caps how many idle connections it retains, not how many requests can run at once.

```rust
use whatsapp_rust_ureq_http_client::UreqHttpClient;

let shared_http_client = UreqHttpClient::new();
let mut handles = Vec::new();

for backend in session_backends {
    let bot = Bot::builder()
        .with_backend(backend)
        .with_http_client(shared_http_client.clone())
        // ...
        .build()
        .await?;
