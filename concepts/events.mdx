...
Additional metadata for message threading and targeting.
...
With `Arc<Event>` dispatch, each event is wrapped in a single `Arc` by the `CoreEventBus` and shared across all handlers. This eliminates deep clones of large event payloads like `HistorySync` conversations and `Message(Box<wa::Message>, Arc<MessageInfo>)`. The `MessageInfo` inside `Event::Message` and `UndecryptableMessage` is also wrapped in `Arc`, enabling zero-cost sharing across the message dispatch, retry receipt, and PDO recovery paths without cloning the full struct.

Combined with `LazyConversation`'s `OnceLock`, all handlers sharing the same `Arc<Event>` get parse-once semantics for free — the first handler to call `lazy_conv.get()` triggers the parse, and subsequent handlers reuse the cached result.
