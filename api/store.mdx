...
pub struct DeviceListRecord {
    pub user: Arc<str>,             // User part of JID (shared with the registry cache's key)
    pub devices: Box<[DeviceInfo]>, // Known devices — mutate via edit_devices()
    pub timestamp: i64,             // Last update timestamp
    pub phash: Option<Box<str>>,    // Participant hash from usync
...
pub struct DeviceInfo { /* device_id: u16, flags: u8, key_index: u32 — private */ }
impl DeviceInfo {
    ...
    pub const fn is_hosted(&self) -> bool  // Hosted PN/LID address space (see below)
}
```
...
<Note>
**Breaking change ([#1353](https://github.com/oxidezap/whatsapp-rust/pull/1353)):** `DeviceInfo`'s three fields — `device_id`, `key_index`, `is_hosted` — are now private, packed into 8 bytes instead of 16, and `device_id` is a `u16` (matching `Jid::device`, which has always been one) rather than a `u32`. Read them through the `device_id()`, `key_index()`, and `is_hosted()` accessors instead of field access; construct with `DeviceInfo::new(device_id, key_index).with_hosting(is_hosted)` as before — struct-literal construction was already unavailable, since `is_hosted` had earlier made the fields non-exhaustive. `DeviceListRecord.user` is now `Arc<str>` (shared with the registry cache's key instead of a second allocation), `devices` is `Box<[DeviceInfo]>` — mutate it through the new `edit_devices(|devices: &mut Vec<DeviceInfo>| ...)` rather than in place — and `phash` is `Option<Box<str>>`. Persisted JSON is byte-for-byte unchanged in both directions; a device-list blob claiming a device id past `u16` now fails to deserialize instead of being silently truncated into a different device. See [USync](/api/usync#hosted-addressing) for how `is_hosted()` is used with `Jid::with_device_hosting`.
</Note>
