...
- **TLS Verification Skip** - Optional feature for testing with mock servers

### Cargo features

| Feature | Description |
|---------|-------------|
| `default` | Standard TLS verification enabled |
| `danger-skip-tls-verify` | Disables TLS certificate verification. **Only use for testing with mock servers.** |

<Warning>
The `danger-skip-tls-verify` feature disables TLS certificate verification, making connections vulnerable to man-in-the-middle attacks. Never enable this in production.
</Warning>

To enable TLS verification skip for testing:

```toml
[dependencies]
whatsapp-rust-ureq-http-client = { version = "0.1", features = ["danger-skip-tls-verify"] }
```
...
The client internally creates and caches a `ureq::Agent` for connection reuse. When the `danger-skip-tls-verify` feature is enabled, the agent is configured to skip TLS certificate verification.
...
The `UreqHttpClient` stores a pre-configured `ureq::Agent` for connection reuse:
...
The `execute` method wraps ureq calls in `spawn_blocking` and uses the cached agent:
