...
Event::PairingCode {
    ...
    timeout: Duration,      // ~180 seconds, remaining at time of emission
}
```
...
Event::PairingCodeRefresh {
    force_manual: bool,     // true when the server requires an explicit re-request rather than an auto-rotation
}
```
...
<Note>
`ServerAck` is the first payload sealed under the stability policy above: it's `#[non_exhaustive]` and constructed via a generated `bon` builder (`ServerAck::builder().id(...).maybe_class(...)…build()`). This only affects code that *constructs* a `ServerAck` (the client itself); reading fields off an event you received, as in the example below, is unaffected.
</Note>
