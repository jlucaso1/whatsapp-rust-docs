...
impl ProtocolStore for MyCustomStore {
    ...
    // (chat, sender) pair, so a bare id key would collide across chats.
    ...
    async fn store_pending_inbound(
        ...
        self.connection.execute(
            "INSERT OR REPLACE INTO pending_inbound_messages \
             (chat, sender, id, message, inserted_at) \
             VALUES (?, ?, ?, ?, strftime('%s', 'now'))",
            &[chat, sender, id, message],
        ).await?;
        Ok(())
    }
    ...
    async fn delete_expired_pending_inbound(
        ...
        let count = self.connection.execute(
            "DELETE FROM pending_inbound_messages WHERE inserted_at < ?",
            &[&cutoff_timestamp],
        ).await?;
        Ok(count as u32)
    }
...
-- For the retention sweep (delete_expired_pending_inbound).
CREATE INDEX idx_pending_inbound_inserted
    ON pending_inbound_messages (inserted_at);
```
