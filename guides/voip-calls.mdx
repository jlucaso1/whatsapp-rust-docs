...
On receive, the WARP tag is verified in constant time *before* the receiver's SRTP rollover counter (ROC) is advanced. This ordering matters: the ROC is derived per-packet via RFC 3711's guess-index, and folding an unauthenticated packet into it would let an on-path relay desync the receiver's keystream with a couple of forged packets. A packet that fails authentication is rejected outright and never touches the ROC state, so it can't cause a persistent decode failure for subsequent legitimate frames.
