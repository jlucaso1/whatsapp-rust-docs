...
- `connected` - Whether the client was already connected (or went on to dispatch [`Event::Connected`](#connected)) when this event fired, rather than an indicator of which bucket is non-empty. As of PR #1291, during the initial bootstrap it's `true` for every outcome that reaches an answer — synced-with-gaps, fatal, retryable, or skipped, including a batch that failed transport-side before producing any per-collection buckets. It's `false` only when the connection itself couldn't be announced: the generation was retired, the client was paused or asked to disconnect mid-teardown, or the server rejected the session (429/503) between the sync finishing and the announcement — in every one of those cases the leftover collections still go to the background sync. Outside the bootstrap, background syncs always run on an already-connected client, so `connected` is `true` there regardless of which buckets are populated.
...
As of PR #1291, a `retryable` or `skipped` outcome during the bootstrap is no longer silent either: the client dispatches `Connected` and this event (`connected: true`) the same way it does for `fatal`, then hands the leftovers to the background sync that follows. That sync retries them itself — up to `APP_STATE_RETRY_MAX_ROUNDS` (8) rounds backing off from 1 second to 128 seconds, roughly four minutes — before giving up and emitting one more `AppStateSyncFailed` for whatever is still unsynced. A handler that only reacted to `fatal` before should now also watch `retryable`/`skipped` if it needs to know the account is running in a degraded state, since both can now arrive alongside a `Connected` that announced a session missing its push name or blocklist.
