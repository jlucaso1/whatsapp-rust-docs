...
They're deliberately distinct. `AudioSilent` is a codec problem: packets are arriving but nothing is becoming sound. `AudioReceptionStalled` is a transport problem: packets have stopped arriving at all. Conflating the two is exactly how issue #1105 stayed open for months before this PR. Neither is terminal — both are diagnostic, so treat them as something to log or alert on, not to react to inside the call.
