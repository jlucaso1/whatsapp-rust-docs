...
|---------|-------------|--------------------|
| `simd` | SIMD optimizations for binary protocol | ✅ Yes |
...
The `simd` feature enables SIMD (Single Instruction, Multiple Data) optimizations for the binary protocol encoder/decoder. This provides significant performance improvements on supported platforms.

To disable SIMD optimizations (for example, on platforms without SIMD support):

```toml Cargo.toml
[dependencies]
whatsapp-rust = { version = "0.3", default-features = false, features = ["sqlite-storage", "tokio-transport", "ureq-client", "tokio-native"] }
```
