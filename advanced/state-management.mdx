...
pub enum DeviceCommand {
    ...
    SetNextPreKeyId(u32),  // Monotonic counter for prekey ID generation
...
pub struct Client {
    ...
    /// Keys are Signal protocol address strings (e.g., "5511999887766@c.us.0").
    /// Generated by `JidExt::to_protocol_address_string()`, which maps
    /// `s.whatsapp.net` → `c.us` and appends a `.0` device-id suffix.
...
impl SqliteStore {
    ...
    pub async fn new_for_device(db_path: &str, device_id: i32) -> Result<Self>
}
```
