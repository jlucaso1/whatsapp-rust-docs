...
Access profile operations for managing your own account.

**Methods:**
- `set_push_name(name: &str)` - Set display name
- `set_status_text(text: &str)` - Set about/status text
...
client.profile().set_status_text("Powered by Rust").await?;
```

<Note>
Requires `Arc<Client>` since it needs to persist state changes.
</Note>
