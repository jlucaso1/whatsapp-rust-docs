...
<Note>
  [PR #1355](https://github.com/oxidezap/whatsapp-rust/pull/1355) added `IncomingCall::video_orientation` (see [Events](/concepts/events#incomingcall)) for the rotation a video-from-start peer announces on its offer, before any `VideoFrame` exists to carry one. Read it off the `Event::IncomingCall` that preceded this call rather than waiting for the first frame, so a call that starts with the camera already rotated doesn't render upright for a beat.
</Note>
