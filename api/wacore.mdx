...
The repository includes a GitHub Actions workflow (`.github/workflows/benchmark.yml`) that runs benchmarks on every push to `main` and on pull requests. It uses [Bencher](https://bencher.dev/) to track performance over time with a 5% regression threshold on instruction counts.
