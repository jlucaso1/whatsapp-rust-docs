...
<Note>
  `buffa` (the crate `waproto` uses to generate protobuf types — see [waproto](/api/waproto)) is **not** re-exported. Add it directly if you construct any message with a sub-message field (i.e. anything beyond a plain-text `conversation`), since that requires naming `buffa::MessageField` from your own crate:
