...
5. **Wraps each conversation** in a [`LazyConversation`](/concepts/events#lazyconversation) — protobuf parsing is deferred until you call `.get()`, and embedded messages are cleared on first parse to reclaim memory. Use `.raw_bytes()` or `.get_with_messages()` if you need access to the full message history
