...
**Emitted:** When a user or group changes their profile picture
...
pub struct PictureUpdate {
    ...
    pub removed: bool,
    /// The server-assigned picture ID. None for deletions.
...
Event::PictureUpdate(update) => {
    if update.removed {
        println!("{} removed their profile picture", update.jid);
    } else {
        println!("{} updated their profile picture (id: {:?})", 
            update.jid, update.picture_id);
    }
    
    // For group pictures, author shows who made the change
    if let Some(author) = &update.author {
        println!("Changed by: {}", author);
    }
}
```
...
**Emitted:** When a user changes their status/about text
...
Event::UserAboutUpdate(update) => {
    println!("{} updated their status: {}", update.jid, update.status);
}
```
...
pub struct DeviceListUpdate {
    ...
    /// Key index info (for add/remove operations)
...
Event::DeviceListUpdate(update) => {
    match update.update_type {
        DeviceListUpdateType::Add => {
            println!("{} added a new device", update.user);
            for device in &update.devices {
                println!("  Device ID: {}", device.device_id);
            }
        }
        DeviceListUpdateType::Remove => {
            println!("{} removed a device", update.user);
        }
        DeviceListUpdateType::Update => {
            println!("{} device list updated", update.user);
        }
    }
}
```
...
**Emitted:** When a business account status changes (verified name, profile, or conversion to personal account)
...
pub enum BusinessUpdateType {
    ...
    RemovedAsBusiness,
    /// Business verified name changed
    VerifiedNameChanged,
    /// Business profile was updated
    ProfileUpdated,
    /// Business products were updated
    ProductsUpdated,
    /// Business collections were updated
    CollectionsUpdated,
    /// Business subscriptions were updated
...
Event::BusinessStatusUpdate(update) => {
    match update.update_type {
        ...
        BusinessUpdateType::VerifiedNameChanged => {
            if let Some(name) = &update.verified_name {
                println!("{} verified business name: {}", update.jid, name);
            }
        }
        BusinessUpdateType::ProfileUpdated => {
            println!("{} business profile updated", update.jid);
        }
        _ => {}
    }
}
