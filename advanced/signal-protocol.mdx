...
As of [#1124](https://github.com/oxidezap/whatsapp-rust/pull/1124), the same "sync hook first, boxed async fallback second" pattern above extends to three more per-message checks: `IdentityKeyStore::try_is_trusted_identity`, `IdentityKeyStore::try_save_identity`, and `SessionStore::try_has_session`. `#[async_trait]` boxes every store method into a `Pin<Box<dyn Future>>` regardless of whether the answer is already known; `is_trusted_identity`'s async body is an unconditional `Ok(true)` (WA Web `isTrustedIdentity` parity — identity *changes* surface through `save_identity`, not through trust checks), so that box was pure overhead paid once per encrypt and once per decrypt.
...
- `try_is_trusted_identity` is the one hook that can always answer, because the trust decision it shortcuts is already unconditional.
