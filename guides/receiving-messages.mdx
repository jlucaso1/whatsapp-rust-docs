...
`stanza` must be a `<message>` node with `id` and `from` attrs — the client parses it once via the canonical message-info parser rather than reusing whatever metadata the caller already extracted. `RetryRequestOptions` is a small builder:
