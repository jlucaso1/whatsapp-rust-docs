...
| `voip_benchmark` | `wacore/benches/voip_benchmark.rs` | VoIP media-path hot loops (requires the `voip-mlow` feature). Under the `bench-internals` feature, adds a `codec_stages` group — eleven rows attributing MLow encoder CPU to a stage (`analyze_frame`, `entropy_encode`, `lpc_front_end`, `fft512_forward`, `fft512_forward_split`, `fft576_roundtrip`, `fft576_roundtrip_split`, `perc_model_frame`, `pitch_search`, `lsf_quantize`, `celp_subframes_frame`) via the `wacore::voip::mlow::stage_bench` harness, so a codec-internal change shows *which* stage moved instead of only moving the single `mlow_encode` row. The `_split` rows measure the opt-in `mlow-fast-fft` half-length FFT path (see [VoIP Calls](/guides/voip-calls#mlow-codec)) next to the reference implementation, which stays compiled in either configuration |
...
- **Inline `<enc>` node storage in message classification** — `classify_incoming_message` collects a received stanza's `<enc>` nodes into a `SmallVec<[&NodeRef; 4]>` instead of a heap-allocated `Vec`. A fan-out addressed to us carries at most one `<enc>` per copy we can read, so the four inline slots cover every shape that reaches this path — 0 allocations instead of 1 per received stanza ([#1326](https://github.com/oxidezap/whatsapp-rust/pull/1326))
