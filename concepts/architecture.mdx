...
pub trait IqSpec {
    ...
    fn encode_iq_direct(&self, _request_id: &str, _out: &mut Vec<u8>)
...
The `encode_iq_direct` method is a performance optimization that allows hot-path IQ specs (like prekey upload) to encode directly into a byte buffer, bypassing the `Node` intermediate representation and marshal step. Most specs use the default implementation (returns `false`), which falls back to `build_iq()` + marshal.
