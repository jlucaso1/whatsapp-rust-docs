...
With `Arc<Event>` dispatch, each event is wrapped in a single `Arc` by the `CoreEventBus` and shared across all handlers. This eliminates deep clones of large event payloads like `HistorySync` conversations and `Message(Box<wa::Message>, MessageInfo)`.
