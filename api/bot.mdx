...
Sets the HTTP client for media operations and version fetching (required). The client is wrapped in an `Arc` internally, so each call gives this bot its own instance — use [`with_http_client_arc`](#with_http_client_arc) to give several bots one client instead.
...
[`with_http_client`](#with_http_client) for a client that's already behind an `Arc<dyn HttpClient>`. Reach for this instead of `with_http_client(shared.clone())` when a process runs several bots and you want them all pointing at one client — and one connection pool — rather than a clone apiece per builder. It's also the only route in for a client that isn't `Clone`, or one already type-erased to `Arc<dyn HttpClient>` because the host picks it at runtime.
