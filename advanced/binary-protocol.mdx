...
`backing_bytes()` goes the other direction: it hands back the entire decoded buffer as a `Bytes` — a refcount bump into the same allocation `slice_bytes()` already views into, not a copy. It's for a consumer that needs to forward the exact stanza onward (to another process, a recording, a replay harness) rather than read it. Re-encoding via `marshal_ref` is the alternative, and a worse one here: it costs a second pass over the tree, and it's only byte-faithful while the token dictionaries match the ones that decoded the node — `backing_bytes()` has no such dependency, since it's what arrived.
