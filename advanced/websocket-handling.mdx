...
The `NoiseSocket` provides encrypted send/receive operations after handshake. It uses zero-copy `BytesMut` buffers for frame construction and runtime-agnostic spawning via the `Runtime` trait.
...
Location: `src/socket/noise_socket.rs:22-35`

Key differences from a naive mutex-based approach:
- **`async_channel`** instead of `tokio::sync::mpsc` — runtime-agnostic, works on any async executor
- **`AbortHandle`** instead of `JoinHandle<()>` — the sender task is automatically aborted on drop (no manual `Drop` impl needed)
- **`bytes::Bytes`** for plaintext — callers pass an immutable, reference-counted buffer instead of transferring ownership of a `Vec<u8>`
- **Encryption and framing buffers** (`enc_buf`, `out_buf`) are owned by the sender task, not by callers — this eliminates per-send allocations
...
4. **Buffer reuse**: The task owns `enc_buf` and `out_buf`, reusing them across sends without per-call allocation

Location: `src/socket/noise_socket.rs:37-69`

#### Sender Task Implementation

The sender task owns the encryption and output buffers, reusing them across frames. The `BytesMut` output buffer uses `split().freeze()` to produce a zero-copy `Bytes` for the transport while retaining the allocation for the next frame:
...
Location: `src/socket/noise_socket.rs:74-100`

### Encryption

#### Small Messages (≤16KB)

Encrypted inline to avoid thread pool overhead. The `enc_buf` is reused across sends:
...
if plaintext.len() <= INLINE_ENCRYPT_THRESHOLD {
    ...
    wacore::framing::encode_frame_into(enc_buf, None, out_buf)?;
}
```

Location: `src/socket/noise_socket.rs:114-123`

#### Large Messages (>16KB)

Offloaded to a blocking thread via the runtime-agnostic `wacore::runtime::blocking` helper:
...
```rust
    ...
    wacore::framing::encode_frame_into(&ciphertext, None, out_buf)?;
}
```

Location: `src/socket/noise_socket.rs:124-143`

#### Zero-copy frame send

After framing, the output buffer is split and frozen into an immutable `Bytes` without copying. The `BytesMut` retains its allocated capacity for the next frame:
...
Location: `src/socket/noise_socket.rs:145-151`

<Note>
  The 16KB threshold (`INLINE_ENCRYPT_THRESHOLD`) is chosen based on benchmarking. Smaller messages benefit from inline encryption (no thread spawning overhead), while larger messages benefit from parallel execution.
</Note>

### Send API

The public API accepts `bytes::Bytes` — a reference-counted, immutable byte buffer. Callers don't manage encryption or framing buffers:

```rust
type SendResult = Result<(), EncryptSendError>;
...
pub async fn encrypt_and_send(&self, plaintext: bytes::Bytes) -> SendResult {
    ...
    // Wait for the sender task to process our job
...
Location: `src/socket/noise_socket.rs:158-179`

### Decryption

Decryption uses in-place mutation on `BytesMut`, avoiding allocation:
...
Location: `src/socket/noise_socket.rs:181-187`

### Cleanup

The `_sender_task_handle: AbortHandle` field automatically aborts the sender task when the `NoiseSocket` is dropped. No manual `Drop` implementation is needed — the `AbortHandle`'s own `Drop` does the work.

Location: `src/socket/noise_socket.rs:190-191`
