...
For large files, use `encrypt_media_streaming` to encrypt media with constant memory usage (~40KB) instead of buffering the entire file in memory. This is the core primitive that `encrypt_media` wraps internally.
...
<ResponseField name="EncryptedMediaInfo" type="struct">
  Encryption metadata (keys and hashes). Unlike `EncryptedMedia`, this does **not** contain the encrypted data — it was already written to the provided writer.
...
<Note>
  Memory usage is ~40KB regardless of file size (8KB read buffer + crypto state + remainder buffer). Data is read in 8KB chunks, encrypted with AES-256-CBC, and written incrementally.
</Note>
...
Use `encrypt_media_streaming` directly when you need to:
- Encrypt files too large to fit in memory
- Write encrypted output directly to disk or a network stream
- Avoid double-buffering (reading file → memory → encryption → memory)
