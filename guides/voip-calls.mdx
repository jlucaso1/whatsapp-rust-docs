...
|------------|-------------|
| `loopback [--video]` | Mic → Opus → E2E-SRTP protect/unprotect → Opus → speaker. No WhatsApp connection — hear yourself processed by the full VoIP stack. `--video` replaces this with a separate, video-only loopback: an ffmpeg source is looped straight into an ffplay window. |
...
**We initiate** — upgrade an audio-only call to video. `start_video` sends the upgrade request and returns immediately; the peer's acceptance arrives later, asynchronously, as an event — it is NOT safe to call `announce_video_enabled` right after `start_video` returns:

```rust
use whatsapp_rust::voip::{CallEvent, VideoState};

// Subscribe before starting, so this can't race a fast peer response.
let events = handle.events();
handle.start_video(camera_source, video_sink).await?;

// Elsewhere, driven by your event loop over `events`:
while let Ok(event) = events.recv().await {
    if let CallEvent::VideoStateChanged { state: VideoState::UpgradeAccept, .. } = event {
        // The peer accepted. Complete the handshake — our local video plane stays
        // gated until this send succeeds.
...
**Peer initiates** — respond to a `CallEvent::VideoStateChanged { state: VideoState::UpgradeRequestV2, .. }` event:
