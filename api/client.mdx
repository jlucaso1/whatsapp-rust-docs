...
**Methods:**
- `create(to: &Jid, name: &str, options: &[String], selectable_count: u32)` - Create a poll (returns `(SendResult, Vec<u8>)` — the send result and message secret)
...
let (send_result, secret) = client.polls().create(&chat_jid, "Agree?", &options, 1).await?;
```
