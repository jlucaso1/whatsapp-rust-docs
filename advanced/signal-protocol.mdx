...
The Signal Protocol implementation uses typed errors for all cryptographic operations:
...
pub enum SignalProtocolError {
    ...
    ApplicationCallbackError(&'static str, Box<dyn std::error::Error + Send + Sync>),
}
```
...
|-------|-------|------------|
| `SessionNotFound` | No session with sender | Request retry, wait for prekey message |
| `UntrustedIdentity` | Identity key changed | Clear old identity, retry |
| `DuplicatedMessage` | Replay attack or duplicate | Silently ignore |
| `InvalidMessage` | Corrupted ciphertext | Request retry from sender |
| `NoSenderKeyState` | Missing group sender key | Wait for SKDM |

### Helper Methods

```rust
impl SignalProtocolError {
    /// Create an ApplicationCallbackError for storage callbacks
    pub fn for_application_callback<E>(method: &'static str) -> impl FnOnce(E) -> Self
    where
...
Location: `wacore/libsignal/src/protocol/error.rs`
