...
For end-to-end numbers, run the `bench-integration` suite — it drives real scenarios (connect, send, receive, reconnect) against a mock server, so point `MOCK_SERVER_URL` at one before you start:
...
For low-level protocol numbers, run the [divan](https://github.com/nvzqz/divan) suites in `wacore` — CI tracks the same ones through [CodSpeed](https://codspeed.io). They cover the full send/receive pipeline (DM and group messaging with various participant counts), binary protocol encoding, Signal Protocol operations, and reporting token generation:

```bash
# Default workspace members only — this leaves out bench-integration,
# which needs the mock server above
cargo bench

# The voip-gated suites need their feature turned on
