...
Waits for both offline sync completion **and** history sync tasks to become idle. This is a superset of `wait_for_connected` — it ensures the client is not only authenticated but also fully synced with the server.

<ParamField path="timeout" type="Duration" required>
  Maximum time to wait for both offline sync and history sync to complete
</ParamField>

<ParamField path="Returns" type="Result<()>">
  Ok if sync completed within the timeout, Err on timeout
</ParamField>
...
// Wait up to 2 minutes for full sync
client.wait_for_startup_sync(Duration::from_secs(120)).await?;
println!("Client fully synced and ready");
```

<Note>
  Use `wait_for_connected` if you only need to wait for authentication and offline sync. Use `wait_for_startup_sync` when you need history sync to finish before proceeding (e.g., before querying chat history).
</Note>
...
The Client also exposes media upload and download methods directly:

- `upload(data: Vec<u8>, media_type: MediaType)` — Upload and encrypt media. See [Upload API](/api/upload).
- `download(downloadable: &dyn Downloadable)` — Download and decrypt media. See [Download API](/api/download).
- `download_to_file(downloadable, writer)` — Download media to a file writer.
- `download_to_writer(downloadable, writer)` — Streaming download with ~40KB memory usage.
- `download_from_params(direct_path, media_key, file_sha256, file_enc_sha256, file_length, media_type)` — Download from raw parameters.
- `download_from_params_to_writer(direct_path, media_key, file_sha256, file_enc_sha256, file_length, media_type, writer)` — Streaming download from raw parameters.
...
Access chat management actions.
...
Sets a privacy setting for a specific category.

<ParamField path="category" type="&str" required>
  Privacy category: `"last"`, `"online"`, `"profile"`, `"status"`, `"groupadd"`, or `"readreceipts"`
</ParamField>

<ParamField path="value" type="&str" required>
  Privacy value: `"all"`, `"contacts"`, `"none"`, `"contact_blacklist"`, or `"match_last_seen"`
</ParamField>
...
See [Privacy API](/api/privacy) for all categories and values.
...
| `device_cache` | `u64` | Moka cache — device info |
