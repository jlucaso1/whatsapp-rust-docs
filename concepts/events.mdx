...
<Note>
Dispatch is single-flight per message, keyed on `(chat, id, sender)` — not `(chat, id)` alone. A stanza id is chosen by the sending client and is only unique within a `(chat, sender)` pair, so two different senders reusing the same id in one chat are two distinct messages, each getting its own event, instead of the second silently folding into the first.

The sender is keyed on its wire-form JID, deliberately left unresolved to its LID/PN identity: resolving it would let the key move as that mapping is learned at runtime. The accepted cost of leaving it unresolved is the reverse case — a redelivery whose sender switches PN/LID namespace mid-flight is treated as a new message and dispatches a second placeholder. That is considered the lesser harm, since a duplicate placeholder is visible and recoverable, unlike a message going missing with nothing to say so.
</Note>
