...
| `timestamp` | Server timestamp from the ack's `t` attribute, when present. For a message ack this is the authoritative send timestamp — whatsmeow reads the same attribute into `SendResponse.Timestamp`. |
