...
<Warning>
**A completed exchange is `Ok`, whatever the status.** `execute`, `execute_streaming`, and `execute_upload` must all return `Ok(response)` for a non-2xx status too — reserve `Err` for the exchange never happening at all: DNS failure, connection refused, TLS error, timeout, or a body that broke the declared cap.

Media download and upload read `status_code` off a successful response to decide whether a failure is retryable on the same host (5xx), needs a refreshed media-auth token (401/403), or a re-derived URL (404/410) — see [Automatic retry and URL re-derivation](/api/download#automatic-retry-and-url-re-derivation). An implementation that maps 4xx/5xx to `Err` hides the status from that logic entirely, so every one of those retries repeats the same stale auth token instead of recovering.

Some HTTP crates default the other way — `ureq`'s `http_status_as_error` treats non-2xx as an `Err` unless explicitly disabled, which is why `UreqHttpClient` disables it on every request (see [Internal Implementation](#internal-implementation)). If you build a custom client on `ureq` or another library with the same default, disable it explicitly; `MockHttpClient` below shows the shape a compliant implementation should have (`Err` only when there's no response to give at all).
</Warning>
...
**Returns:**
- `HttpResponse` for any completed exchange, non-2xx `status_code` included — callers read `status_code` to classify the response
- `anyhow::Error` only when the request never completed (a transport-level failure)
...
**Returns:**
- `StreamingHttpResponse` for any completed exchange, non-2xx `status_code` included
- `anyhow::Error` on a transport-level failure, or if streaming is not supported
...
ureq 3.3 defaults to treating any 4xx/5xx as `ureq::Error::StatusCode` rather than a response — the opposite of the `HttpClient` contract above. Every request goes through `status_as_response`, which flips `http_status_as_error(false)` **per request** rather than once on the agent, so a caller-supplied agent from [`with_agent`](#with_agent) — which carries ureq's own defaults, not `UreqHttpClient`'s — still honors the contract:
...
`read_body` is where the non-2xx contract meets the `max_body_bytes` cap. A 2xx body *is* the payload, so an over-cap read there stays an `Err` — the caller must never mistake a truncated media file for a complete one. A non-2xx body is diagnostic text (an error page), so it's truncated instead, capped at 64 KiB on top of `max_body_bytes` rather than instead of it — losing the tail of an error page costs nothing, but losing the status code costs the media-conn refresh:
...
`execute_upload` follows the same shape: `status_as_response` on the request builder, `read_body` on the response. This is what lets [media auth-refresh on upload](/api/download#automatic-retry-and-url-re-derivation) work at all — before this, a rejected upload's status was swallowed into an opaque transport error and the media-conn refresh it should have triggered was unreachable.
...
Note what this example does *not* do: it never calls `.error_for_status()`. `reqwest::Response::send()` already returns `Ok` for a non-2xx status by default, matching the `HttpClient` contract for free — calling `.error_for_status()` would break it by turning every 4xx/5xx into an `Err`.
