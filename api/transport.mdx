...
Feeds into [`Client::resource_report()`](/api/client#resource_report). The bundled Tokio WebSocket transport (below) overrides this with static estimates, since `tokio-websockets` and `rustls` don't expose their live buffer sizes.
...
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
pub trait TransportFactory: MaybeSendSync {
...
while let Ok(event) = events.recv().await {
    match event {
        ...
        TransportEvent::Disconnected(reason) => { println!("Disconnected: {reason}"); break; }
    }
}
```
...
pub enum TransportEvent {
    ...
    /// The connection was lost, carrying why it closed
...
pub enum DisconnectReason {
    /// Server sent a WebSocket close frame. `code` is the RFC 6455 close
    /// code when present; `reason` is the close payload text.
    ServerClose { code: Option<u16>, reason: String },
    /// The stream ended (EOF) without a close frame.
    StreamEnded,
    /// A transport-level read error occurred.
    ReadError(String),
    /// Local shutdown — no reason available from the wire.
    Unknown,
}
```
...
// Custom Origin header (e.g. connecting through a relay that requires its own)
let factory = TokioWebSocketTransportFactory::new()
    .with_origin("https://relay.example");
```
...
<Note>
Use `default_tls_connector()` to inspect or replicate the default TLS configuration as a starting point before customizing. Note that a connector built this way does not get the single-host resumption sizing described [below](#default_tls_connector) unless you apply it yourself — that sizing is applied internally by `default_tls_connector()`, not by `Connector::Rustls`.
</Note>
...
Opens the socket with no `Origin` header at all — this crate's behavior before the header was added. Use it only for a peer that rejects the upgrade because of the header; no known WhatsApp endpoint does.
...
```toml
[dependencies]
whatsapp-rust-tokio-transport = { version = "0.7", features = ["danger-skip-tls-verify"] }
```
...
Every upgrade created by `TokioWebSocketTransportFactory` also carries an `Origin` header by default:
...
async fn read_pump<S: AsyncRead + AsyncWrite + Unpin + Send + 'static>(
    ...
    let _ = tx.send(TransportEvent::Disconnected(DisconnectReason::StreamEnded)).await;
}
```
...
impl TransportFactory for TcpTransportFactory {
    ...
    ) -> Result<(Arc<dyn Transport>, async_channel::Receiver<TransportEvent>), anyhow::Error>
    {
        ...
        tokio::task::spawn(async move {
            ...
            event_tx.send(TransportEvent::Disconnected(DisconnectReason::StreamEnded)).await.ok();
        });
