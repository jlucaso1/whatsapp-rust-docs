...
The client tracks the expected sequence number for offline stanzas and logs a warning if delivery arrives out of order. This provides observability without altering processing behavior — messages are still processed in arrival order.

### State reset on reconnect

All offline sync state (counters, sequence tracker, timing, concurrency semaphore) is fully reset on reconnect so stale state does not carry over to the next connection.
