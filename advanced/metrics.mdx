...
```toml Cargo.toml
[dependencies]
whatsapp-rust = { version = "0.7", features = ["metrics"] }
metrics = "0.24"
...
| `wa_unkeyable_device_total` | `reason` = `no_bundle`, `session_setup`, `session_lookup`, `rejected_406`, `rejected_4xx`, `rejected_5xx`, `rejected_other`, `refused_batch`, `fetch_failed`, `encrypt` | Failed attempts to obtain key material for one device, by reason. Usually the device is then dropped and the send continues to everyone else — this is the rate to watch after a session-repair change, or to page on when a chat sits on "Waiting for this message". Attempt counters, not per-delivered-device: a retry that fails the same way counts again. `406` keeps its own label since it's the only code that changes behavior; a batch-wide refusal (`refused_batch`) is kept apart from a named rejection (`rejected_*`) because it names no device. `stats()`'s [`devices_unkeyed_*` fields](/api/client#stats) carry the same totals without the per-code split ([#1304](https://github.com/oxidezap/whatsapp-rust/pull/1304)) |
