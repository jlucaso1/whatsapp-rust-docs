...
A caller that already holds the skmsg as `Bytes` — the receive path slices it straight out of the frame buffer — calls `group_decrypt_shared` directly and skips that copy: `SenderKeyMessage` parses in place and keeps a reference-counted slice of the caller's buffer as its `serialized` storage (via `SenderKeyMessage: TryFrom<Bytes>`) instead of allocating its own copy, the same technique `SignalMessage` already used.
