...
| Setting | Default | Description |
|---------|---------|-------------|
| `wanted_pre_key_count` | 812 | Number of one-time pre-keys generated and uploaded per batch. Mirrors WhatsApp Web's `UPLOAD_KEYS_COUNT`. Clamped at upload time to `5..=16_777_214`; values outside that range log a `warn!` and are clamped to the nearest bound. |

Leave this at the default unless you have a specific reason to change it — for example, a memory-constrained or embedded consumer that wants to upload smaller batches. Lower values shrink the working set during each upload, but also shorten the interval between uploads as peers consume keys, so the trade-off is per-batch memory vs. upload frequency.

<Warning>
The clamp floor of 5 exists because smaller pools either flag the bundle as uploaded while leaving it empty, or loop on re-upload (the count guard never clears). The ceiling of 16,777,214 prevents a single batch from emitting duplicate 24-bit pre-key IDs, which would cause permanent `InvalidPreKeyId` failures on the peer side.
</Warning>
...
let config = CacheConfig {
    // Upload a smaller batch on memory-constrained hosts
    wanted_pre_key_count: 256,
    ..Default::default()
};
```
