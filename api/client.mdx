...
- `unlink_subgroups(jid: &Jid, subgroups: &[Jid])` - Unlink groups from a community
...
- `query_linked_group(jid: &Jid)` - Query a linked group's community metadata
- `join_subgroup(jid: &Jid)` - Join a community subgroup
...
Access media reupload operations. Requires `Arc<Client>`. Use this when a media download fails because the URL has expired.
