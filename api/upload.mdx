...
// A file-backed UploadSource: each attempt re-reads the ciphertext from
// disk, so the full blob never lives in memory. `try_clone` gives an
// independent cursor, so seeking one reader doesn't disturb another.
struct FileSource {
    file: Arc<File>,
    len: u64,
}
