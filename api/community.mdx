...
`CreateCommunityResult` is `#[non_exhaustive]`. Field reads are unaffected; only exhaustive struct destructuring from outside the crate requires adding `..`.
...
`CommunitySubgroup` is `#[non_exhaustive]`.
...
`LinkSubgroupsResult` is `#[non_exhaustive]`.
...
`UnlinkSubgroupsResult` is `#[non_exhaustive]`.
