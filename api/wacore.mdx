...
| `voip_benchmark` | `wacore/benches/voip_benchmark.rs` | VoIP media-path hot loops (requires the `voip` feature). Under the `bench-internals` feature, adds a `codec_stages` group — nine rows attributing MLow encoder CPU to a stage (FFT, CELP, pitch search, LSF quantize, LPC front end, entropy coding) via the `wacore::voip::mlow::stage_bench` harness, so a codec-internal change shows *which* stage moved instead of only moving the single `mlow_encode` row |
...
- **Stack arrays in the MLow codec's analysis internals** — `smpl_nlsf2a` sizes its scratch buffers by the fixed LPC order (16); three of its four `Vec`s moved to stack arrays. `smpl_lsf_quant::get_maxi_k`'s `used` mask is bounded by `n ≤ 17` and moved to the stack the same way `smpl_celp::smpl_get_maxi_k` already had. `CelpEncoder::encode_subframe`'s per-rate `vec![0.0f32; 160]` scratch, previously allocated fresh inside its rate loop, is now pooled and hoisted out. None of the three touch floating-point arithmetic — pinned by the `voip::mlow` golden-checksum tests. Whole-frame `mlow_encode`: 633 → 534 allocations (−15.6%), 519.8 KB → 499.2 KB (−4.0%) ([#1320](https://github.com/oxidezap/whatsapp-rust/pull/1320))
...
- The `wacore`/`wacore-noise` shard passes `--features voip-mlow,bench-internals`. A CodSpeed shard that names no `features` builds bench targets under only their crate's default features, and both `voip_benchmark` and `sframe_varint_benchmark` declare `required-features` — a target whose required features are off is *silently skipped* by cargo rather than erroring, so without this the entire VoIP media plane (MLow encode/decode, E2E-SRTP, SFrame, and the `codec_stages` per-stage rows) built nowhere and uploaded nothing ([#1320](https://github.com/oxidezap/whatsapp-rust/pull/1320))
