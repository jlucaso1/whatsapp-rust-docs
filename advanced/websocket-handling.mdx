...
**Why an accumulation buffer instead of adopting each payload.** An earlier version had `feed_bytes(Bytes)`, an owned path meant to adopt the transport's payload allocation zero-copy via `Bytes::try_into_mut` when the decoder's buffer was empty and the payload had no other references. In practice that fast path never fired: `tokio-websockets` builds every payload with `BytesMut::split_to`, which promotes the codec's read buffer to a shared (`KIND_ARC`) allocation with refcount 2 — so `try_into_mut` always saw a sibling reference and fell back to copying anyway, while the steady-state branch also donated the decoder's whole buffer downstream via `mem::take`, forcing a fresh allocation on the very next read.

`feed_bytes` was removed rather than fixed. The accumulation-buffer design amortizes the buffer's allocation over every frame that fits in a `CHUNK_SIZE` (1 KiB) chunk instead of allocating per read, and because a buffer that has been `split_to` is already reference-counted, `Bytes::freeze()` further down the receive path (in-place AEAD decryption) becomes a pointer move rather than a boxed `Shared` allocation. `MAX_IDLE_CAPACITY` (64 KiB) bounds how much capacity a drained buffer is allowed to keep, and `MAX_EAGER_RESERVE` (64 KiB) caps how far the decoder will grow ahead of an announced-but-not-yet-received frame, since the 3-byte length prefix is the peer's claim, not evidence — reserving the whole declared size on the first three bytes would let a hostile peer pin an arbitrary allocation with almost no data sent.
