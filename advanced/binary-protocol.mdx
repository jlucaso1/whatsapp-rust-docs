...
Both single-byte and double-byte tokens are resolved by a single compile-time [hashify](https://crates.io/crates/hashify) `tiny_map` (length-bucketed dispatch: `match key.len()` first, then compare discriminator bytes — no full-key hash), generated by a build script from `tokens.json`. A single call to `index_of_token` resolves any known protocol string:
...
The unified token map is generated at compile time via `hashify::tiny_map!` (length-bucketed dispatch), so the single lookup is O(1) and hash-free. Since the vast majority of node tags and attribute keys are part of the WhatsApp token dictionary, this eliminates most heap allocations during protocol decoding.
