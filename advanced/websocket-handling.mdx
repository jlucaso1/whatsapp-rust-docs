...
1. The write counter is now incremented **immediately before** `transport.send()` is awaited, not after it returns — so "a counter value is never used twice" holds regardless of whether the send later fails.
