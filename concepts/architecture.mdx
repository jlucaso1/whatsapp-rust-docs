...
After successful authentication, the client runs a series of initialization tasks (each gated by a connection generation check to abort if the connection drops):

1. Send unified session
2. Establish session with primary phone for PDO (before passive mode exit)
3. **Sync own device list** — calls the server to refresh the local device registry for both the PN and LID JIDs, bypassing cache. This ensures DM fan-out includes all companion devices (matches WhatsApp Web's `syncMyDeviceList()`)
4. Upload pre-keys
5. Exit passive mode (triggers offline message delivery)
6. Wait for offline sync to complete
7. Run post-connect tasks (fetch props, presence, dirty bits, etc.)
...
| `message_queues` | Invalidated | Drop per-chat queue senders so workers exit via channel close — prevents stale workers from the old connection surviving reconnects with outdated signal/crypto state |
...
<Note>
Message queue invalidation is critical for correctness. Without it, stale message processing workers from the previous connection survive reconnects, holding outdated Signal session state that causes decryption failures on the new connection.
</Note>
...
5. **Bounded channel streaming** — a `tokio::sync::mpsc::channel::<Bytes>(4)` streams conversation bytes from the blocking parser thread to the async event dispatcher, providing backpressure with only ~4 conversations in-flight
...
Prevents race conditions where a later message is processed before the PreKey message. Each chat gets a bounded channel with a capacity of **500** messages, providing backpressure to prevent memory amplification when many chats are active simultaneously:

```rust
pub(crate) message_queues: Cache<String, mpsc::Sender<Arc<Node>>>,
// Each queue: async_channel::bounded::<Arc<Node>>(500)
```
...
The DM send path acquires locks for **all recipient devices** (both the recipient's and sender's linked devices) in sorted order to prevent deadlocks. This matches the decrypt path's per-device locking granularity, preventing ratchet desync when concurrent sends and receives operate on the same Signal session. The `build_session_lock_keys()` helper resolves encryption JIDs, sorts, and deduplicates the keys before acquisition.

The peer message path (single-device) acquires a single lock for the resolved encryption JID. Group messages do not hold client-level session locks — each participant device is encrypted separately inside `prepare_group_stanza`.
