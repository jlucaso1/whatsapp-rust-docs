...
For cleaner message handling, use `MessageContext` to wrap the message, metadata, and client together. This provides convenience methods like `send_message` (auto-targets the source chat), `build_quote_context`, and `edit_message`:
...
| `Client/PDO` | Peer Data Operations — message recovery from primary phone and retry fallback |
