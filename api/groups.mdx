...
println!("Created group: {} ({})", result.metadata.subject, result.metadata.jid);
for participant in &result.metadata.participants {
    println!(
        "- {} ({})",
        participant.display_name.as_deref().unwrap_or("(no label)"),
        participant.jid,
    );
}
```
...
<Note>
Since v0.6, `create_group` returns the full `GroupMetadata` (matching `get_metadata`) instead of just the JID. Inspect `result.metadata` for participants, addressing mode, ephemeral timer, and parent linkage in a single round-trip.
</Note>
...
<Note>
This method uses the MEX protocol and returns `MexError` instead of `anyhow::Error`. See the [MEX API](/api/mex) for details on MEX errors.
</Note>
...
The `metadata` field carries the full group state returned by the server (same shape as [`get_metadata`](#get_metadata)), including participant `display_name` labels captured from `<participant>` children.

<Note>
Prior to v0.6 this struct only exposed a `gid: Jid`. Replace `result.gid` with `result.metadata.jid` when upgrading. The same migration applies to `CreateCommunityResult`.
</Note>
