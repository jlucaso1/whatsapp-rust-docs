...
<Note>
Breaking change (PR #1310): retired. Nothing constructs or dispatches this variant, and nothing ever will — the payload promised an `old_push_name`/`new_push_name` comparison this library cannot make, since it holds no contact store to source the previous name from. `EventKind::PushNameUpdate` is renamed `EventKind::RetiredPushNameUpdate` the same way.

Both the `EventKind` and `Event` variants keep their slot rather than being deleted: `EventKind`'s discriminant is an `EventInterest` bit index a consumer may persist, and `Event` derives `Serialize` into index-keyed formats (bincode, postcard) that key a variant by position, so removing either would renumber every variant after it and change how already-stored data decodes. The payload struct is emptied instead, so the slot is held without claiming anything.

A handler matching `Event::PushNameUpdate(..)` should drop the arm (it can never fire) or rename it to `Event::RetiredPushNameUpdate(..)`. Read the current push name from [`MessageInfo::push_name`](#messages) on the message events you already handle — every incoming message carries it — and diff against your own contact store if you need change detection.
</Note>
