...
| `entry_count_async` | `async fn entry_count_async(&self) -> u64` | Entry count under an awaited read guard, delegating to the custom backend if available. Unlike `entry_count`, the in-process path is always awaited, so it never under-reads a cache that's being written to — use this for a report rather than `entry_count` ([#1408](https://github.com/oxidezap/whatsapp-rust/pull/1408) fixed the in-process path, which previously called through to the same `try_read` as `entry_count`) |
