...
- `Reconnecting` — between connections, with something driving the client back to one. Covers a 429/503 rate-limit backoff too. The one state `wait_until_reachable()` waits out; only `reachability()` ever reports it.
