...
A stanza that came off the wire is one byte short of that: `OwnedNodeRef::backing_bytes()` returns node bytes only, since the receive path already stripped the format byte on the way in. Forward it through `wacore_binary::util::pack` first — see [Binary Protocol: the format byte](/advanced/binary-protocol#the-format-byte).
...
<Note>
As of PR #1259, the shape of `plaintext` is checked before it reaches the socket, so a malformed payload now fails locally with a typed error instead of being written to the wire and having the server close the connection.
</Note>
