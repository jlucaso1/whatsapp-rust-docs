...
<Note>
  For the AD-capable servers (`Pn`, `Lid`, `Hosted`, `HostedLid`), the wire form spells the server as a leading domain byte on `AD_JID` (see [AD_JID](#ad_jid-device-specific-jid) below), which the decoder resolves into `server` and does not also copy into `agent`. Earlier versions kept a redundant copy in `agent`, so a JID decoded off the wire (`agent` set to the domain byte) and the same JID parsed from text (`agent` always `0`) compared unequal under the derived `PartialEq`/`Hash` — despite `Display` rendering them identically. `agent` now stays `0` for these servers on both paths; it is only ever nonzero for `Bot`/`Interop`, which do render it.
</Note>
...
On decode, the reverse used to have its own asymmetry: `domain_type` resolves to `server`, but the decoder also wrote that same byte into `agent` — a field the encoder never reads back out (it always re-derives `domain_type` from `server`) and `Display` never renders for these servers. The only effect was that `agent` differed depending on provenance: `0` for a JID parsed from text, the domain byte for one just decoded off the wire. Since `agent` is part of the derived `PartialEq`/`Hash`, the identical JID could compare unequal — the same shape of bug as the encoder one above, just on the other side of the wire. The decoder now leaves `agent` at `0` for `Pn`/`Lid`/`Hosted`/`HostedLid`, so encode → decode is idempotent and a wire-decoded JID equals the same JID parsed from text.
