...
Access profile operations for the user's own account.

**Methods:**
- `set_push_name(name: &str)` - Set display name (push name)
- `set_status_text(text: &str)` - Set status text (about)
...
// Change your status/about text
client.profile().set_status_text("Hello, world!").await?;
```

<Note>
`set_push_name` sends a presence stanza and syncs across devices via app state. `set_status_text` uses an IQ-based approach matching WhatsApp Web's behavior.
</Note>
