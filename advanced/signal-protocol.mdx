...
<Note>
**Concurrent preflights for the same address now coalesce into one fetch ([#1315](https://github.com/oxidezap/whatsapp-rust/pull/1315)).** Before this, the pre-fetch existence probe in `ensure_e2e_sessions` couldn't deduplicate a burst of concurrent callers for the same protocol address (user + device): the probe answers before the IQ goes out, so every caller in the burst read the same "no session" and every one of them fetched prekeys independently. Each returned bundle was then installed over the last, so a burst could leave several mutually incompatible session states behind — none matching the ratchet key the peer was actually encrypting under — while burning one of the peer's one-time prekeys per redundant fetch.

`Client` now holds an `EnsureRegistry` that claims an address synchronously, before the first await, splitting a batch into addresses this call owns and addresses already claimed by another in-flight call. The claiming caller ("leader") fetches and installs as before; other callers wait on it instead of fetching again. Release happens on `Drop`, so a cancelled or panicking leader still wakes its waiters. A waiter whose leader failed re-probes for itself rather than reporting a session that was never established — mirroring WA Web's own `ensureE2ESessions` wid-to-promise map in `WAWeb/Manage/E2ESessionsJob.js`, except that a waiter does not inherit its leader's error directly (our errors are inspected via `ErrorChainExt`, and `anyhow::Error` isn't `Clone`).
