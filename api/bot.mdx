...
<Note>
  Opt-in and off by default — instrumenting every poll has measurable overhead, so treat this as a diagnostics tool rather than an always-on meter. See [`Client::stats()` and `Client::memory_report()`](/api/client#diagnostics) for the always-on, zero-cost-when-unused wire I/O and memory surfaces.
</Note>
