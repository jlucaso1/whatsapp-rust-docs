...
The `partition_dm_devices` function classifies all resolved devices into recipient and own groups, and excludes the exact sender device (the current device) entirely. It partitions `all_devices` in place — swapping recipient devices to the front of the caller's own `Vec` — instead of allocating two new device vectors per send:
...
struct PartitionedDmDevices {
    devices: Vec<Jid>,
    recipient_count: usize,
}

impl PartitionedDmDevices {
    fn valid_devices(&self) -> &[Jid];      // recipient + own, sender excluded
    fn recipient_devices(&self) -> &[Jid];   // recipient devices only
    fn own_other_devices(&self) -> &[Jid];   // own non-sender devices only
}
```
