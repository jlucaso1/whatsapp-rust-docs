...
All download methods automatically retry when WhatsApp's CDN returns HTTP 401 or 403. On an auth error, the client invalidates the cached media connection, fetches fresh credentials, and retries the download once. Non-auth HTTP errors (such as 500) cause the client to try the next available CDN host without refreshing credentials.
