...
<Warning>
  `shutdown_signal()` landed on `main` with [PR #960](https://github.com/oxidezap/whatsapp-rust/pull/960) and will be included in the next published release after `0.6.0`. The published `whatsapp-rust = "0.6"` dependency shown above doesn't export it yet — depend on the git source (see [Add to your project](#add-to-your-project)) to use it before then.
</Warning>
