...
Tokens can arrive keyed by phone number before a contact's LID is known — for example, a token received while the peer was still PN-only. Token lookups are keyed through `resolve_tc_token_key`, which answers with the LID as soon as a PN↔LID mapping exists for that contact and with the PN before that. So the moment a mapping is learned, a token still filed under the PN becomes unreachable — the send path then behaves as if no token had ever arrived, even though one is stored.

To prevent that, the library migrates a PN-keyed token to the LID key as part of LID discovery, alongside the device-registry and Signal-session re-keying that already happens there (see [LID-PN mappings](/concepts/storage#patched-cache-domains)):
...
The newer-wins guarantee in step 2 relies on [`store_received_tc_token`](/api/store#tctoken-storage) being atomic. The built-in `SqliteStore` and `InMemoryBackend` implement it as a single atomic upsert, so this is safe out of the box. A custom [`Store`](/api/store) backend that keeps the default read-modify-write implementation of that method is not protected: a concurrently-arriving LID-keyed token can land between the migration's read and write, and the migration's write would then clobber it.
