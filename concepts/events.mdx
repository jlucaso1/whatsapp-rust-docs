...
/// outstanding flow), and an unanswered `companion_finish`.
///
...
pub struct PairingCodeRefresh {
    ...
    /// the unanswered-`companion_finish` timeout trigger.
...
Event::PairingQrCodesExhausted(PairingQrCodesExhausted { disconnected, .. }) => {
    if disconnected {
        // No pair-code flow was outstanding — the client already tore the
        // socket down. Reconnect to get a fresh batch of refs.
        println!("QR refs exhausted, disconnected — reconnect to retry");
    } else {
        // A pair-code flow is still outstanding, so the socket was left up
        // to keep carrying it. There is simply no QR to show right now.
