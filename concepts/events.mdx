...
This is the counterpart to `OfflineSyncCompleted`, not a variant of it: the drain did not finish, the client is not caught up, and the rest of the backlog is still queued server-side. `delivered` does not mean *acked to the server*, though — offline delivery receipts are flushed once per durable [commit batch](/advanced/inbound-durability#batching) as the drain progresses, matching WA Web's `createSnapshot` → `sendAggregateOfflineReceipts` per snapshot, not only at the very end. So most of what `delivered` counts has already been confirmed and will not come back. What the next connection's `OfflineSyncPreview` actually redelivers is only the batch still open — undurable, unacked — at the instant the connection dropped. A consumer that gates "caught up" UI or startup work on `OfflineSyncCompleted` should treat this event as "not caught up yet, wait for the next preview" rather than as completion.
...
If instead the connection ends before the drain finishes, the resume is never left silent: it is reported as `OfflineSyncInterrupted` rather than `OfflineSyncCompleted` (added in PR #1380). Only the batch still open at that instant is redelivered on the next connection's `OfflineSyncPreview` — offline delivery receipts are flushed per durable commit batch throughout the drain, so earlier batches in the same drain are already acked and do not come back.
</Note>
