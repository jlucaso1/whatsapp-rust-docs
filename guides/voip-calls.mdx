        ...
        Event::IncomingCall(incoming) => {
            ...
            if let CallAction::Offer { is_video, .. } = &incoming.action {
                ...
                if *is_video {
                    ...
                    // An audio-only accept can still upgrade later via `CallHandle::accept_video`.
...
|------------|-------------|
| `loopback [--video]` | Mic → Opus → E2E-SRTP protect/unprotect → Opus → speaker. No WhatsApp connection — hear yourself processed by the full VoIP stack. With `--video`, an ffmpeg source is looped into an ffplay window instead. |
| `listen [accept] [--video]` | Connect to WhatsApp and print incoming calls. Rejects by default; pass `accept` to auto-answer. With `--video`, an accepted call answers with video media too. |
...
You can supply video up front with `.video(source, sink)` on the call builder (from-start video), or start/stop it mid-call on the `CallHandle`:

```rust
// Upgrade an audio-only call to video (we initiate). Sends the upgrade request; the peer's
// acceptance arrives as CallEvent::VideoStateChanged { state: VideoState::UpgradeAccept, .. }
// on handle.events().
handle.start_video(camera_source, video_sink).await?;

// On seeing that UpgradeAccept event, complete the handshake. Our local video plane stays
// gated until this send succeeds — accept_video (below) sends its own Enabled internally,
// but as the initiator this standalone send is on us.
handle.announce_video_enabled().await?;

// Accept a peer's upgrade request, surfaced as
// CallEvent::VideoStateChanged { state: VideoState::UpgradeRequestV2, .. }. This already
// sends both the accept and the Enabled stanza — no extra step needed on this side.
handle.accept_video(camera_source, video_sink).await?;

// Downgrade back to audio-only. Idempotent.
...
| `handle.start_video(source, sink).await` | Upgrade to video (we initiate); sends the video-upgrade offer and enables the media plane once the peer accepts |
| `handle.accept_video(source, sink).await` | Accept the peer's pending video-upgrade request |
| `handle.announce_video_enabled().await` | Initiator-only: send the standalone `Enabled` stanza once the peer's `UpgradeAccept` arrives, completing a `start_video` upgrade. Not needed by `accept_video` (sends it internally) or for video-from-start |
...
2. E2E-SRTP keys are derived with HKDF + the libsrtp AES-CM KDF, independently for the audio and video SSRCs.
