...
As of [#1324](https://github.com/oxidezap/whatsapp-rust/pull/1324), four more short-lived allocation sources move to stack-backed storage, without changing the wire format, the crypto, or any existing call site.

**Addon crypto** (reactions, poll votes, message edits, comments): `derive_use_case_secret`'s HKDF `info` string (`stanzaId || parentSender || modificationSender || <usecase>`) and `build_aad`'s AAD buffer are bounded by WhatsApp's own addressing shapes, so both move from `Vec<u8>` to `SmallVec` aliases that spill to the heap only for pathological JIDs:
...
**Stored record bridges**: `prekey_structure_to_record` and `signed_prekey_structure_to_record` (`wacore/libsignal/src/store/record_helpers.rs`) used to parse a stored structure's keys just to validate them, then rebuild the structure from the parsed keys — reallocating both key fields (and the signature, for the signed variant) on every prekey read. They now validate and adopt the caller's structure directly, via the new `PreKeyRecord::from_storage` and `GenericSignedPreKey::from_stored_structure`, which normalize the stored public key exactly as `deserialize` always did — a pre-0.7 33-byte row still heals on its next write.
...
Benchmarked with `divan::AllocProfiler`: `PreKeyBundle::new`/`clone` and `SessionRecord::serialize_into` drop to 0 allocations each (from 1), and the addon/ADV paths lose 1–8 allocations per call depending on the message type and prefix family. Wall time is flat everywhere a fixed-cost primitive — AES-GCM, HKDF, or XEdDSA verification — already dominates the call.
