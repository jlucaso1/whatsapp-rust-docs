...
When sending group or status messages, the library validates the participant hash (`phash`) returned in the server's acknowledgment against the locally computed `phash` from the outgoing stanza. A mismatch indicates that the server's view of group participants differs from the client's — meaning the local device list is stale.

**How it works:**

1. Before sending, the client reads the `phash` attribute from the prepared message stanza (if present)
...
| Send path | Sender key device cache | Group info cache |
|-----------|------------------------|-----------------|
| Group messages | Invalidated | Invalidated |
| Status messages | Invalidated | Not invalidated |

This ensures the next send to the same group re-fetches the current participant device list and group metadata from the server, preventing repeated sends to stale device lists.

```rust
// src/send.rs — simplified phash validation flow
let our_phash = stanza.attrs().optional_string("phash").map(|s| s.into_owned());

if let Some(phash) = &our_phash {
    let ack_rx = self.register_ack_waiter(&request_id).await;
    self.send_node(stanza).await?;
    self.spawn_phash_validation(ack_rx, phash.clone(), to.clone(), invalidate_group_cache);
}
```
