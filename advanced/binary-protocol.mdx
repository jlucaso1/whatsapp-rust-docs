...
The binary protocol uses typed errors for parsing and encoding failures:
...
| Variant | Cause |
|---------|-------|
| `InvalidToken(u8)` | Unknown token byte in stream |
| `InvalidNode` | Malformed node structure |
| `NonStringKey` | Attribute key was not a string |
| `AttrParse(String)` | Attribute value parsing failed |
| `MissingAttr(String)` | Required attribute missing |
| `InvalidUtf8` | Binary data not valid UTF-8 |
| `Zlib(String)` | Decompression error |
| `Jid(JidError)` | Invalid JID format |
| `UnexpectedEof` | Premature end of data |
| `EmptyData` | Expected payload was empty |
| `LeftoverData(usize)` | Extra bytes after decoding |
| `AttrList(Vec)` | Multiple attribute errors |

### Error Conversion

`BinaryError` implements `From` for common error types:
