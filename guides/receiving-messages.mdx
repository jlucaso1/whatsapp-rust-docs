...
3. A PDO request (`PlaceholderMessageResend`) is sent to your primary phone (device 0)
4. The phone responds with the full `WebMessageInfo` containing the decrypted message
5. The client dispatches the recovered message as a normal `Event::Message`
...
<Note>
PDO requests are deduplicated — if a request is already pending for a given message, subsequent requests are skipped. Pending requests expire after 30 seconds.
</Note>
