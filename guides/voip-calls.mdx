...
The encoder's two perceptual/LPC-analysis FFTs pack their real-valued (forward) or Hermitian (inverse) input into one half-length complex transform, rather than running a full-length complex transform on data that's half redundant. This roughly halves the butterflies each of those FFTs needs and cuts whole-encode CPU by about 21%, with no separate encoder-side heap arena to carry for it.

This is the encoder's only analysis-FFT path — there is no feature flag and no full-length fallback to opt into. [PR #1330](https://github.com/oxidezap/whatsapp-rust/pull/1330) removed the earlier `mlow-fast-fft` opt-in after establishing perceptual equivalence with the reference encoder it replaced (PESQ-WB/ESTOI/segSNR/LSD on 991 s of clean and noisy speech) and confirming every emitted frame keeps the same TOC byte and packet-size envelope. It also turns out to be the technique the `smpl` C reference implementation this codec was validated against already uses.
