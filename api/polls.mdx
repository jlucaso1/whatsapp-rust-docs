...
<Note>
`Polls`, `PollOptionResult`, and `PollVoteCiphertext` are re-exported from the crate root, so `whatsapp_rust::Polls` and `whatsapp_rust::PollOptionResult` work as well as the `whatsapp_rust::features::` path shown above.
</Note>
