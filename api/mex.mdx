...
`get_username` uses this to tell "no username set" (a fatal 404, either as `MexError::ExtensionError { code: 404, .. }` or as `MexError::Request(IqError::ServerError { code: 404, .. })`, matching the two shapes WhatsApp Web's own MEX client raises a 404 from) apart from every other failure, and returns `Ok(None)` for it instead of an error.
