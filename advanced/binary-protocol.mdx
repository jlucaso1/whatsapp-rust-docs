...
Reach for `parse_known` when you only need the `Option` — for example, classifying a short string that might not even be a JID, such as an email address that happens to contain an `@` — and `try_from` when the caller needs the descriptive error.
