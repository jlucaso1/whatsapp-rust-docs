...
Communities are parent groups that contain linked subgroups. You can create communities, link and unlink subgroups, and query community metadata using the `community()` API:
...
let result = community.create(options).await?;

// List subgroups
let subgroups = community.get_subgroups(&result.gid).await?;
for sg in &subgroups {
    println!("{}: {}", sg.id, sg.subject);
}
```

You can determine what type of group you're dealing with using the `group_type()` helper:
...
match group_type(&metadata) {
    ...
    GroupType::LinkedSubgroup => println!("This is a community subgroup"),
    GroupType::Default => println!("This is a regular group"),
    _ => {}
}
```

See the [Community API reference](/api/community) for the full API.
...
- [Community API reference](/api/community) - Manage WhatsApp communities
