...
[`ClientBuilder::with_ab_props_fetch(bool)`](/api/bot#with_ab_props_fetch) (default `true`) controls whether `fetch_props()` runs at all on connect. The catalog fetch is now streamed (see [`execute_streaming`](#execute_streaming)), so on a host it costs nothing worth turning off — the switch exists for a target whose whole heap can't afford even the compressed frame plus the inflate state at the moment it arrives. Turned off, no `abt` request is sent (whatsmeow never sends one either, and the server accepts both), and every flag reads as its registry default: an account the server has 1:1-LID-migrated is not recognized as such, and privacy-token and trusted-contact-token gates run on their defaults instead of the server's values.
