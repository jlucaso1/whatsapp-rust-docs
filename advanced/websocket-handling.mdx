...
Two ceilings bound a batch: `MAX_BATCH_FRAMES` (16) and `MAX_BATCH_WIRE_BYTES` (64 KiB). The byte ceiling is checked against the *next* frame's projected wire size **before** it is appended — a frame that would overflow is held over (`carry_over`) to open the next batch rather than pushing the current one past its bound. Dropping a held-over job (e.g. on shutdown) drops its response channel, which its caller observes as a closed sender: a held-over job can be lost, but it can never hang its caller.

The whole batch shares the fate of its one `transport.send()` call:

- **A crypto or framing error** is detected before any byte reaches the wire and leaves the write counter untouched. Only the offending job's caller sees the error; every frame already encrypted into the buffer still goes out (the peer's counters must stay contiguous).
- **A transport error** is ambiguous (the peer may have partially or fully received the write), so it poisons the sender and every waiter in the batch is told the send was lost. Since `EncryptSendError` wraps an `anyhow::Error` and isn't `Clone`, every waiter shares one `Arc<EncryptSendError>` (`SharedSendFailure`) rather than a re-worded copy, so the real cause reaches every caller.
...
Location: `src/socket/noise_socket.rs:339-360`
...
Location: `src/socket/noise_socket.rs:362-379`
...
The sender task coalesces whatever send jobs are already queued (never blocking to wait for more) into one encrypted buffer and issues a single `transport.send()` for the batch, capped at 16 frames or 64 KiB. See [Write Batching (Frame Coalescing)](#write-batching-frame-coalescing) under NoiseSocket for the mechanics. Measured against a 120k-message/12k-per-second pingpong harness, this cut write syscalls per message by ~8.7% and allocator calls by ~0.76%; CPU impact was not statistically significant in that workload. The gain scales with how many independent producers (replies, delivery receipts, stanza acks) happen to queue within the same scheduler tick — a lone frame is written exactly as before.
