...
Provides detailed error information with buffer recovery for retry scenarios:
...
pub struct EncryptSendError {
    ...
    pub plaintext_buf: Vec<u8>,  // Original plaintext for retry
    pub out_buf: Vec<u8>,        // Partially encrypted output
}
```
...
match client.send_node(node).await {
    ...
    Err(ClientError::EncryptSend(e)) => {
        match e.kind {
            EncryptSendErrorKind::Transport => {
                // Network issue - can retry with e.plaintext_buf
            }
            EncryptSendErrorKind::Crypto => {
                // Encryption failed - check session state
            }
            _ => {}
        }
    }
