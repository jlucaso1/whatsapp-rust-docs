...
| `handle.invite_participant(&jid).await` / `handle.ring_participant(&jid).await` | Add or re-ring a participant, including promoting a 1:1 call to a group call — see [Promoting a 1:1 call to a group call](#promoting-a-1-1-call-to-a-group-call) |
| `handle.set_approval_required(bool).await` / `handle.admit_waiting_user(&jid).await` / `handle.deny_waiting_user(&jid).await` | Call-link waiting-room admin controls — see [Waiting rooms](#waiting-rooms) |
| `handle.set_hand_raised(bool).await` / `handle.send_reaction(emoji)` / `handle.start_screen_share(id).await` / `handle.stop_screen_share().await` | See [Reactions, Hand Raising & Screen Sharing](#reactions-hand-raising-and-screen-sharing) |
...
println!("share: {}", link.url()); // https://call.whatsapp.com/video/<token>

// Preview a link without joining it (creator, waiting-room state, whether you're the admin)
let preview = client.voip().preview_call_link(&link.token, CallLinkMedia::Video).await?;

// Join it and attach media
let handle = client.voip()
    .call_link(&link.token, CallLinkMedia::Video)
    .audio(mic_source, speaker_sink)
