...
Waiting restores the ability to ask, never the request that was refused — nothing is re-sent. Issue the call again after this returns, knowing the answer can already be stale by the time that next call reaches the socket.

<Warning>
Not for use from an event handler: the event bus dispatches on the read loop, so a handler that awaits this blocks the very connection it's waiting for.
</Warning>
...
match client.contacts().get_user_info(&[jid.clone()]).await {
    Ok(info) => { /* ... */ }
    Err(_) => match client.wait_until_reachable().await {
