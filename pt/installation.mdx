...
<Note>
  Um histórico de conversas/mensagens (`whatsapp-rust-chat-store`) já existiu neste repositório como um crate opcional. Ele foi extraído para seu próprio repositório, já que materializar um fluxo de eventos em conversas, prévias e contadores de não lidas é uma decisão de aplicação, não de protocolo — ele não é mais distribuído a partir de `whatsapp-rust`.
</Note>
