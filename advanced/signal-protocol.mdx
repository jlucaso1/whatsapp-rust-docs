...
// Usage in session_cipher.rs — buffer is cleared, borrowed, then retained
let message = ENCRYPTION_BUFFER.with(|buffer| {
    ...
    aes_256_cbc_encrypt_into(ptext, message_keys.cipher_key(),
                            message_keys.iv(), buf)?;
    let ctext = buf.as_slice(); // borrow — no allocation
    let message = SignalMessage::new(..., ctext, ...)?;
    // Release oversized buffers immediately rather than waiting for the
    // periodic shrink in get_buffer (prevents pinning per worker thread).
...
// Additionally, message_encrypt releases the buffer immediately after a send
// if it grew beyond MAX_CAPACITY, rather than waiting for the periodic shrink.
// This prevents a single large plaintext from pinning ~16 KiB per worker thread.
```
