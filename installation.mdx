...
| `mlow-fast-fft` | Opt-in half-length real FFT for the MLow encoder's analysis stage — same transform, ~27% fewer encoder instructions, but not bit-identical to the reference encoder's output bitstream. Implies `voip-mlow`; off by default because the resulting bitstream hasn't been validated against a real WhatsApp decoder. See [VoIP Calls](/guides/voip-calls#mlow-codec) | ❌ No |
