...
|------|--------|-------------|
| `wa_recv_total` | `outcome` = `decrypted`, `duplicate`, `duplicate_resend`, `undecryptable`, `skmsg` | Inbound messages by decrypt outcome. `duplicate_resend` is a resent message suppressed by the [dispatch-once gate](/concepts/events#messages) ([#1352](https://github.com/oxidezap/whatsapp-rust/pull/1352)) — distinct from `duplicate`, the byte-identical redelivery the Signal ratchet itself rejects |
