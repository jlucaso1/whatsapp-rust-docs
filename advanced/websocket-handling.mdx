...
**Receive path** — a read the transport still shares with its own buffer is copied once into `FrameDecoder`'s accumulation buffer, amortized over every frame that fits in a chunk; a read the transport hands over as its own uniquely-owned `Bytes` is adopted instead:

```rust
frame_decoder.feed_owned(data); // copies only if data isn't uniquely owned
