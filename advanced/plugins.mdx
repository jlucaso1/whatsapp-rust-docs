...
<Note>
The plugin host is intentionally native-only today: it is trusted in-process Rust code, not a sandboxed or dynamically-loaded extension model. There is no dynamic (`.so`/`.wasm`) loading and no pre-ack decision hook.
</Note>
...
| `PluginCapability::StanzaInterception` | `PluginStanzaInterception` | Claiming a decoded stanza before the built-in pipeline handles it |
...
`PluginCapability::CoreEvents` lets a plugin *watch* the stream. `PluginCapability::StanzaInterception` lets it *act*: a plugin that models a stanza this client does not can claim it before the built-in pipeline runs, instead of watching the client nack it.
...
// Inside `install`, given a `PluginContext` that requested `StanzaInterception`:
let registration = context
    .stanza_interception()
    .expect("StanzaInterception capability was requested in the manifest above")
    .register(Arc::new(VendorInterceptor))?;
```

`register()` returns a `PluginInterceptorRegistration` — the same shape of RAII token `PluginCoreEvents::subscribe` returns: dropping it unregisters, `unregister()` is the explicit form, and the host indexes it weakly so terminal shutdown invalidates a token a plugin API retained without extending the life of one the plugin already released.

**Panic isolation.** A directly registered `StanzaInterceptor` (via `Client::add_stanza_interceptor`) is trusted not to panic — the read loop calls it inline, and an unwind there takes the connection with it. A plugin is not trusted with that: one faulty plugin must not kill the connection. A panicking plugin interceptor is counted in `PluginStats::stanza_interception_panics`, logged, and read as `Interception::Pass` — the stanza is left with the client, the same outcome as the plugin not being there. This also degrades the plugin's `PluginHealth`.

**Signal durability.** Interception runs *before* the built-in pipeline, so a claimed stanza is one the client did no Signal work on at all: nothing decrypted, no session mutated, no prekey consumed. There is no half-advanced state to reconcile — but the claim is final. The ack that follows tells the server not to redeliver, so a claimed `<message>` stays undecrypted forever and a claimed `<notification type="encrypt">` is a prekey top-up that never happens. A plugin claiming those takes that on whole — match narrowly.

See [`Client::add_stanza_interceptor`](/api/client#add_stanza_interceptor) for what may be claimed at all (connection-critical stanzas like `success`/`failure`/`ack`/`stream:error` and server pings are never offered) and what the server is owed once a stanza is claimed.

## Diagnostics

`Client::plugin_stats()` returns a `PluginStats` snapshot per installed plugin: lifecycle state (`PluginState`), sticky health (`PluginHealth`), callback failures, spawned-task panics, drain timeouts, active task scopes, and subscription/publisher counters — including `stanza_interceptors` (active registrations) and `stanza_interception_panics` (isolated before they could unwind through the read loop; any non-zero count degrades `PluginHealth`). `PluginEventRouter::stats()` and `PluginEvents::stats()` report queue depth and backpressure totals for custom events. `Client::memory_report()` folds in plugin resource accounting, including a `plugin_stanza_interceptors` total across every installed plugin.
...
The current host deliberately does not support dynamic plugin loading, pre-ack decisions on inbound stanzas (a stanza is claimed only after it is fully decoded — see [Stanza interception](#stanza-interception)), a foreign (non-Rust) wire protocol, or process isolation/sandboxing. These are real design commitments, not just missing features, and would only be added behind a concrete consumer need.

## See also

- [Events](/concepts/events#decryptedpayload) — `Event::DecryptedPayload`, the lower-level, always-available seam for observing a plaintext payload before it is decoded, independent of the plugin host.
