...
Edit-path counterpart of `send_message_with_options`. Accepts an `EditOptions { stanza_id: Option<String> }` to pin the outer stanza id to an existing message's id (best-effort — server/client dependent) instead of the fresh id `edit_message` generates.
