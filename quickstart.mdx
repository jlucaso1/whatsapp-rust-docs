...
<Note>
  Constructing a message with a sub-message field (like `image_message` below) requires `buffa::MessageField`. Add `buffa = "0.8"` to your `Cargo.toml` — it isn't re-exported by `whatsapp-rust`. See [Installation](/installation#add-to-your-project).
</Note>
