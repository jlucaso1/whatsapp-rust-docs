...
The default stays correct for a relay that simply forwards to WhatsApp — the origin names the endpoint being stood in for, not the host actually dialled. Only override it when the relay itself validates the header. `without_origin()` drops the header entirely, for a peer that rejects the upgrade because of it.
