...
### Deferred SKDM recipient caching

To avoid resending Sender Key Distribution Messages on every group message, the client tracks which devices have already received the sender key for each group. This cache is stored in the `skdm_recipients` table (see [Storage - ProtocolStore](/concepts/storage#protocolstore)).

The cache update is **deferred until after the server acknowledges** the message stanza. This matches WhatsApp Web's behavior where `markHasSenderKey()` is only called after the server confirms receipt.

**Why deferred?** If the cache were updated immediately after building the stanza (but before sending), a network failure between stanza build and send would leave stale entries — devices would be marked as having the sender key when they never actually received it. Subsequent messages would skip SKDM for those devices, causing decryption failures.

**Implementation:**

- **Group path:** An `SkdmUpdate` struct captures the deferred update data (`group_jid`, `devices`, `is_full_distribution`, `participants`) during stanza preparation. The update is only applied after `send_node()` succeeds.
- **Status path:** A late-init boolean tracks whether full distribution occurred. The SKDM recipient cache is only updated after the status stanza is successfully sent.
- **Error recovery:** If `prepare_group_stanza` fails with `NoSenderKeyState`, all cached SKDM recipients for that group are cleared and the send is retried with full distribution.

**Incremental targeting:**

Rather than distributing the sender key to all group devices on every message, the client:
1. Loads known recipients from persistent storage
2. Resolves all current group participant devices
3. Computes the diff — only new devices receive the SKDM
4. Merges any "forget marks" from retry handling (devices that need fresh SKDMs due to prior failures)

Location: `src/send.rs`, `src/client/sender_keys.rs`
...
The `SignalStoreCache` stores sessions as deserialized `SessionRecord` objects rather than serialized bytes, matching WhatsApp Web's architecture where the JS object IS the cache. Serialization only happens during `flush()` to the database — not on every `store_session` call.
...
This eliminates prost encode (on store) and decode (on load) from the per-message hot path. The `store_session` method takes `SessionRecord` by value, enabling zero-cost moves from the protocol layer:
...
| `flush` (batched) | write bytes to DB | serialize + write bytes to DB |
...
Sender keys use unconditional `put()` since they change with every message — the byte comparison would always fail and waste CPU cycles. This split avoids unnecessary database writes during `flush()` while not adding overhead where it provides no benefit.

### Key reuse in cache

The `key_for()` method on both `SessionStoreState` and `ByteStoreState` reuses existing `Arc<str>` keys from the HashMap via `get_key_value()`, avoiding a heap allocation on every cache operation:
