...
- **Time-to-idle (TTI)** — entries expire after a fixed duration of no access. A lookup renews the idle deadline lazily rather than on every access, so a hot key stays on the cache's read path instead of taking a write lock per read; the tradeoff is that the recorded access time can lag the real one by up to 1/16 of the TTI, so an entry may idle out up to that much *early*. It is never served past its TTI
