...
Call `state.has_baseline()` before you send an outgoing patch. It checks whether there's a real ltHash to build the patch on top of (`bootstrapped || version > 0`) — a different question from whether a bootstrap run specifically finished.
