...
Both single-byte and double-byte tokens are stored in a single compile-time [hashify](https://crates.io/crates/hashify) PTHash (perfect hash function using FNV-1a) map, generated by a build script from `tokens.json`. A single call to `index_of_token` resolves any known protocol string:
...
When converting decoded `NodeRef` values to owned `Node` values, the `intern_cow` function maps known protocol strings to their static references using the unified hashify PTHash lookup:
...
The unified token map is generated at compile time via `hashify` using PTHash with FNV-1a, so the single lookup is O(1). PTHash is a minimal perfect hash function that maps N keys to N slots with zero collisions, and FNV-1a is a fast, non-cryptographic hash well-suited for static, known-at-compile-time token sets. Since the vast majority of node tags and attribute keys are part of the WhatsApp token dictionary, this eliminates most heap allocations during protocol decoding.
