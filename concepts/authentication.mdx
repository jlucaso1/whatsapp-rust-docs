...
`pair_with_code` now enforces the same rule: it fails with [`PairCodeError::CodeAlreadyOutstanding { remaining }`](#pair-code-errors) while a previously issued code is still within its validity window, instead of silently overwriting it. Call `cancel_pair_code` first when the replacement is intentional:
...
`Client::cancel_pair_code()` abandons the outstanding flow, if any — WA Web's `initializeAltDeviceLinking()`. After this the previous code can no longer complete: a `primary_hello` for it is dropped rather than answered with a bundle its holder cannot open.
...
<Note>
  `CodeAlreadyOutstanding` and `Cancelled` are new — see [One code at a time](#one-code-at-a-time). `CodeAlreadyOutstanding` means `pair_with_code` refused to supersede a code still within its validity window; `Cancelled` means `cancel_pair_code()` withdrew the request while stage 1 (`companion_hello`) was in flight.
</Note>
