...
- `crypto` - Cryptographic primitives (HKDF, HMAC, AES-CBC, AES-GCM). Public exports include `Aes256GcmKey` (connection-lifetime pre-keyed AEAD state; holds the expanded AES cipher and GHASH subkey for the Noise transport layer), `TransportAead` (pluggable AEAD interface used by `NoiseCipher`), `transport_aead(key: &[u8; 32])` (creates a `Box<dyn TransportAead>` from the active `SignalCryptoProvider`), and `SignalCryptoProvider` (override `transport_aead` to precompute key-dependent state once per connection)
...
- `IkServerHelloOutcome` - Either `Continue(Box<IkHandshakeOutcome>)` (IK succeeds; `Continue` is boxed because `IkHandshakeOutcome` holds two `NoiseCipher` fields, making it oversized relative to the already-boxed `Fallback` variant — clippy's `large_enum_variant` lint) or `Fallback` (pivot into XXfallback)
...
- **Pre-keyed Noise transport cipher** — `NoiseCipher` holds a `Box<dyn TransportAead>` backed by a pre-keyed `Aes256GcmKey` (via `RustCryptoProvider`), computing the AES key schedule and GHASH subkey once per connection instead of on every frame. Decrypt improves ~20% on 1500-byte stanzas (AES-NI host); encrypt improves ~7%. See [changelog](/changelog/2026-06-11-noise-prekeyed-gcm).
