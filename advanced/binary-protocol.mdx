...
- **`JidRef<'a>`** — a borrowing version of `Jid` where `user` is `Cow<'a, str>` and `server` is the `Server` enum (already `Copy`). Used for zero-copy decoded JIDs in `NodeRef` attributes
...
The decoder uses `NodeRef<'a>` to avoid allocations. String and byte payloads borrow directly from the input buffer:
...
Location: `wacore/binary/src/node.rs:364-369`, `336-341`
...
`ValueRef` provides three methods: `as_str()` (returns `Cow<'a, str>` — zero-copy for `String`, allocates for `Jid`), `as_jid()` (returns `Option<&JidRef>`, only for `Jid` variant), and `to_jid()` (converts either variant to owned `Jid`, parsing from string if necessary).
