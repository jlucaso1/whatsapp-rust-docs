...
<ParamField path="enabled" type="bool" required>
  `true` to enable raw node forwarding, `false` to disable
</ParamField>

**Example:**
```rust
// Enable raw node forwarding for protocol-level debugging
client.set_raw_node_forwarding(true);
```

<Note>
When no raw node listeners are needed, keep this disabled. The check is a single atomic load per incoming stanza, but the event dispatch and cloning add overhead. See [RawNode event](/concepts/events#rawnode) for usage details.
</Note>
