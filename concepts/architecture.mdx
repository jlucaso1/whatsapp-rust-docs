...
**The batched critical-collections IQ reports a per-collection outcome** — synced, fatal (the server refused the collection outright, e.g. 400/404), retryable, or skipped (another in-flight sync or patch send already held it) — rather than a bare success/failure, and the bootstrap acts on it:

- **All synced:** the watchdog is cancelled and the client dispatches `Connected` normally.
- **Any collection fatal:** repeating the request would get the same answer, so the client stops waiting on it, connects anyway, and dispatches `Connected` followed by [`Event::AppStateSyncFailed`](/concepts/events#appstatesyncfailed) with `connected: true`. The account is usable but missing whatever that collection carries — for `critical_block` that includes push name, so presence stays unavailable until a later background retry succeeds. WhatsApp Web instead notifies the primary device and logs out on a fatal `critical_block`; this client does not end a session on its own.
