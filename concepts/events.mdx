...
**Fields:**
- `fatal` - Collections the server refused outright (HTTP 400/404 on the syncd IQ). Terminal for this connection — repeating the request gets the same answer, so the client stops retrying these until the next fresh connection.
...
- `connected` - Whether the client went on to dispatch [`Event::Connected`](#connected) despite the incomplete sync. `true` only when a `fatal` collection was involved (see the Note below); a purely `retryable`/`skipped` outcome during the initial bootstrap keeps the connection from completing and reconnects instead, so `connected` is `false` there.

<Note>
Collections are identified by their wire name (`critical_block`, `regular_high`, `regular_low`, `regular`) rather than an enum, so this payload stays stable if the set of collections changes.

`fatal` is the bucket a consumer usually has to act on. WhatsApp Web treats a fatal `critical_block` failure as grounds to notify the primary device and log out; this library does not end a session on its own. Instead, during the initial connection bootstrap, a fatal `critical_block` outcome makes the client stop waiting on it, dispatch `Event::Connected` anyway, and then dispatch this event with `connected: true` — the account is usable but missing whatever that collection carries (for `critical_block`, that includes push name, so presence stays unavailable until a later background retry succeeds). See [Critical app-state sync (pairing bootstrap)](/concepts/architecture#critical-app-state-sync-pairing-bootstrap) for the full bootstrap flow.

A background sync outside the bootstrap path (the `ib` dirty-resync handler, group `server_sync` notifications, periodic app-state resync) reports through this same event when it leaves collections unsynced, always with `connected: true` since the client was already connected before that sync ran.
</Note>
...
Event::AppStateSyncFailed(AppStateSyncFailed { fatal, retryable, connected, .. }) => {
    if !fatal.is_empty() {
        eprintln!("app-state collections permanently unsynced: {fatal:?}");
    }
