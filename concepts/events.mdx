...
<Note>
  [PR #1355](https://github.com/oxidezap/whatsapp-rust/pull/1355) added `video_orientation`. A video-from-start peer announces its camera rotation exactly once, on the offer (or your own on the accept you send back) — it sends no mid-call `<video>` of its own until the camera actually turns. Before this field existed, that initial rotation was dropped, so a call starting with the phone already sideways rendered upright until the peer's next rotation change. See [Video I/O](/guides/voip-calls#video-io) in the VoIP guide for how this compares to `VideoFrame::orientation`.
</Note>
