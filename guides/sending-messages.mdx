...
For a DM, `result.recipient_fanout` also tells you whether the send actually reached every one of the recipient's devices — a DM that lost some but not all devices still returns `Ok` with a real message id, matching WA Web. See [`RecipientFanout`](/api/send#recipientfanout) for the counts, and the [Error Types migration guide](/api/errors#from-pr-1362-a-dms-partial-fan-out-is-now-visible-and-a-primarys-406-now-fails-the-send) for why this doesn't change what a partial fan-out returns.
...
Incoming encrypted comments are decrypted transparently. The comment body is dispatched as part of an `Event::Messages` batch and the parent post key is available on `MessageInfo::comment_target`:
