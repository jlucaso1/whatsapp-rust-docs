...
As of PR #1360, `ConnectError::Version` no longer follows from every version-source failure on the `wasm32` target. That target's source, the Facebook JS SDK bundle at `connect.facebook.net`, sits on common tracker blocklists, so a client blocked from reaching it now connects anyway on the version the device already holds, reporting the fallback on [`Event::Connected`](/concepts/events#connected) via `app_version_fallback` instead of failing `connect()`. The native target's source (`sw.js`) is unchanged: a client that can't reach it still fails with `ConnectError::Version`, since that host serves WhatsApp Web itself and a client that cannot reach it has hit a real break. See [Connected](/concepts/events#connected) for the fallback payload.
