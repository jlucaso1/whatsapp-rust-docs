...
for (jid, metadata) in communities {
    println!("{}: {} ({} participants)", jid, metadata.subject, metadata.participants.len());
}
```

See [Community API reference](/api/community#get_participating) for details.
...
Incoming encrypted comments are decrypted transparently on the receive path. The decrypted body is dispatched as part of a normal `Event::Messages` batch. Because the inner `Message` proto has no slot for the parent post key, the threading link surfaces on `InboundMessage::comment_target`:
