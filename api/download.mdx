...
match client.download(downloadable).await {
    ...
    Err(e) => {
        ...
        match cause.http_status() {
            ...
            None => eprintln!("download failed with no HTTP exchange (transport/decrypt failure): {e}"),
        }
    }
}
```

`None` means the failure never reached a CDN status at all — a socket that never connected, or a body that failed decryption/hash validation — so it's the caller's own bug to handle, not an upstream refusal to pass along.
