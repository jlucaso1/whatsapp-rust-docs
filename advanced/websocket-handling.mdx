...
The client maintains connection health through a keepalive loop that sends periodic pings and monitors responses.
...
Location: `src/keepalive.rs:10-13`
        ...
        tokio::select! {
            _ = tokio::time::sleep(interval) => {
                ...
                if is_success {
                    ...
                    // Force reconnect after 180s of failures
...
Location: `src/keepalive.rs:38-84`

### Shutdown Handling

The keepalive loop uses `tokio::select!` to listen for a shutdown notification alongside the sleep interval. When the connection is cleaned up, the `shutdown_notifier` signals the loop to exit gracefully, preventing:

- Orphaned background tasks
- Resource leaks
- Unnecessary keepalive pings after disconnection

```rust
// During connection cleanup
self.shutdown_notifier.notify_waiters();  // Signal all background tasks
```
