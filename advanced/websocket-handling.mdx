            ...
            event_result = transport_events.recv().fuse() => {
                match event_result {
                    Ok(TransportEvent::DataReceived(data)) => {
                        ...
                        // Dropped before any await below: `data` is a view into the
                        // websocket's shared read buffer, so holding it while a node
                        // is processed would keep that allocation alive alongside
                        // the decoder's own copy of the same bytes.
                        frame_decoder.feed(&data);
                        drop(data);
...
Approximately every 12 keepalive ticks (~5 minutes), the keepalive loop runs two background sweeps:

- **Database retention cleanup** — expired sent messages, based on `CacheConfig::sent_message_ttl_secs`.
- **In-process cache maintenance** ([`Client::run_cache_maintenance()`](/api/client#run_cache_maintenance), added in [#1408](https://github.com/oxidezap/whatsapp-rust/pull/1408)) — sweeps expired entries out of the TTL/TTI caches (`recent_messages`, `message_retry_counts`, `session_recreate_history`, `undecryptable_dispatched`, `dispatched_messages`, `pdo_*`, `device_registry_cache`, `sender_key_device_cache`, `lid_pn_cache`, `group_cache`). These caches expire lazily — an entry leaves only on the access that finds it stale, or under capacity pressure from a new insert — so without this tick a quiet connection would retain every expired entry, up to the 20,000-entry `device_registry_cache`, for as long as it stayed connected. [`memory_report()`](/api/client#memory_report) counts include entries expired but not yet swept, so reports taken between completed sweeps can read higher than a fresh cache lookup would.

Both sweeps are spawned as separate tasks off the keepalive loop, so neither blocks ping delivery or dead-socket detection.
...
```rust
frame_decoder.feed(&data);
drop(data); // release the transport's view before any await below

// Each frame is split out of the shared accumulation buffer, so the
// downstream freeze() is a pointer move rather than a fresh allocation.
while let Some(frame) = frame_decoder.decode_frame() { /* ... */ }
```

See [Frame Decoder](#frame-decoder) for why this amortized-allocation design replaced an earlier per-payload zero-copy adoption path (`feed_bytes`) that never actually avoided a copy in practice.
