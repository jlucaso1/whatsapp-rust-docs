...
The tctoken (no cstoken fallback) is also attached automatically at three other privacy-gated call sites, matching WA Web's `USyncStatusProtocol`/`OutSpamTCTokenMixin`/`StartCall.js`:
