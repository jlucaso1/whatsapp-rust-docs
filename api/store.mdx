...
Persists the 32-byte `messageSecret` values needed to decrypt later add-ons keyed off an original message: poll votes, poll/event edits, message edits (`secret_encrypted_message`), and Meta AI / fbid bot replies (`<enc type="msmsg">`). Secrets are keyed by `(chat, sender, msg_id)` and carry an absolute expiry so they can be pruned by policy (see [messageSecret retention](/api/bot#messagesecret-retention)).
...
<Note>
Custom backends only need to implement `put_msg_secrets`, `get_msg_secret`, `get_msg_secret_with_ts`, and `delete_expired_msg_secrets`; `put_msg_secret` has a default that delegates to `put_msg_secrets` with `expires_at = 0`.
</Note>
