...
Os dados da sessão são armazenados no diretório `/data` dentro do contêiner. Monte um volume para persisti-los entre reinicializações. O contêiner é desligado graciosamente em `docker stop` — o bot se desconecta limpamente do WhatsApp antes de sair. Veja o [guia de instalação](/pt-BR/installation#implantação-com-docker) para mais detalhes.
