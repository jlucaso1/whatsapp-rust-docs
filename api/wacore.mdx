...
<Note>
  `wacore-binary` supports both **nightly** (with SIMD) and **stable** Rust. The `simd` feature is enabled by default and uses `portable_simd` for optimized encoding/decoding. Disable it for stable Rust compatibility.
</Note>
...
|---------|-------------|---------|
| `simd` | SIMD-accelerated encoding/decoding (requires nightly) | ✅ Yes |
| `serde` | Serde serialization support | ❌ No |

To build on stable Rust:
