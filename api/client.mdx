...
<Note>
Added in PR #1265: [`pause()`](#pause) does **not** end `run()` — the loop parks instead of connecting, and this call keeps running until [`resume()`](#resume) or one of the terminal calls above.
</Note>
...
client.pause().await;
assert!(!client.is_terminal()); // still between connections, not finished
...
| `pause()` called (added in PR #1265) | Connection dropped; the run loop parks and opens nothing until `resume()` — no backoff owed |
