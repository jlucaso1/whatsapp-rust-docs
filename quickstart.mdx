...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let bot = Bot::builder()
        ...
        .on_message(|ctx| async move {
            println!("Message from {}: {:?}", ctx.info.source.sender, ctx.message.text_content());
        })
        .build()
        .await?;
...
<Step title="Build and run the bot">
  ...
  `bot.run().await` runs the bot on the current task until it disconnects or logs out. To run in the background instead, use `bot.spawn()`, which returns a [`BotHandle`](/api/bot#bothandle) with `client()`, `shutdown()`, and `abort()`.
</Step>
