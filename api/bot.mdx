...
<Note>
Not to be confused with [`client.bots()`](/api/bots), which fetches WhatsApp's server-side directory of first-party AI bots. `Bot` here is the client-side framework for building a program that answers messages — a different domain that happens to share the word.
</Note>
