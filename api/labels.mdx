...
Associate a label with a single message, distinct from [`add_chat_label`](#add_chat_label) above — the association is keyed by the message as well as the chat, under the `label_message` action.
