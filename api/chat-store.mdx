...
- **Outgoing timestamps converge on the server clock.** A positive message ack replaces the optimistic local timestamp set by [`record_outgoing`](#recording-outgoing-messages) with the server's, reordering the thread and chat list as needed — see [Outgoing timestamp reconciliation](#outgoing-timestamp-reconciliation).
