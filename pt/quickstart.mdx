...
Para números ponta a ponta, execute a suíte `bench-integration` — ela roda cenários reais (conectar, enviar, receber, reconectar) contra um servidor mock, então aponte `MOCK_SERVER_URL` para um antes de começar:
...
Para números de protocolo de baixo nível, execute as suítes [divan](https://github.com/nvzqz/divan) do `wacore` — o CI acompanha as mesmas pelo [CodSpeed](https://codspeed.io). Elas cobrem o pipeline completo de envio/recebimento (mensagens DM e em grupo com várias contagens de participantes), codificação do protocolo binário, operações do Signal Protocol e geração de tokens de reporte:

```bash
# Apenas os membros padrão do workspace — isso deixa de fora o
# bench-integration, que precisa do servidor mock acima
cargo bench

# As suítes atrás da feature voip precisam dela habilitada
cargo bench -p wacore --features voip
```
