...
When the server sends a `groups_dirty` notification naming a group as stale, the library automatically invalidates the cache. On your next `query_info` call, the library re-fetches the group instead of returning the outdated snapshot. See [Stale group metadata](/concepts/events#stale-group-metadata-groups_dirty) for details.
