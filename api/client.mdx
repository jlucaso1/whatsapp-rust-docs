...
<Note>
Added in PR #1265: [`pause()`](#pause) does **not** end `run()` by itself — the loop parks instead of connecting, and this call keeps running until [`resume()`](#resume) or one of the terminal calls above. That's conditional on auto-reconnect: `pause()` does not touch `enable_auto_reconnect`, and the loop checks that flag the same way it does after every other connection end, pause-induced or not — see the `pause()`/`resume()` rows in [General reconnection behavior](#general-reconnection-behavior). With auto-reconnect disabled, the connection `pause()` tore down still ends `run()`, the same as any other disconnect.
</Note>
...
- `Paused` - added in PR #1265. [`pause()`](#pause) is in effect, so no connection is opened — including an attempt already in flight when `pause()` was called, which is retracted rather than published. Unlike `Shutdown` this is not final: [`resume()`](#resume) lifts it and the next `connect()` proceeds normally
...
Added in PR #1265. Drops the current connection and stays offline until [`resume()`](#resume) — the middle of the range between `reconnect()`, which comes back on the library's schedule, and `disconnect()`, which does not come back at all:

| | drops the connection | comes back | on whose schedule |
|---|---|---|---|
| `disconnect()` / `logout()` | yes | never — a new `Client` is required | — |
| `reconnect()` | yes | after a deliberate ~5s backoff | the library's |
| `reconnect_immediately()` | yes | immediately | the library's |
| `pause()` / `resume()` | yes | when asked | the caller's |

The supervision loop started by [`run()`](#run) stays alive and parked, so the future a caller is awaiting keeps running, and the client is **not** terminal — it is between connections, on purpose, for as long as the application wants.

**What it guarantees:**
- Once this returns, the published socket is closed and pending receipts and Signal state have been flushed on the same terms as `disconnect()`. An unpublished transport still completing its handshake closes at its next refusal check, as noted below.
- **No connection will be opened by anyone** until resumed — `connect()` is refused with [`ConnectError::Paused`](/api/errors#connecterror) for as long as it holds, including an attempt already in flight when `pause()` was called, which is retracted rather than published.
- Idempotent — pausing a paused client just tears down again.

**What it deliberately does not do:**
- Dispatches no `Event::Disconnected` — the application ended this connection, so the teardown is not news, the same reasoning `reconnect()` applies.
- Not a protocol-level presence change — the account stays registered, and other devices see nothing.
- Does not override `enable_auto_reconnect: false` — a client whose application already asked it not to come back still stops when its connection ends, pause or no pause.

<Note>
One qualification on "the socket is closed on return": an attempt still inside its handshake holds its transport out of `pause()`'s reach and closes it at its own next refusal check, which can land just after `pause()` returns. What holds unconditionally is the part a caller can act on — no such attempt ever becomes a published, readable connection.
</Note>

**Example:**
```rust
// Go offline until the application decides to come back
client.pause().await;
assert!(client.is_paused()); // still between connections, not finished
...
Added in PR #1265. Releases a [`pause()`](#pause): if [`run()`](#run) is still active — the ordinary case, since `pause()` alone does not end it — the loop reconnects at once, with no backoff owed for the offline window the application chose.

`resume()` itself is just the flag clear and notify; it does not restart a supervision loop that has already exited. If auto-reconnect was disabled, `pause()` already ended `run()` (see the [`run()`](#run) note above), and nothing is listening for this call's notify — call [`run()`](#run) (or [`connect()`](#connect)) again afterward to actually reconnect.

Returns once the loop has been told, not once it is connected — wait for that with [`wait_for_connected`](#wait_for_connected). A no-op on a client that is not paused, and on one that has since been disconnected: `resume()` clears the pause, it does not undo a shutdown.

<Note>
Safe to call while a `pause()` is still tearing down, and deliberately does not wait for one — the teardown ends in an untimed socket close, and a `resume()` that queued behind it could be held off for as long as an unresponsive transport cared to take.
</Note>
...
Added in PR #1265. Returns `true` while [`pause()`](#pause) is in effect and no connection will be opened by [`run()`](#run) until [`resume()`](#resume).
...
| `pause()` called (added in PR #1265) | Connection dropped; if auto-reconnect is enabled, the run loop parks and opens nothing until `resume()` — no backoff owed. If auto-reconnect is disabled, the loop still ends `run()`, same as any other disconnect |
| `resume()` called (added in PR #1265) | Immediate reconnect, no backoff owed for the paused window |
