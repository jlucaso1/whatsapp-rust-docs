...
<Note>
  [PR #1355](https://github.com/oxidezap/whatsapp-rust/pull/1355) added `IncomingCall::video_orientation` (see [Events](/concepts/events#incomingcall)) for the rotation a video-from-start party announces once, on whichever call-signaling stanza it sent — the peer's `Offer` if you're the callee, or their `Accept` if you're the caller — before any `VideoFrame` exists to carry one. Read it off that `Event::IncomingCall` rather than waiting for the first frame, so a call that starts with the peer's camera already rotated doesn't render upright for a beat.
</Note>
...
<Note>
  [PR #1355](https://github.com/oxidezap/whatsapp-rust/pull/1355) added `CallEvent::VideoKeyframeNeeded`. The engine can withhold non-IDR access units — on a video-from-start ungate, a group epoch commit, a source switch, or the peer's own RTCP keyframe request — but it never touches pixels, so only your encoder can produce the IDR that lifts the gate. This event tells you when that's happened; it fires once per requirement, not once per dropped frame. Without reacting to it, video stays dark until your encoder's own keyframe interval comes round on its own, which can be most of a short call.
</Note>
