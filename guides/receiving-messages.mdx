...
For a group retransmission, the client re-resolves group metadata under your requested freshness policy and preserves the group's addressing mode; broadcast-list retransmissions omit it. For a status retransmission, the client rebuilds the sender key immediately for just the requesting device — including sender-key distribution when required — without fanning out to the rest of the audience. The status retry count itself stays operation-level state, since the captured WhatsApp Web wire format doesn't encode it on the status receipt.
