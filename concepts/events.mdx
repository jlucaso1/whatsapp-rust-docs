...
**Emitted:** When the in-progress phone-number pairing code should be replaced. Covers two triggers (WA Web `Alt/DeviceLinkingApi.js` + `Link/DevicePhoneNumberCodeScreen.react.js`): the server asking for it (`refreshAltLinkingCode` / `forceManualRefresh`, only while a pair-code flow is outstanding and the server's ref matches it — a `refresh_code` notification for a stale or unrelated flow is silently ignored), and an accepted `companion_finish` whose `pair-success` then went unanswered for a minute (`PairCodeUtils::primary_hello_pair_success_timeout()`) — a primary that could not open the key bundle just goes quiet at that point, so silence is the only signal there is. A `companion_finish` the server actively *refuses* is a different case and does not fire this event — see [`PairingCodeError`](#pairingcodeerror) below.
...
/// outstanding flow), and pair-success going unanswered after an accepted
/// `companion_finish`.
///
...
<Note>
The silent-`pair-success` timeout trigger (formerly described as the "unanswered-`companion_finish`" trigger) is not new by itself. What changed is narrower: a `companion_finish` the server actively refuses used to fall silently into this same timeout, since nothing read its answer. It now surfaces immediately as `PairingCodeError` instead — see below. If your handler assumed `PairingCodeRefresh` only ever came from the server, no code changes are required either way — the reaction (call `pair_with_code` again) is the same, and the outstanding flow is already cleared by the time the event fires.
</Note>

### PairingCodeError

**Emitted:** When a phone-number pair-code flow fails, so no linking will come of it. `Client::pair_with_code` dispatches this in addition to returning `Err` — it's the *only* surface that reports the failure when pairing is driven by [`BotBuilder::with_pair_code`](/api/bot#with_pair_code), since that request runs in a detached task and its `Err` reaches no caller.
