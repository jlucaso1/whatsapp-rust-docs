...
pub struct Message {
    ...
    pub reaction_message: Option<Box<ReactionMessage>>,
    pub edited_message: Option<Box<FutureProofMessage>>,
    pub keep_in_chat_message: Option<Box<KeepInChatMessage>>,
...
<Note>
As of [PR #866](https://github.com/oxidezap/whatsapp-rust/pull/866), all inline content variant fields on `wa::Message` that were not already boxed are now `Option<Box<T>>`. This shrinks the struct from 3784 → 952 bytes (−75%). Field **reads** are transparent (auto-deref); only **construction sites** need `Box::new(...)`. See the [changelog entry](/changelog/2026-06-14-wa-message-boxing) for the full list and migration examples.
</Note>
...
let delete = wa::Message {
    protocol_message: Some(Box::new(wa::message::ProtocolMessage {
        key: Some(key),
        r#type: Some(wa::message::protocol_message::Type::Delete as i32),
        ..Default::default()
    })),
    ..Default::default()
};
