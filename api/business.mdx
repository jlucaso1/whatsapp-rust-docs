...
loop {
    ...
    for collection in &collections.collections {
        ...
        if collection.products.len() as u32 == options.item_limit {
            println!("  (may be truncated — this collection has more products)");
        }
    }
...
**Constructors:**
- `BusinessHoursConfig::new(day_of_week, mode)` — a day whose mode carries no explicit range (`Open24H`, `AppointmentOnly`, or a closed day)
...
<Warning>
`Collection::products` can be truncated, and says so only by its length: the collections query returns a prefix with no per-collection cursor, so `products.len() == item_limit` is the only signal more products exist. Reading a collection to the end would need a different, not-yet-wired-up operation.
</Warning>
