...
| `devices_unkeyed_encrypt` | `u64` | Keying attempts that had a session and still produced no ciphertext — the one reason here that points at local state (an unusable stored session) rather than the server ([#1304](https://github.com/oxidezap/whatsapp-rust/pull/1304)) |
