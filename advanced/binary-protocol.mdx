...
The encoder only takes this path when `server == Server::Interop && integrator != 0`. Any other JID — including a zero-integrator interop JID — still encodes as `JID_PAIR`.
...
This token is asymmetric between the two directions, and deliberately so. WA Web's own writer (the evidence for what the server actually accepts) emits only the three fields above; its *decoder* additionally reads a trailing server byte, because that describes what the server sends to the client, not what it expects to receive. Our decoder mirrors that same trailing read. The consequence is that encoding an interop JID this way does not round-trip through our own decoder for that token — that is a property of the protocol having two different shapes for the two directions, not a bug to paper over by making both ends agree locally.
