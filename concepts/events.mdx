...
**Emitted:** When the server sends (or withdraws) an `<ib><client_expiration t="...">` marker — the date it expects to stop accepting the client build currently running. Fires only when the deadline actually changes; a restatement of the same deadline is silent, so a handler wired to alert on this event doesn't fire on every reconnect.
...
The recorded deadline is never sooner than **three days** out, even when the server's own answer is more abrupt (`t` at or before now) — a server that says "now" still has to leave a window in which the build can be replaced. A deadline also only ever moves **closer**: a later `t` than the one already held is treated as a stale retransmit or a host that hasn't caught up, and is ignored rather than granted as an extension. Because the comparison is against the (possibly floored) stored value, a server that keeps re-sending the same abrupt `t` keeps rolling the three-day floor forward from whenever each stanza arrives, rather than pinning one fixed date — a dated deadline, once stored, settles and further repeats change nothing. See [`ServerClientExpiration`](/concepts/storage#serverclientexpiration) for the persisted record and the full decision rule.
</Note>
