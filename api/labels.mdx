...
All methods return `Result<(), AppStateError>`. Empty `label_id` (and empty `name` for `create_label`) is rejected before any network work with `AppStateError::InvalidRequest`.
