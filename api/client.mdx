...
Two on-`Client` surfaces answer "what does this session cost?" without any feature flag: always-on wire I/O counters via [`stats()`](#stats), and an on-demand memory breakdown via [`memory_report()`](#memory_report). Both are dependency-free and safe to call once per client even when running many clients in one process. For CPU/custom attribution (e.g. per-session allocator tracking), see [`BotBuilder::with_task_instrument`](/api/bot#with_task_instrument).
...
All counters are monotonic over the client's lifetime (they survive reconnects); only the two timestamps reset on connection teardown.
