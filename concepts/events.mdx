...
Event::PairingQrCodesExhausted(PairingQrCodesExhausted { disconnected, .. }) => {
    if disconnected {
        ...
        // socket down. Wait for the subsequent Event::Disconnected, then
        // reconnect to get a fresh batch of refs.
        println!("QR refs exhausted, disconnecting — reconnect once Disconnected fires");
    } else {
