...
<Note>
  `buffa` is not re-exported by `whatsapp-rust`, so naming `buffa::MessageField` yourself requires adding `buffa = "0.8"` to your own `Cargo.toml` — see [Installation](/installation#add-to-your-project). This is only needed if you construct or read protobuf sub-message fields directly; plain scalar fields (`conversation`, etc.) don't need it.
</Note>
