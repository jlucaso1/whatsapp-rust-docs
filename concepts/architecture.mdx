...
As of [#1124](https://github.com/oxidezap/whatsapp-rust/pull/1124), `spawn_detached` gives fire-and-forget callers a way to skip building that `AbortHandle` in the first place: `spawn` has to box a `dyn FnOnce` capturing the executor's own handle to construct it, and a caller that detaches on the very next line pays for a box it immediately discards. The library's own detached spawns (background init queries, the ack/receipt workers, phash-mismatch handling) all go through `spawn_detached` now. The default keeps existing `Runtime` implementations correct; override it only if your executor can spawn without any cancellation bookkeeping at all.
