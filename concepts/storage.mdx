...
- **A deadline only moves closer.** A later `t` than the one already held is treated as a stale retransmit or a host that hasn't caught up, and does not extend the deadline.
- **At least three days of notice.** Whatever the server says, the recorded `expires_at` is floored at `now + 3 days`, so a server that says "now" still leaves a window in which the build can be replaced.
...
Because the comparison is against the stored (possibly floored) value rather than the raw `t`, an abrupt deadline that keeps getting resent keeps rolling the three-day floor forward from whenever each stanza arrives; a dated deadline, once stored, settles and further repeats change nothing.
...
| `read_pool_size` | `0` | Extra connections reserved for reads only, on top of `pool_size`; `0` queues reads on the write path's permit too, unchanged from before this knob existed |
...
pub struct SqliteStoreConfig {
    ...
    ///
    /// Costs one connection's page cache (`cache_size_kib`) each, which is why
    /// it is off by default in a process holding many per-session stores.
