...
If your process runs many WhatsApp sessions, build **one** `UreqHttpClient`, wrap it in an `Arc`, and give it to every builder with [`BotBuilder::with_http_client_arc`](/api/bot#with_http_client_arc) instead of constructing one per session. `UreqHttpClient` is also `Clone` — cloning it shares the underlying `ureq::Agent` and therefore its connection pool, so `with_http_client(shared_http_client.clone())` shares just as well — but `with_http_client_arc` hands every bot the exact same instance instead of a clone per builder, and it's the only route in once the client is already type-erased to `Arc<dyn HttpClient>`.

This is worth doing for a process with pooled HTTP traffic — media, in practice. Idle sessions pay nothing (the version fetch sends `Connection: close`, so it pools no connection either way), but a session that has transferred media retains its own connection-pool buffers — on the order of tens of KiB of live heap over plain HTTP, more over TLS — for as long as its `UreqHttpClient` lives. Building one per bot pays that cost once per session; sharing collapses the whole fleet onto one pool.
...
```rust
use std::sync::Arc;
use whatsapp_rust::http::HttpClient;
use whatsapp_rust_ureq_http_client::UreqHttpClient;
