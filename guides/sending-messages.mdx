...
Incoming encrypted comments are decrypted transparently. The comment body is dispatched as part of an `Event::Messages` batch and the parent post key is available on `InboundMessage::comment_target`:
...
For incoming messages, read it from `InboundMessage` (not `MessageInfo` — the shared info stays cheap to clone since this field is known before decryption and rides the event instead):
