...
<ParamField path="extra_stanza_nodes" type="Vec<Node>" default="[]">
  ...
  <Note>
    A node whose tag collides with one this send derives from the message content — `<biz>` for a native-flow interactive message, or `<bot>` for a business DM (see [Automatic business node detection](#automatic-business-node-detection)) — is refused with `SendError::InvalidRequest` instead of being appended alongside the derived node. Sending both used to produce a stanza with two `<biz>` (or `<bot>`) children that the recipient renders as an empty message, with no error on either end. A tag this send doesn't derive, and `<meta>` in particular, is unaffected and passes through unchanged.
  </Note>
</ParamField>
...
<Note>
  On a DM (not a group), deriving a `<biz>` node also emits a `<bot biz_bot="1"/>` node immediately before it. If `extra_stanza_nodes` in `SendOptions` supplies its own `<biz>` — or, on a DM, its own `<bot>` — the send is refused with `SendError::InvalidRequest` naming the colliding tag, instead of sending a stanza with two. The two nodes can encode different intentions (e.g. a caller-supplied `native_flow_name` that disagrees with the one this button derives), and there's no way to know which one the server or recipient would honor, so neither side silently wins — see [`extra_stanza_nodes`](#sendoptions). A caller node on a tag this send doesn't derive, such as `<meta>`, is unaffected.
</Note>
