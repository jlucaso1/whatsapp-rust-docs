...
Fires for **every** failure, including local validation — a phone number that's too short never reaches the server. `rejection` is what distinguishes the two cases: `None` means the request never got an answer from the server (local validation, no connection, or a timeout); `Some(PairCodeRejection)` means the server answered with a refusal. A claim the failed request itself held is released before this fires, so `pair_with_code` can be called again immediately.
