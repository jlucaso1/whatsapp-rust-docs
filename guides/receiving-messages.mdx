...
- `chat` — the conversation the original message belongs to. Its JID class selects the route: a group JID (`@g.us`) retransmits over the group's sender-key path, `status@broadcast` retransmits over the status sender-key path, a broadcast-list JID retransmits pairwise, and any other user JID is a direct (1:1) retransmission.
- `requester` — the requesting device's JID. Retransmissions are always pairwise to this one device — group and broadcast retries never fan out to the full audience, matching the requesting participant's own retry receipt.
- `message` / `message_id` / `retry_count` — the original message content, its original ID, and the retry count carried on the requesting receipt. `message_id` and `retry_count` are preserved verbatim on the outgoing stanza.
- `with_recipient` — only accepted for a direct retransmission. It's required when `requester` is one of your own companion devices (a self-device retransmission must say which device the message is actually addressed to) and otherwise optional; when supplied, `requester` must be a local companion device or a bot. Mirrors the `recipient` attribute WA Web propagates on self-device and bot retry receipts.
- `with_group_metadata_freshness` — for group retransmissions, selects the [`Freshness`](/api/groups#freshness) policy used to load the group's participant list and addressing mode. `CachePreferred` (default) reuses a cached snapshot; `Refresh` forces a fresh fetch first.

`retransmit_message` validates every field before touching Signal ratchet state: `chat`/`requester` must be non-empty, `message_id` non-empty, `retry_count` in `1..MAX_RETRY_COUNT`, `requester` must be a user device JID (not a group/broadcast/server JID), and `recipient` is rejected outside the direct+local/bot case above. Any violation returns `SendError::InvalidRequest` before ratchet advancement or persistence.

For a group retransmission, the client re-resolves group metadata under the requested freshness policy and preserves the group's addressing mode; broadcast-list retransmissions omit it. For a status retransmission, the sender key is rebuilt immediately for just the requesting device — including sender-key distribution when required — without fanning out to the rest of the audience; the status retry count itself stays operation-level state, since the captured WhatsApp Web wire format doesn't encode it on the status receipt.
