...
| Feature | Contents |
|---------|----------|
| `voip-runtime` | Tokio driver, relay transport, DTLS/SCTP — no audio codec |
...
An application that already produces raw Opus packets (FFmpeg, libopus, a hardware encoder, …) needs only `voip-encoded` — no codec dependency is compiled in. See [Audio Codecs](#audio-codecs) below.
...
<Warning>
  Container data isn't accepted. Ogg pages from `ffmpeg -f opus` must be demuxed first, and FFmpeg RTP output must have its RTP header stripped — this core builds and protects the WhatsApp RTP packet itself. libavcodec integrations can hand each raw `AVPacket` straight through.
</Warning>

`OPUS_MLOW_16KHZ_60MS` requires CELT-only Opus. Run each outbound packet through `packetize_opus_for_mlow`; decode inbound packets with `depacketize_opus_from_mlow`. Both only rewrite the packet header — SILK/Hybrid Opus, or arbitrary Opus↔MLOW conversion, needs full decode/re-encode, which these helpers don't do.

### Negotiation

MLOW capability index 31 controls which codec the peer sends. A native-Opus answer clears that bit and adds a minimal settings overlay: `encode.use_mlow_codec_v1=false` selects the peer's decoder for the reverse direction, and `options.enable_48khz_rtp_clock` independently selects PT111/48 kHz (`false` is the PT120/16 kHz default). Both are required together for full-duplex native Opus.
...
`WA_AUDIO_PROFILE=pt111` selects the 48 kHz RTP variant instead of the PT120 default, and `WA_AUDIO_PROFILE=mlow` selects the CELT-in-MLOW escape.
