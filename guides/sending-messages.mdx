...
match client.send_message(jid.clone(), message).await {
    ...
    Err(SendError::NoRecipientDevice(e)) => {
        eprintln!("Recipient unreachable, retry with a device refresh: {}", e);
    }
    Err(e) => eprintln!("Send error: {}", e),
}
```

A DM where every recipient device failed to encrypt — or where the fan-out resolved no recipient device at all — returns `SendError::NoRecipientDevice` instead of reporting success. See [`NoRecipientDeviceError`](/api/errors#norecipientdeviceerror) for the two ways this can happen.
