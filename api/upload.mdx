...
<Note>
  Memory usage is constant regardless of file size. `update()`/`finalize()` (the `Vec` destination) buffer only the crypto state and at most 15 bytes of remainder plaintext. `update_to_writer()`/`finalize_to_writer()` also stage ciphertext internally and flush it to the writer in ~8 KiB runs rather than once per 16-byte block, to amortize the `Write` call — so writer-mode memory is bounded at crypto state + remainder + ~8 KiB, not just 15 bytes. Unlike `encrypt_media_streaming`, `MediaEncryptor` does not require a sync `Read` source, making it suitable for async contexts.
</Note>
