...
- `get_info(phones: &[&str])` - Get contact info for phone numbers
...
Access typing indicator operations.

**Methods:**
- `send(to: &Jid, state: ChatStateType)` - Send chat state
- `send_composing(to: &Jid)` - Send typing indicator
- `send_recording(to: &Jid)` - Send recording indicator
- `send_paused(to: &Jid)` - Send paused indicator
...
**Methods:**
- `send_text(text, background_argb, font, recipients, options)` - Post text status
- `send_image(upload, thumbnail, caption, recipients, options)` - Post image status
- `send_video(upload, thumbnail, duration, caption, recipients, options)` - Post video status
- `send_raw(message, recipients, options)` - Post custom status
- `revoke(message_id, recipients, options)` - Delete a status
...
**Methods:**
- `set_push_name(name)` - Update display name
- `set_status_text(text)` - Update "About" text
- `set_profile_picture(data)` - Update profile picture
