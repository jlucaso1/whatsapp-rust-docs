...
<ParamField path="message_secret" type="&[u8]" required>
  The 32-byte secret of the original message. Must be exactly 32 bytes. Stored when the original message was sent (returned in `SendResult` via `send_message_with_options`) or retrieved from your message store.
</ParamField>
