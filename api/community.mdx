...
The `metadata` field carries the full community parent metadata from the server, with the inline description already populated. See [`GroupMetadata`](/api/groups#groupmetadata) for the full field list.
