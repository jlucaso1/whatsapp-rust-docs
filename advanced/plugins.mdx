...
use whatsapp_rust::{ClientPlugin, PluginCapability, PluginContext, PluginFuture, PluginManifest};
...
Request a capability by chaining `with_capability` on the manifest, then retrieve its handle from the `PluginContext` passed into `install`. Continuing the `SearchPlugin` example above, requesting `CoreEvents` and storing its handle on `SearchApi` looks like this:
