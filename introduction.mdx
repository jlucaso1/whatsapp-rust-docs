...
- **Full media plane in Rust** - Mic/camera capture and speaker/display playout are the consumer's job. Audio encode/decode (MLow) plus encrypt/relay/decrypt of both streams are in Rust; video encode/decode (H.264) stays with the consumer — the library only owns the RTP/SRTP transport around it
