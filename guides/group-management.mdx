...
println!("Locked: {}", metadata.is_locked);
println!("Announcement: {}", metadata.is_announcement);
println!("Ephemeral: {} seconds", metadata.ephemeral_expiration);
println!("Membership approval: {}", metadata.membership_approval);
...
See [set_locked](/api/groups#set_locked)

### Announcement Mode

Set announcement mode so only admins can send messages:
...
See [set_announce](/api/groups#set_announce)

### Disappearing Messages

Set ephemeral (disappearing) messages timer:
...
// Set 90-day disappearing messages
client.groups().set_ephemeral(&group_jid, 7776000).await?;

// Disable disappearing messages
client.groups().set_ephemeral(&group_jid, 0).await?;
```

See [set_ephemeral](/api/groups#set_ephemeral)
...
See [set_membership_approval](/api/groups#set_membership_approval)

### Member Add Mode

Control who can add new members during group creation:
...
Set all options during creation:

```rust
let options = GroupCreateOptions {
    subject: GroupSubject::new("My Group")?,
    participants: vec![
        GroupParticipantOptions::new(participant1),
    ],
    member_add_mode: Some(MemberAddMode::AdminAdd),
