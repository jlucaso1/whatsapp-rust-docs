...
RTP payload type 120 carries **either** codec — it's shared by MLow and native Opus, so the payload type alone never tells you which one a packet is. Codec choice is read from the peer's negotiated capability, not sniffed from the bytes.

MLOW capability index 31 (`use_mlow_codec_v1`) drives the decision, read through `wacore::stanza::call::capability_bit`, which returns one of three states for a peer's `<capability>` blob — and two of them are handled in **opposite** directions:

| peer `<capability>` | effect |
|---|---|
| carries index 31 | MLow survives if we asked for it too |
| valid, without index 31 | MLow drops, both directions |
| absent | that participant is skipped — nothing resets |
| present but unreadable (`ver` missing/unparseable, or below the index's version) | falls back to a capability that answers false for everything — all of it resets |

Applying the decision needs no mutable `AudioFormat`: the **callee** learns the peer's capability from the `<offer>`, before the engine exists, so the call simply starts on the right codec; the **caller** learns it from `<preaccept>`/`<accept>`, riding the same channel that carries the answering device's LID, since both have to land before the first inbound packet. Mid-call, `MLOW_16KHZ_60MS` and `OPUS_16KHZ_60MS` agree on payload type, clock rate, timestamp step, sample rate, channels and samples per frame, so the swap changes no RTP header byte.

<Note>
  Group calls don't implement the mutual-AND walk yet — a group participant outside the rollout keeps the whole call on MLow. `GroupCallDevice::capability()` is public for when that gets implemented.
</Note>
...
Negotiation has two blind spots on receive: the capability can be absent from signaling (our own video `<accept>` omits it), and on the caller side it can lose the race with the first inbound packet. For those cases only, `codec_probe` corroborates without guessing — it never reads the payload alone. It checks whether the duration the peer's Opus header declares (via a structural, libopus-free RFC 6716 read that also works on wasm32/ESP32) agrees with the step its RTP timestamps actually advance by, **and** that step matches the call's negotiated cadence. The second check isn't redundant: MLow and Opus SILK read the same TOC bits as different duration sets at 10 ms/20 ms, so the two would agree by construction there even for a genuine MLow stream. Three consecutive agreements switch the decoder once and emit `CallEvent::AudioCodecSwitched { source: CodecDecisionSource::Content, .. }` — worth acting on, since it means the capability model of that peer was wrong, not just that one call got rescued.
...
They're deliberately distinct — one is a codec problem, the other transport — and conflating them is exactly how issue #1105 stayed open for months before this PR. Neither is terminal; both are diagnostic, so treat them as something to log or alert on, not to react to inside the call.
...
| `handle.media_stats()` | Snapshot of per-call audio-path counters (`CallMediaStats`) — see [Media stats and silence detection](#media-stats-and-silence-detection) |
...
<Warning>
  **Platform support depends on which crate and feature you use:**
  ...
  - `whatsapp-rust` with `features = ["voip-encoded"]` — adds a Tokio driver task built on the sans-IO `rtc-dtls`/`rtc-sctp`/`rtc-datachannel` crates (DTLS/SCTP), no audio codec linked. **Does not compile on `wasm32` or `espidf`** — a `compile_error!` enforces this at build time.
