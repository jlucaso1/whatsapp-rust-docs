...
| `passkey` | Passkey (SHORTCAKE_PASSKEY) companion-linking flow: `whatsapp_rust::passkey`, `set_passkey_authenticator`, `send_passkey_response`, `send_passkey_confirmation`. Without it, a `passkey_prologue_request` notification reaches the consumer as `Event::Notification` instead of the `Event::PairPasskey*` events. See [Authentication — Passkey linking](/concepts/authentication#passkey-linking-shortcake_passkey) | ❌ No |
...
<Warning>
  **Breaking change (unreleased):** `passkey` used to be compiled into every build; it's now opt-in and off by default. If you use passkey linking, add `features = ["passkey"]` before upgrading — see the row above.
</Warning>
