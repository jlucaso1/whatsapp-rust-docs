...
whatsapp-rust provides derive macros in `wacore-derive` for type-safe protocol node generation. These macros reduce boilerplate and ensure consistent serialization/deserialization.
...
```rust
use wacore::{ProtocolNode, StringEnum};
...
|--------|------------|-------------|
| (none) | `String` | String attribute (required) |
...
| `string_enum` | enum | Type implementing `StringEnum` |
| `u64` | `u64` / `Option<u64>` | Numeric attribute |
| `u32` | `u32` / `Option<u32>` | Numeric attribute |

**Example with all attribute types:**
...
For protocol nodes with only a tag (no attributes):

```rust
use wacore::EmptyNode;
...
This generates a unit struct that implements `ProtocolNode` with the specified tag.

### StringEnum

For enums with string representations used in protocol attributes:
...
- `TryFrom<&str>` - Parse from string
- `Default` - First variant or `#[string_default]` marked variant

**Fallback variant for unknown values:**
...
// Unknown values are captured instead of erroring
assert_eq!(PrivacyCategory::from("unknown"), PrivacyCategory::Other("unknown".to_string()));
```

When `#[string_fallback]` is present:
- Generates `From<&str>` instead of `TryFrom<&str>`
- `as_str()` returns `&str` instead of `&'static str`

Location: `wacore/derive/src/lib.rs`
...
- Derive macros: `wacore/derive/src/lib.rs`