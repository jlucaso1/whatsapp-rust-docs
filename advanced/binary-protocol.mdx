...
The encoder can use SIMD instructions for fast packing of long strings when the `simd` feature is enabled (default). On stable Rust or when SIMD is disabled, a scalar fallback is used automatically.

<Note>
  The `simd` feature requires nightly Rust for `portable_simd`. When disabled, `wacore-binary` compiles on stable Rust with equivalent scalar implementations.
</Note>
...
To build without SIMD (stable Rust compatible):

```bash
cargo build -p wacore-binary --no-default-features
```
