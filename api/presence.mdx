...
### Client-level tracking methods

These methods are available directly on the `Client` for advanced use cases:

```rust
// Track a presence subscription manually
client.track_presence_subscription(jid.clone()).await;

// Remove a JID from tracking
client.untrack_presence_subscription(&jid).await;

// Get all tracked subscriptions
let tracked: Vec<Jid> = client.tracked_presence_subscriptions().await;
```
