...
<Warning>
Make `self.send_request(&request)` below return `Ok` for a completed exchange, even a 4xx/5xx status. Reserve `Err` for when the exchange never happens at all — DNS failure, connection refused, a TLS error, or a timeout.

Media download and upload read `status_code` off the `Ok` response to retry the same host, refresh a stale media-auth token, or re-derive an expired URL. If you turn a non-2xx status into `Err` instead, you hide it from that logic, and every retry repeats the same stale token. See [HTTP Client Trait](/api/http-client#httpclient-trait) for the full contract, including how the bundled `UreqHttpClient` works around `ureq`'s opposite default.
</Warning>
