...
- `growth_locked: Option<GrowthLockInfo>` - Growth lock status (invite links temporarily disabled)
...
- `has_group_history: bool` - Whether group history sharing is enabled
...
- `participant_type: ParticipantType` - Participant role (`Regular`, `Admin`, or `SuperAdmin`)

**GroupParticipant methods:**
- `is_admin() -> bool` - Returns `true` if the participant is an admin or super admin
- `is_super_admin() -> bool` - Returns `true` if the participant is the group creator (super admin)
...
**Parameters:**
- `jid` - Group JID
- `restrict` - `true` to restrict frequently-forwarded messages, `false` to allow
...
Enable or disable group history sharing for new members.
...
**Parameters:**
- `jid` - Group JID
- `enabled` - `true` to enable history sharing, `false` to disable
...
Set who can share invite links. This uses the MEX (Meta Experience) API.
...
Set who can share message history with new members. This uses the MEX (Meta Experience) API.
...
**Example:**
```rust
use whatsapp_rust::MemberShareHistoryMode;

let group_jid: Jid = "123456789@g.us".parse()?;

client.groups().set_member_share_history_mode(&group_jid, MemberShareHistoryMode::AllMemberShare).await?;
```

### set_limit_sharing

Enable or disable limit sharing in the group. This uses the MEX (Meta Experience) API.
...
**Parameters:**
- `jid` - Group JID
- `participants` - JIDs of the users whose requests to cancel
...
**Parameters:**
- `jid` - Group JID
- `participants` - JIDs of the participants whose invitation codes to revoke
...
Query group info for multiple groups at once (max 10,000 groups per request).
...
**Returns:**
- `Vec<BatchGroupResult>` - One result per group, which can be `Full(GroupMetadata)`, `Truncated { id, size }`, `Forbidden(Jid)`, or `NotFound(Jid)`

**Example:**
```rust
let jids: Vec<Jid> = vec![
    "123456789@g.us".parse()?,
    "987654321@g.us".parse()?,
];
...
for result in results {
    match result {
        BatchGroupResult::Full(metadata) => {
            println!("Group: {} ({})", metadata.subject, metadata.id);
        }
        BatchGroupResult::Truncated { id, size } => {
            println!("Truncated: {} (size: {:?})", id, size);
        }
        BatchGroupResult::Forbidden(id) => println!("Forbidden: {}", id),
        BatchGroupResult::NotFound(id) => println!("Not found: {}", id),
    }
}
```
...
Batch fetch group profile pictures (max 1,000 groups per request).
...
- `picture_type` - `PictureType::Preview` for thumbnail or `PictureType::Image` for full-size

**Returns:**
- `Vec<GroupProfilePicture>` - Profile pictures for each group that has one

**Example:**
```rust
use whatsapp_rust::features::groups::PictureType;

let jids: Vec<Jid> = vec!["123456789@g.us".parse()?];
let pictures = client.groups().get_profile_pictures(jids, PictureType::Preview).await?;

for pic in pictures {
    println!("Group {}: {}", pic.jid, pic.url);
}
```
...
**Methods:**
- `is_admin() -> bool` - Returns `true` for both `Admin` and `SuperAdmin`

### GrowthLockInfo

Growth lock status information. When a group is growth-locked, invite links are temporarily unavailable.

```rust
pub struct GrowthLockInfo {
    pub expiration: Option<u64>,
    pub lock_type: Option<u32>,
}
```
...
pub enum BatchGroupResult {
    ...
    Truncated { id: Jid, size: Option<u32> },
    Forbidden(Jid),
    NotFound(Jid),
}
```
...
Profile picture result from a batch profile picture query.
...
Type of profile picture to fetch.
...
Most methods return `Result<T, anyhow::Error>`. Methods that use the MEX API (`set_member_link_mode`, `set_member_share_history_mode`, `set_limit_sharing`) return `Result<T, MexError>` instead. Common errors:
...
- Batch limit exceeded (10,000 for `batch_get_info`, 1,000 for `get_profile_pictures`)
