---
title: Status
description: Post, react to, and manage WhatsApp status/story updates
---
...
- `font` - Status font. `FontType` is a typed protocol enum, so a value outside it can't be passed (an older `i32` form silently dropped out-of-range values at encode time)
...
<Note>
See [Send API — send_reaction](/api/send#send_reaction) for the full parameter reference and additional examples.
</Note>
...
- `message_id` - Same override use case as `SendOptions::message_id` on 1:1/group sends (see [Send API — SendOptions](/api/send#sendoptions)): resending a failed status with the same ID, or ensuring idempotency. Leave `None` to auto-generate.
- `extra_stanza_nodes` - Neutral children appended to the outgoing `<message>` stanza. Structural children owned by the send path (e.g. `<enc>`, `<participants>`) are rejected before any cryptographic state changes; use this only for genuinely additional nodes.
- `device_freshness` - `Freshness::CachePreferred` (default) reuses the cached recipient device list; `Freshness::Refresh` forces a fresh device-list fetch before sending. See [`Freshness`](/api/groups#freshness).
