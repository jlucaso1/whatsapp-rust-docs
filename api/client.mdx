...
- `Timeout { stage, timeout }` - a step of the connect flow (`ConnectStage::VersionFetch`/`Transport`/`Socket`/`Ready`) ran out of time (20s for version fetch and transport, each independently)
