...
<Note>
As of PR #1360: on the `wasm32` target, the version source is a different one — the Facebook JS SDK bundle at `connect.facebook.net`, since `sw.js` can't be fetched cross-origin from a browser — and it sits on common tracker blocklists. A wasm client blocked from reaching it does not fail to connect; it settles for the version the device already holds and reports this on [`Event::Connected`](/concepts/events#connected) via `app_version_fallback`. `with_version` avoids the fetch (and this fallback) entirely, on either target.
</Note>
