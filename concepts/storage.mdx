...
Three BLOB columns use **protobuf (via `prost`)** for on-disk encoding:
...
- App-state sync keys: the client re-requests the missing key from the primary device, which re-shares it. The next `set_sync_key` call overwrites the row in protobuf format.
...
| `busy_timeout` | `5 s` | SQLite busy-timeout |
...
pub struct SqliteStoreConfig {
    /// Connection pool size. Drives both r2d2 `max_size` and the
    /// internal semaphore permits, so they always stay in sync.
    pub pool_size: u32,
    ...
    pub thread_pool: Option<Arc<r2d2::ScheduledThreadPool>>,
}
```
