...
- `IkServerHelloOutcome` - Either `Continue(Box<IkHandshakeOutcome>)` (IK succeeds; `Continue` boxes its payload because `NoiseCipher` grew to hold inline AES round keys, making the unboxed variant too large relative to `Fallback`) or `Fallback` (pivot into XXfallback)
...
- **Pre-keyed Noise transport cipher** — `NoiseCipher` holds an `Aes256GcmKey` that precomputes the AES key schedule and GHASH subkey once per connection instead of on every frame. Decrypt improves ~20% on 1500-byte stanzas (AES-NI host); encrypt improves ~7%. See [changelog](/changelog/2026-06-11-noise-prekeyed-gcm).
