...
| `spawn_detached` | Spawn a fire-and-forget task without ever building an `AbortHandle`. Defaults to `spawn(future).detach()`; override only if your runtime can skip that allocation entirely (see [#1124](https://github.com/oxidezap/whatsapp-rust/pull/1124)) |
