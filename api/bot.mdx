...
Registers a handler for [`Event::PairingCodeError`](/concepts/events#pairingcodeerror), fired when a pair-code request fails and no code will be issued. The counterpart to [`on_pair_code_refresh`](#on_pair_code_refresh) on the failure path, and the **only** way to observe a [`with_pair_code`](#with_pair_code) request failing — that request runs in a detached task, so its `Err` reaches no caller.
...
Bot::builder()
    ...
    .on_pair_code_error(|err, client| async move {
        eprintln!("Pair code request failed: {}", err.error);
        match err.rejection {
            Some(r) if r.is_throttled() => {
