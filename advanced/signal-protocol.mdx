...
`SenderKeyState` memoizes the parsed signing `PrivateKey` in a `OnceLock<PrivateKey>` field alongside the protobuf state. The key is pre-warmed via `PrivateKey::precompute_signing_cache()` before memoization — clones of an initialized `OnceLock` carry its contents, so every caller receives a warm clone and skips the XEdDSA scalar + Edwards basepoint re-derivation.

**Population points:**

- **`SenderKeyState::new`** — the caller-supplied key is inserted directly; no re-deserialization
- **`signing_key_private()` on first call after a cold load** — the key is deserialized, warmed, then memoized; a benign concurrent-first race computes equal values and `OnceLock` discards the loser silently
