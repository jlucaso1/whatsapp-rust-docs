...
- **Full media plane** - Mic capture, encode, encrypt, relay, decrypt, decode, and playout in pure Rust
- **Pure Rust MLow codec** - WhatsApp's proprietary audio codec, no FFI; compiles to WASM and embedded targets
- **E2E-SRTP encryption** - Call key from the Signal session, HKDF-derived SRTP keys, AES-128-CTR + WARP integrity
- **Sans-IO `CallEngine`** - No owned sockets or threads; runs on Tokio, WASM, and embedded targets (esp32)
