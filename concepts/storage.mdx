...
pub trait SignalStore: Send + Sync {
    ...
    async fn get_max_prekey_id(&self) -> Result<u32>;  // For ID collision prevention
...
<Note>
The `get_max_prekey_id()` method returns the highest pre-key ID in the store. This is used alongside `Device.next_pre_key_id` to prevent ID collisions during pre-key upload.
</Note>
...
pub struct Device {
    ...
    pub signed_pre_key: KeyPair,           // Signed pre-key for session init
    pub signed_pre_key_id: u32,            // Signed pre-key identifier
    pub account: Option<wa::AdvSignedDeviceIdentity>,
    pub next_pre_key_id: u32,              // Monotonic pre-key ID counter
    // ... more fields
}
```

<Note>
The `next_pre_key_id` field is a monotonically increasing counter matching WhatsApp Web's `NEXT_PK_ID` pattern. It prevents pre-key ID collisions when keys are consumed non-sequentially from the store. See the [Signal Protocol](/advanced/signal-protocol#pre-key-management) page for details.
</Note>
...
pub enum DeviceCommand {
    ...
    SetDeviceProps(Option<String>, Option<AppVersion>, Option<PlatformType>),
