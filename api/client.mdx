...
<Note>
A terminal shutdown (`disconnect()`, `logout()`, or `signal_shutdown_sync()`) that lands while `run()` is parked in its reconnect backoff interrupts the wait immediately instead of waiting it out. The backoff can run up to the 900s cap (see [Auto-Reconnection](#auto-reconnection)), so without this a caller `.await`ing `run()` right after requesting a stop — e.g. `Bot::run` — could see it hang for up to 15 minutes. A per-connection shutdown, the kind `run()` itself reconnects from, does *not* cut the backoff short; only a terminal shutdown does.
</Note>
...
| `disconnect()` / `logout()` / `signal_shutdown_sync()` during a backoff wait | Wait is interrupted immediately; `run()` returns without waiting out the remaining delay |
