...
6. Wait, racing the delay against a terminal shutdown so `disconnect()`/`logout()`/`signal_shutdown_sync()` interrupts it immediately instead of holding `run()` for up to the 900s cap → attempt reconnection (with 20s connect timeout)
