...
When a new LID-PN mapping is learned (via `add_lid_pn_mapping`), the client scans devices 0–99 for PN-keyed sessions and migrates them:
...
pub(crate) async fn migrate_signal_sessions_on_lid_discovery(&self, pn: &str, lid: &str) {
    for device_id in 0..=99u16 {
        // If PN session exists and no LID session → move session to LID address
        // If both exist → delete the stale PN session
...
2. Attempt to migrate PN sessions to LID (same logic as proactive migration)
3. Reload the session and identity into the signal cache
4. Retry decryption with the migrated session
5. If `DuplicatedMessage` occurs during post-migration retry, it is silently ignored
6. Fall back to retry receipt only if migration does not resolve the issue
...
<Note>
  The proactive migration runs synchronously when a LID mapping is first discovered. The on-the-fly migration handles the case where the database already contains stale PN sessions from before the mapping was known.
</Note>
...
The DM send path encrypts to multiple devices (the recipient's linked devices plus the sender's own linked devices). To prevent ratchet desync when concurrent sends and receives operate on the same Signal session, the DM path acquires per-device session locks for **all** devices involved — not just the primary device. The `build_session_lock_keys()` helper:

1. Resolves each device JID to its encryption JID (handling LID-PN mapping)
2. Converts each to a protocol address string
3. Sorts and deduplicates the keys
4. Acquires locks in sorted order to prevent deadlocks

This ensures send and receive paths serialize on the exact same lock key for each device, matching the decrypt path's lock format (`message.rs`).
