...
- Gracefully exits when connection is cleaned up via shutdown notification

**Shutdown handling:**

The keepalive loop uses `tokio::select!` to listen for both the sleep interval and a shutdown signal. When the connection is cleaned up, the shutdown notifier signals the keepalive loop to exit immediately, preventing resource leaks and orphaned tasks.
