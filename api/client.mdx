...
- `Paused` - added in PR #1265. [`pause()`](#pause) is in effect. Unlike `Shutdown` this is not final — [`resume()`](#resume) lifts it and `connect()` works again. Refused at every checkpoint of the connect graph, so an attempt already in flight when `pause()` lands is retracted rather than published.
...
Added in PR #1265. Drops the current connection and keeps it down until [`resume()`](#resume) — the middle of the range between [`reconnect()`](#reconnect), which comes back on the library's schedule, and [`disconnect()`](#disconnect), which does not come back at all. The [`run()`](#run) supervision loop stays alive and parked, so the future a caller is awaiting keeps running; the client is **not** terminal, it is between connections on purpose.

Once `pause()` returns: the socket is closed, pending receipts and Signal state are flushed on the same terms as `disconnect()`, and **no connection will be opened by anyone** until `resume()` — [`connect()`](#connect) is refused with `ConnectError::Paused` for as long as the pause holds, including an attempt already in flight when `pause()` was called, which is retracted rather than published. Idempotent — pausing an already-paused client just tears down again.

<Note>
`pause()` dispatches no `Event::Disconnected` (the application ended this connection, so the teardown is not news — the same reasoning `reconnect()` applies), is not a protocol-level presence change (the account stays registered, other devices see nothing), and does not override `enable_auto_reconnect`: a client the application already told not to come back still stops when its connection ends, pause or no pause.
</Note>
...
Added in PR #1265. Releases a [`pause()`](#pause): the run loop reconnects at once, with no backoff owed for the offline window the application chose. Returns once the loop has been told, not once it is connected — wait for that with [`wait_for_connected()`](#wait_for_connected). A no-op on a client that is not paused, and on one that has since been [`disconnect()`](#disconnect)ed — `resume()` clears the pause, it does not undo a shutdown. Safe to call while a `pause()` is still tearing down; it does not wait for the teardown to finish, since that teardown ends in an untimed socket close.
...
| `pause()` called | No reconnect attempt; `run()` parks instead of connecting, no backoff owed |
| `resume()` called | Reconnects immediately, no backoff owed for the paused window |
