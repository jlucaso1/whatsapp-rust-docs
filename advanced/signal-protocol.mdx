...
| `BadMac` | Ratchet desync (`InvalidMessage` / mac failure) on an existing session | Mark the session for re-creation, throttled per peer via the new `session_recreate_history` map so repeated BadMacs don't loop, and re-send via a peer-addressed `pkmsg` carrying our identity. |

The throttle keeps a short history of recent recreate attempts per peer/device. If a peer is already in cooldown, the client skips re-creation and falls back to a normal retry receipt rather than thrashing the session. Peer-addressed `pkmsg` carries the protocol identity so the receiver can verify ownership before installing the new session, blocking spoofed sibling recoveries.
