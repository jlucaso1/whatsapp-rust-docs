...
Share the client because `Client::connect()` fetches `https://web.whatsapp.com/sw.js` to check for a version update, unless you've set `with_version` or the cached version is under 24h old. Each session that connects opens a pooled TLS connection this way, even if it never touches media. ureq keeps that connection idle instead of closing it, at roughly 98 KiB per client — a measured figure for one retained connection, not the same thing as the `resource_report()` pool-cap estimate below (see the note there for why they differ). Share one agent across sessions and you pay that cost once for the whole process instead of once per session, which is worth doing from roughly ten concurrent sessions up.

You can share the connection safely. `sw.js` is unauthenticated and identical for every session, and media requests carry their own auth per request, so a shared connection doesn't leak anything between sessions beyond what the shared source IP already reveals. Sharing doesn't limit concurrency either: ureq opens a new connection whenever no idle one matches the target host, so the pool only caps how many idle connections it retains, not how many requests can run at once.
