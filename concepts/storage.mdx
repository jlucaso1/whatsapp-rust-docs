...
<Note>
Coordination caches (`session_locks`, `chat_lanes`), the signal write-behind cache, `msg_secret_buffer`, and `pdo_pending_requests` always stay in-process — they hold live Rust objects (mutexes, channel senders) that cannot be serialized to an external store.
</Note>
