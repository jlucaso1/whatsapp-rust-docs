...
- `AppVersionFallback::compiled_default` — `true` when that version is the one compiled into the library, so its staleness is the release's age. `false` when the device had already resolved a version earlier (or was given one via `with_version`), so its staleness is only the length of this outage.
