...
**Emitted:** When a user or group profile picture changes
...
**Emitted:** For group metadata changes, participant updates, and administrative actions
...
pub struct GroupUpdate {
    ...
    /// The specific action (subject change, participant add/remove, etc.)
...
Event::GroupUpdate(update) => {
    println!("Group {} updated by {:?}", update.group_jid, update.participant);
    match &update.action {
        GroupNotificationAction::Subject { subject, .. } => {
            println!("Subject changed to: {}", subject);
        }
        GroupNotificationAction::Add { participants, .. } => {
            println!("Added: {:?}", participants);
        }
        GroupNotificationAction::Remove { participants, .. } => {
            println!("Removed: {:?}", participants);
        }
        _ => {}
    }
}
...
pub struct StarUpdate {
    pub chat_jid: Jid,
    /// The participant who sent the message (Some for group messages, None for 1-on-1)
