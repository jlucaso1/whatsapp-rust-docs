...
The `Device` struct implements the libsignal `SessionStore`, `IdentityKeyStore`, and other traits. These are wrapped by `SignalProtocolStoreAdapter`, which adds the `SignalStoreCache` layer for deferred writes:
...
impl SessionStore for Device {
    async fn load_session(
        ...
        // Reads from the SignalStore backend trait
    }
}
```
...
The `SignalStoreCache` skips writes when the incoming data is byte-identical to the cached value. This avoids marking entries as dirty and eliminates unnecessary database writes during `flush()`.
...
This optimization is particularly impactful during message decryption, where Signal sessions are loaded into the cache and then "saved" back even when the session state has not changed. Without this check, every decryption would produce a dirty entry and trigger an unnecessary database write on the next `flush()`.
