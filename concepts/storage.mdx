...
History sync's `Other`-sourced seed itself draws from two places inside the sync: the bulk `HistorySync.phoneNumberToLidMappings` block, and each conversation's own `pnJid`/`lidJid` fields — a conversation named in one namespace can still yield a complete pair from just the opposite field, and in practice the bulk block omits pairs the conversations carry. Both are harvested, then collapsed to one pair per LID before either reaches the cache: the bulk block is the server's explicit identity table, so it outranks a conversation-derived pair naming the same LID or phone number; ties within one source keep the first occurrence. This resolution happens ahead of, and independently from, the source-aware write policy above — `LidPnCache` never sees more than one candidate pair per LID out of a single history sync ([#1345](https://github.com/oxidezap/whatsapp-rust/pull/1345)).
