...
<Note>
  Once the peer accepts, `whatsapp-rust`'s own `<call>` stanza handler completes the handshake for you — it sends the standalone `Enabled` stanza that ungates your local video plane automatically, no further call required. `handle.events()` still surfaces `CallEvent::VideoStateChanged { state: VideoState::UpgradeAccept, .. }` at that point, purely as a notification (e.g. to update your UI) — react to it if you want, but don't call `handle.announce_video_enabled()` in response, or you'll send a redundant second `Enabled`. That method exists for consumers driving call signaling themselves outside the standard handler.
</Note>
