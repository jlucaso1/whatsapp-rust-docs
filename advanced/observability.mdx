...
To tag your own spans or error context with the same account identity, call `Client::identity_tags()` (also gated by the `tracing` feature):

```rust
#[cfg(feature = "tracing")]
{
    let tags = client.identity_tags();
    tracing::info!(lid = ?tags.lid, pn = ?tags.pn, "custom event");
}
```

`identity_tags()` returns an `IdentityTags { lid: Option<String>, pn: Option<String> }` snapshot — a named struct rather than a tuple, so LID/PN cannot be silently transposed at the call site. Both fields read from the same device snapshot used internally, so they stay consistent with what the library's own spans record.
