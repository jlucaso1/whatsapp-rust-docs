...
Handle WhatsApp app state synchronization. This example's `app_state_versions` table needs a `bootstrapped` column alongside `name`, `version`, and `hash` — add it via a migration if you already have this table from before that field existed:
