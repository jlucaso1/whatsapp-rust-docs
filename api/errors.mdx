...
As of PR #1299, a DM send that reaches no device of its recipient now fails instead of silently succeeding. A DM's recipient devices and the sender's own companion devices write into the same participant list, so the old guard — which only checked whether that list was empty — missed the case where every recipient device failed to encrypt (no session, a refused pre-key bundle) but one of the sender's own companions still succeeded: the stanza went out carrying only the sender's own devices, the server acked it, and `send_message` returned `Ok` even though the recipient received nothing. Both ways a DM can now lose its recipient — every resolved recipient device failing to encrypt, and the fan-out resolving no recipient device at all for a destination that isn't one of the sender's own identities — surface as the new `SendError::NoRecipientDevice(wacore::send::NoRecipientDeviceError)` variant instead of `Ok` or the `Internal` catch-all. See [`NoRecipientDeviceError`](#norecipientdeviceerror) below.
...
`NoRecipientDevice` — added in PR #1299. A DM could not be encrypted for a single device of its recipient, so nothing was sent. Distinct from a transport failure: the connection is fine and the message id was never on the wire, so the useful retry is one that re-resolves the device list (`Freshness::Refresh`) and tries again, not an immediate resend. Wraps [`NoRecipientDeviceError`](#norecipientdeviceerror), the typed cause from `wacore`, reachable via `source()`.
...
match client.send_message(to.clone(), message).await {
    ...
    Err(SendError::NoRecipientDevice(e)) => {
        ...
        // Worth retrying with a forced device-list refresh, not an immediate resend.
    }
    Err(e) => eprintln!("send failed: {e}"),
}
```
