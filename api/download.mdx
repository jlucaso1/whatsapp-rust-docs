...
| Method | Description |
|--------|-------------|
| `verify_and_decrypt(data, media_key, media_type, file_length)` | Verifies HMAC and decrypts AES-256-CBC in one call |
| `decrypt_stream(reader, media_key, media_type)` | Streaming decryption from a reader |
| `decrypt_stream_to_writer(reader, writer, media_key, media_type)` | Streaming decryption directly into a writer |
| `validate_plaintext_sha256(data, expected_sha256)` | Validates SHA-256 hash of plaintext media |
| `prepare_download_requests(direct_path, ...)` | Builds CDN request URLs with host failover |
