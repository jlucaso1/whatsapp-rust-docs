...
whatsapp-rust supports end-to-end encrypted 1:1 voice and video calls that interoperate with the official WhatsApp app. The full audio media path is implemented in pure Rust: mic capture, encoding, E2E-SRTP encryption, relay transport, decryption, decoding, and playout.
...
```rust
// Upgrade an audio-only call to video (we initiate).
handle.start_video(camera_source, video_sink).await?;

// Accept a peer's upgrade request, surfaced as
// CallEvent::VideoStateChanged { state: VideoState::UpgradeRequestV2, .. }.
...
| `handle.announce_video_enabled().await` | Send the standalone `Enabled` stanza after a mid-call upgrade completes (not needed for video-from-start) |
