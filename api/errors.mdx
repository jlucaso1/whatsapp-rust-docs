...
pub enum GroupError {
    ...
    InvalidRequest(String),
    #[error("group description changed on the server since it was last read")]
    DescriptionConflict,
    #[error("{0}")]
    Internal(#[from] anyhow::Error),
}
...
If you were walking `source()` by hand to recover a server rejection or classify a failure, switch to [`ErrorChainExt`](#error-chain-recovery):
