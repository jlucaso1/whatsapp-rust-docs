...
The `Bots` feature fetches the server's directory of first-party AI bots offered to this account: a default bot plus display sections of `(jid, persona_id)` pairs.
...
Every section is returned, not just ones typed `all`. WhatsApp Web itself reads bots out of every section and only uses `type`/`display_type` for presentation, so a bot whose sole carrier is a `category` or `featured` section stays reachable.
...
`display_type` is present only in `v="3"` responses.
...
- [MEX (GraphQL)](/api/mex) - GraphQL operations, including bot-profile hydration
