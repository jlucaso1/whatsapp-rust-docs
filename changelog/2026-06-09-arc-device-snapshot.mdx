...
```rust
// Before
let device = client.get_device_snapshot().await;
...
// After
let snapshot = client.get_device_snapshot();   // fn, no .await
...
`get_device_arc()` is kept but its role is narrowed to store-adapter internals that need `&mut Device` trait access. All plain read paths should use `get_device_snapshot()` instead.
